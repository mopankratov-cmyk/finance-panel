-- Выкупы РНП → цена до СПП. Колонка price_with_disc в wb_sales + RPC coalesce(price_with_disc, finished_price).
alter table public.wb_sales add column if not exists price_with_disc numeric;
...
language sql stable as $$
  with o as (select date::date d, count(*)::int oc,
    ...
    from wb_orders where date::date between p_from and p_to and coalesce(is_cancel,false)=false group by 1),
  s as (select date::date d, count(*)::int bc,
      sum(coalesce(price_with_disc, finished_price, 0)) bs
    from wb_sales where date::date between p_from and p_to group by 1),
  a as (select date::date d, sum(coalesce(spent,0)) ad
    from wb_advert_nm_daily where date::date between p_from and p_to group by 1)
...
language sql stable as $$
  with o as (select date::date d, nm_id, count(*)::int oc,
      sum(coalesce(total_price,0)*(1-coalesce(discount_percent,0)/100.0)) os
    from wb_orders where date::date between p_from and p_to and coalesce(is_cancel,false)=false group by 1,2),
  s as (select date::date d, nm_id, count(*)::int bc,
      sum(coalesce(price_with_disc, finished_price, 0)) bs
    from wb_sales where date::date between p_from and p_to group by 1,2),
  a as (select date::date d, nm_id, sum(coalesce(spent,0)) ad
    from wb_advert_nm_daily where date::date between p_from and p_to group by 1,2)
