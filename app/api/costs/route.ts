...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await db.from("product_costs").select("article, name, cost_rub, brand").order("article");
  if (error) return NextResponse.json({ rows: [], error: error.message });
  let rows = (data ?? []).map((r) => ({ article: r.article as string, name: (r.name as string) ?? "", cost_rub: Number(r.cost_rub ?? 0), brand: (r.brand as string) ?? "" }));
...
export async function POST(request: NextRequest) {
  ...
  if (existing) {
    ...
    ({ error } = await db.from("product_costs").insert({ article, cost_rub: cost, name: (b.name || "").trim() || article, entity: OPIU_ENTITY }));
  }
