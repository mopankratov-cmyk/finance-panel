...
export async function POST(req: NextRequest) {
  try {
    ...
    let dbErr: string | null = null; // не маскируем сбой БД под «пустую библиотеку» или пустое имя товара
    if (!name && article) {
      const { data, error } = await db.from("product_costs").select("name").eq("article", article).maybeSingle();
    ...
    if (article) {
      ...
      if (error && !dbErr) dbErr = error.message;
