...
export async function GET(req: NextRequest) {
  ...
  const safe = async (fn: () => Promise<any>, fallback: any) => { try { const v = await fn(); return v ?? fallback; } catch { return fallback; } };
  ...
  const hooks_by_niche = await safe(async () => {
  ...
  // 3) История генераций + тренд ОТК (generation_history)
  const gh = await safe(async () => {
  ...
  // тренд ОТК по дням: средний балл + проход/фейл
  const byDay: Record<string, { sum: number; n: number; pass: number; fail: number; gen: number }> = {};
  for (const r of gh as Row[]) {
    const day = String(r.created_at || "").slice(0, 10) || "?";
    byDay[day] = byDay[day] || { sum: 0, n: 0, pass: 0, fail: 0, gen: 0 };
    byDay[day].gen++;
    ...
    if (r.status === "otk_fail" || r.status === "rejected" || r.status === "artifact_fail") byDay[day].fail++;
  }
  const otk_trend = Object.entries(byDay).map(([day, v]) => ({ day, avg_otk: v.n ? Math.round((v.sum / v.n) * 10) / 10 : null, pass: v.pass, fail: v.fail, gen: v.gen })).sort((a, b) => a.day.localeCompare(b.day));

  // 4) Пресеты-победители (node_templates from_winner) — переиспользуемые формулы
  const winner_presets = await safe(async () => {
  ...
  const winners = await safe(async () => {
