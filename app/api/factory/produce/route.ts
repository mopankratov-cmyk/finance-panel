...
- "ai_avatar" (НЕНАДЁЖНО, по умолчанию НЕ брать) — говорящий AI-аватар (HeyGen). Часто «AI-запах»: липсинк не в губы, мёртвые глаза, роботичная подача. ОТК по кадрам это НЕ ловит (движение/звук не видны), поэтому маршрут рискованный. Бери ТОЛЬКО если явный talking-head и НЕТ альтернативы; почти всегда лучше slideshow или real_ugc.
...
1. «Рассказывает / отзыв / founder / GRWM / talking-head» → real_ugc (живой человек) или slideshow с закадром. ai_avatar — только как последний вариант, если ничего другого нет (он ненадёжен).
