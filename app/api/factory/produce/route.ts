...
Верни СТРОГО JSON: {"route":"slideshow|ai_generation_ref|ai_generation|repurpose_cut|real_ugc","engine":"seedance|kling|pika (ТОЛЬКО если route=ai_generation_ref, иначе пусто)","why":"кратко","needs":"что нужно","alt_route":"запасной","cost":"low|medium|high"}. Только JSON.`;
  ...
  try {
    ...
    if (!decision) return NextResponse.json({ target_platform: targetPlatform, ...fallbackDecision({ available: av, reason: "пустое/нечитаемое решение продюсера" }), blueprint: blueprintResult.blueprint, blueprint_valid: blueprintResult.valid, blueprint_errors: blueprintResult.errors, hook_policy: blueprintResult.hook_policy });
    return NextResponse.json({ target_platform: targetPlatform, decision, blueprint: blueprintResult.blueprint, blueprint_valid: blueprintResult.valid, blueprint_errors: blueprintResult.errors, hook_policy: blueprintResult.hook_policy });
  } catch (e) {
