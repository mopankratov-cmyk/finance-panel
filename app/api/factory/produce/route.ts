...
3. ЛЮБОЙ показ товара — распаковка, демо, текстура, нанесение, до/после, обзор → slideshow (реальное фото, товар точный). Даже если «хочется движения» — НЕ ai_generation: он испортит товар. Нужна реальная динамика нанесения → real_ugc (креатор), не AI.
4. ai_generation выбирай ПОЧТИ НИКОГДА — только если в кадре НЕТ конкретного узнаваемого товара (абстрактный фон, эмоция, общий лайфстайл). Для WB-товара это почти всегда НЕ тот случай.
5. Сомневаешься → slideshow (это безопасный дефолт).
