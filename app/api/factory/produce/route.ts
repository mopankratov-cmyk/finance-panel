...
ПРАВИЛА выбора (ВИДЕО — приоритет: Seedance теперь надёжен на всех формах, держит и форму, и текст):
1. «Рассказывает / отзыв / founder / GRWM / talking-head» → real_ugc (живой человек). AI-аватары НЕ используем.
2. Есть реальная съёмка товара → repurpose_cut.
3. ДЕФОЛТ для показа товара = "ai_generation_ref" с engine "seedance" — ДИНАМИЧНОЕ ВИДЕО. Бери для БОЛЬШИНСТВА идей (POV, hero, до/после, обзор, лайфхак, распаковка, «что влезает»). Лента любит движение — видео > статика.
4. "slideshow" (карусель) — ТОЛЬКО когда формат реально про МНОГО текстовых пунктов сразу (чистый listicle «5 фактов / 3 причины с тезисами на экране»), где важнее текст, чем движение. Это спец-случай, НЕ дефолт.
5. Чистая атмосфера БЕЗ конкретного товара → ai_generation.
6. Сомневаешься → "ai_generation_ref" + "seedance" (видео — новый дефолт; ОТК отфильтрует брак и при провале сам сделает фолбэк на карусель).
