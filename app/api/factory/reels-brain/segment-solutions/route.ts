...
export async function GET(req: NextRequest) {
  try {
    ...
    const solutions = buildReelsBrainSegmentSolutions({
      decisionSnapshot: body as {
        summary?: Record<string, unknown> | null;
        ship_now?: Record<string, unknown>[];
        validate_next?: Record<string, unknown>[];
        research_queue?: Record<string, unknown>[];
    ...
    return NextResponse.json({
      error: "segment-solutions reels-brain упал: " + String((e as Error)?.message || e).slice(0, 180),
    }, { status: 500 });
  }
}
