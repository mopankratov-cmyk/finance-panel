...
export async function GET(req: NextRequest) {
  try {
    ...
    if (niche) {
      ...
      } catch (e) { warnings.push("reels brain summary unavailable: " + String((e as Error)?.message || e).slice(0, 140)); }
      return NextResponse.json({ ok: true, niche, feed, templates, recipes, platform_summary, preferred_source_providers, source_provider_history, incidents, warnings }, { headers: { "Cache-Control": "no-store" } });
    }
    ...
    } catch (e) { warnings.push("reels brain portfolio unavailable: " + String((e as Error)?.message || e).slice(0, 140)); }

    return NextResponse.json({ ok: true, niches, generations, recipes, observability, reels_brain_portfolio, warnings }, { headers: { "Cache-Control": "no-store" } });
  } catch (e) {
