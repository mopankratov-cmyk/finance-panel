...
function errorJson(message: string, init: ResponseInit, extra: Record<string, unknown> = {}) {
  return NextResponse.json({ error: message, detail: message, ...extra }, init);
}
...
export async function POST(req: NextRequest) {
  if (!creatifyReady()) return errorJson("Creatify не подключён: добавь CREATIFY_API_ID и CREATIFY_API_KEY в Vercel env", { status: 503 });
  ...
  if (images.length || url) {
    ...
    if (res.error || !res.token) return errorJson(res.error || "Creatify не запустил", { status: 502 }, debugMode ? { debug: res.debug } : {});
  ...
  if (!script) return errorJson("Нужен товар (артикул/URL) или текст для актёра", { status: 400 });
  const res = await creatifyLipsync(script, { creator: (body.creator || "").trim() || undefined });
  if (res.error || !res.token) return errorJson(res.error || "Creatify не запустил", { status: 502 });
