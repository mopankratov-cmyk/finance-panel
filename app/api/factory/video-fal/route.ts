  ...
  if (!prompt) {
    ...
    try {
      ...
      if (client) {
        ...
        const res = await client.messages.create({ model: "claude-sonnet-4-6", max_tokens: 350, system: sys, messages: [{ role: "user", content: `Товар: ${product}. Идея/настроение: ${brief || "показать товар эффектно"}.${shot_visual ? ` Первый кадр (из сценария): ${shot_visual}.` : ""}\nСКЕЛЕТ (пиши поверх, сохрани camera-move и preservation): ${skeleton}${recipe}` }] });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
