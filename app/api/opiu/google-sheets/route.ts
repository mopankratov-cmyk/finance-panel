...
export async function POST(request: Request) {
  ...
  for (const sheet of sheets) {
    ...
    const template = sheet.template?.trim();
    if (template === "opiu" && !sheet.sheetName?.trim().startsWith("Финансовый отчёт WB ")) {
    ...
    if (template === "opiu_monthly" && !sheet.sheetName?.trim().startsWith("ОПиУ ")) {
      return NextResponse.json({ error: "Лист ОПиУ должен иметь отдельное имя с префиксом «ОПиУ»" }, { status: 400 });
    }
  }
