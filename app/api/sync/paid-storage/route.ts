...
async function processCabinet(
  ...
  const deleteError = await db
    ...
    .gte("date", state.periodStart!)
    .lte("date", state.periodEnd!)
    .then((res) => res.error?.message ?? null);
  if (deleteError) {
    await writeWbSyncState(db, cabinetId, JOB, {
      cursor: state.frontier ?? null,
      status: "error",
    ...
    throw new Error(`очистка wb_paid_storage_rows перед перезаписью: ${deleteError}`);
  }
