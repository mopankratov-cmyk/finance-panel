...
async function copyDay(
  ...
  //
  // `taken` выше — снимок на момент чтения. Между ним и записью в клетку
  // мог прийти чужой апдейт, и обычный upsert затёр бы его переносом. Строка
  // с пустой note в этой таблице не хранится (пустая заметка — это удаление,
  // см. POST ниже), поэтому «клетка занята» и «строка существует» — одно и
  // то же: ignoreDuplicates просит базу саму, атомарно, вставлять только
  // туда, где строки ещё нет, и молча пропускать конфликт, а не переписывать
  // чужое решение.
  let copied = 0;
  for (let index = 0; index < fresh.length; index += 500) {
    ...
    const { error } = await db.from("wb_rk_notes").upsert(chunk.map((row) => ({
      ...
      source: "human",
      updated_at: stamp,
    })), { onConflict: "cabinet_id,nm_id,advert_id,date", ignoreDuplicates: true });
