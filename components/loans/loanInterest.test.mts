...
test("monthly loan interest is fixed and rounded to kopecks", () => {
  assert.equal(fixedMonthlyInterest(5_000_000, 38), 158_333.33);
});
...
test("a five-percent monthly rate is split by period days and tranche dates", () => {
  ...
  assert.equal(june16?.interest, 952.92);
  assert.equal(june30?.interest, 845.83);
