    ...
    <div className={`mx-auto max-w-[1600px] space-y-4 px-4 py-4 transition-opacity sm:px-5 ${updating ? "opacity-60" : ""}`}>
      {loading && !data ? <OzonLoading rows={10} /> : noCabinets ? <EmptyState title="Кабинет Ozon не подключён" detail="Добавьте кабинет с ключами Seller API и Performance API — после этого экраны наполнятся данными." href="/cabinets" /> : error && !data ? <OzonError message={error} onRetry={reload} /> : !data ? <EmptyState title="Нет заказов" detail="Проверьте период и Seller API." /> : <>
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-2 border-b border-slate-100 p-3 xl:flex-row xl:items-center"><div className="flex gap-1">{(["all", "FBO", "FBS"] as const).map((value) => <button key={value} onClick={() => setScheme(value)} className={`min-h-11 rounded-lg px-3 text-xs font-semibold sm:min-h-8 ${scheme === value ? "bg-sky-700 text-white" : "bg-slate-50 text-slate-600"}`}>{value === "all" ? "Все схемы" : value}</button>)}</div><select value={state} onChange={(event) => setState(event.target.value as typeof state)} className="h-11 rounded-lg border border-slate-200 bg-white px-3 text-xs text-slate-600 sm:h-8"><option value="all">Все статусы</option><option value="active">В работе</option><option value="awaiting">Ждут отгрузки</option><option value="delayed">Просроченные</option><option value="delivered">Доставленные</option><option value="cancelled">Отменённые</option></select><div className="xl:ml-auto"><OzonCsvButton count={rows.length} onExport={exportCsv} /></div><label className="relative flex-1 xl:max-w-sm"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Номер, товар, артикул, кабинет" className="h-11 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-xs outline-none focus:border-sky-400 sm:h-8" /></label></div>
          {rows.length === 0 ? <div className="p-4"><EmptyState title="Заказы не найдены" detail="Измените фильтр, поиск или период." /></div> : <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr>
            <SortableTh label="Отправление" align="left" active={sort?.key === "postingNumber"} dir={sort?.dir ?? "desc"} onToggle={() => setSort((c) => nextSortState(c, "postingNumber"))} />
