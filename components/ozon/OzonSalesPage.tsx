...
interface SalesRow {
  ...
  // null — разнесения расхода по товарам за период нет. Ноль читался бы как
  // «на этот товар не тратили», а это другое утверждение.
  crCart: number | null; crOrder: number | null; stock: number; adSpend: number | null; drr: number | null; adAllocated?: boolean;
        ...
        {loading && !data ? <OzonLoading rows={9} /> : noCabinets ? <EmptyState title="Кабинет Ozon не подключён" detail="Добавьте кабинет с ключами Seller API и Performance API — после этого экраны наполнятся данными." href="/cabinets" /> : error && !data ? <OzonError message={error} onRetry={reload} /> : !data ? <EmptyState title="Нет данных о продажах" detail="Проверьте выбранный кабинет и Seller API." /> : <>
          ...
          <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="flex flex-col gap-3 border-b border-slate-100 p-3 lg:flex-row lg:items-center">
              ...
              <label className="relative flex-1 lg:max-w-sm"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" enterKeyHint="search" placeholder="Товар, артикул, SKU, кабинет" className="h-11 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-xs outline-none focus:border-sky-400 sm:h-8" /></label>
