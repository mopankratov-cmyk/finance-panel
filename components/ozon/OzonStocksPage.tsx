...
import { OzonCsvButton, EmptyState, SortableTh, Freshness, MetricCard, OzonError, OzonLoading, OzonStaleNotice, OzonWarnings, ProductCell, StatusPill, formatNumber } from "./OzonUi";
...
type StockSortKey = "name" | "free" | "reserved" | "orders" | "dailySales" | "daysCover" | "reorderQty";
...
export function OzonStocksPage() {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = (data?.rows ?? []).filter((row) => (status === "all" || row.status === status)
      && (!needle || `${row.name} ${row.offerId} ${row.cabinet}`.toLocaleLowerCase("ru-RU").includes(needle)));
    // Порядок сервера — «сначала то, что горит»: критичные запасы сверху.
    // Он осмысленный, поэтому третий клик по заголовку возвращает именно его.
    return sortRows(filtered, sort, (row, key) => key === "name" ? `${row.name} ${row.offerId}` : row[key]);
  }, [data?.rows, query, sort, status]);
  const exportCsv = () => {
    if (!data) return;
    downloadCsv(
  ...
  return <div>
    ...
    <div className={`mx-auto max-w-[1600px] space-y-4 px-4 py-4 transition-opacity sm:px-5 ${updating ? "opacity-60" : ""}`}>
      {loading && !data ? <OzonLoading rows={10} /> : noCabinets ? <EmptyState title="Кабинет Ozon не подключён" detail="Добавьте кабинет с ключами Seller API и Performance API — после этого экраны наполнятся данными." href="/cabinets" /> : error && !data ? <OzonError message={error} onRetry={reload} /> : !data ? <EmptyState title="Нет остатков" detail="Проверьте Seller API и выбранный кабинет." /> : <>
        <div className="flex flex-wrap items-center justify-between gap-2"><div className="text-xs font-semibold text-slate-600">{data.scope.label} · спрос за {days(period.days)}</div><Freshness generatedAt={data.generatedAt} /></div>{error ? <OzonStaleNotice message={error} onRetry={reload} /> : null}<OzonWarnings warnings={data.warnings} />
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <div className="flex flex-col gap-2 border-b border-slate-100 p-3 lg:flex-row lg:items-center"><div className="flex gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [&>*]:shrink-0 sm:flex-wrap sm:overflow-visible">{(["all", "out", "critical", "warning", "ok", "overstock"] as StockStatus[]).map((value) => <button key={value} type="button" onClick={() => setStatus(value)} className={`min-h-11 rounded-lg px-2.5 text-[11px] font-semibold sm:min-h-8 ${status === value ? "bg-sky-700 text-white" : "bg-slate-50 text-slate-600 hover:bg-slate-100"}`}>{({ all: "Все", out: "Нет остатка", critical: "≤ 7 дней", warning: "≤ 14 дней", ok: "Норма", overstock: "Излишек" } as Record<StockStatus, string>)[value]}</button>)}</div><div className="flex items-center gap-2 lg:ml-auto lg:contents"><div className="lg:ml-auto"><OzonCsvButton count={rows.length} onExport={exportCsv} /></div><label className="relative flex-1 lg:max-w-sm"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" enterKeyHint="search" placeholder="Товар, артикул, кабинет" className="h-11 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-xs outline-none focus:border-sky-400 sm:h-8" /></label></div></div>
          {rows.length === 0 ? <div className="p-4">{data.summary.cabinetsWithStocks === 0 ? <EmptyState title="Ozon не отдал остатки" detail="Это сбой на стороне Ozon, а не пустой склад: причина — в блоке «Часть данных недоступна» выше. Нажмите «Обновить» через пару минут." /> : <EmptyState title="Нет товаров в этом срезе" detail="Измените фильтр, поиск или период спроса." />}</div> :<div className="scroll-x"><table className="w-full min-w-[1080px] text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr>
              {([["name", "Товар", "left"], ["free", "Доступно", "right"], ["reserved", "Резерв", "right"], ["orders", "Продано", "right"], ["dailySales", "В день", "right"], ["daysCover", "Дней запаса", "right"], ["reorderQty", "К пополнению", "right"]] as [StockSortKey, string, "left" | "right"][]).map(([key, label, align]) => (
                <SortableTh key={key} label={label} align={align} sticky={key === "name"} active={sort?.key === key} dir={sort?.dir ?? "desc"} onToggle={() => setSort((current) => nextSortState(current, key))} />
              ))}
              <th className="px-3 py-2 text-left">Склады</th><th className="px-4 py-2 text-right">Статус</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key} className="group border-t border-slate-100 hover:bg-sky-50/40"><td className="sticky left-0 z-10 bg-white px-4 py-2 group-hover:bg-[#f9fdff]"><ProductCell image={row.image} name={row.name} code={row.offerId} cabinet={data.scope.count > 1 ? row.cabinet : undefined} /></td><td className={`px-3 py-2 text-right font-semibold tabular-nums ${row.free <= 0 ? "text-red-600" : ""}`}>{formatNumber(row.free)}</td><td className="px-3 py-2 text-right tabular-nums">{formatNumber(row.reserved)}</td><td className="px-3 py-2 text-right tabular-nums">{formatNumber(row.orders)}</td><td className="px-3 py-2 text-right tabular-nums">{row.dailySales.toLocaleString("ru-RU")}</td><td className="px-3 py-2 text-right font-semibold tabular-nums">{row.daysCover == null ? "Нет продаж" : `${row.daysCover.toLocaleString("ru-RU")} дн.`}</td><td className="px-3 py-2 text-right font-semibold tabular-nums text-sky-700">{row.reorderQty ? `${formatNumber(row.reorderQty)} шт.` : "—"}</td><td className="max-w-64 px-3 py-2 text-[10px] text-slate-500">{row.warehouses.slice(0, 3).map((warehouse) => `${warehouse.name}: ${formatNumber(warehouse.value)}`).join(" · ") || "—"}</td><td className="px-4 py-2 text-right"><StatusPill status={row.status} /></td></tr>)}</tbody></table></div>}
        </section>
        <p className="text-xs text-slate-400 md:text-[10px]">Рекомендация пополнения = прогноз продаж на 30 дней минус доступный остаток. Это ориентир, а не готовый заказ поставщику.</p>
      </>}
    </div>
  </div>;
}
