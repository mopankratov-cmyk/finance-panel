...
import { CalendarRange, ChevronDown, ChevronRight, Link2, Pencil, Trash2 } from "lucide-react";
        ...
        {groups.map(group => {
          ...
          return <Fragment key={group.key}>
            <tr id={`payment-${p.id}`} className={p.id === highlightedPaymentId ? "bg-emerald-100/70 ring-2 ring-inset ring-emerald-400" : group.chainId ? "bg-violet-50/30" : "hover:bg-slate-50/50"}>
              ...
              <td data-label="Статья" className="px-3 py-3 font-medium" title={group.chainId ? undefined : p.category}>{group.chainId ? <button type="button" aria-expanded={open} aria-controls={`parts-${group.key}`} onClick={() => toggle(group.key)} className="flex min-h-11 items-center gap-1 rounded-lg px-1 text-left text-violet-700 hover:bg-violet-50">{open ? <ChevronDown className="h-4 w-4 shrink-0"/> : <ChevronRight className="h-4 w-4 shrink-0"/>}<span>Частей: {group.parts.length}</span></button> : group.bankTransferId ? <button type="button" aria-expanded={open} aria-controls={`transfer-${group.key}`} onClick={() => toggle(group.key)} className="min-h-11 rounded-lg px-1 text-left hover:bg-emerald-50"><span className="lg:line-clamp-2">{p.category}</span><span className="mt-1 flex items-center gap-1 text-xs font-normal text-emerald-700"><Link2 className="h-3.5 w-3.5"/>{group.linkedTransfers?.length === 2 ? "Связаны 2 операции" : "Ожидается вторая сторона"}</span></button> : <span className="lg:line-clamp-2">{p.category}</span>}</td>
              <td data-cell="actions" className="px-2 py-3"><div className="flex items-center justify-end gap-1">{!group.chainId && !group.bankTransferId && p.amount < 0 && <button type="button" onClick={() => onOpen({ paymentId: p.id })} className="min-h-11 rounded-lg border border-violet-200 px-2 text-xs text-violet-700">Разбить</button>}{!group.chainId && !group.bankTransferId && p.amount < 0 && <button type="button" aria-label="Распределить расход по месяцам ОПиУ" title="По месяцам ОПиУ" onClick={() => onAllocateOpiu(p)} className="tap rounded-lg text-slate-500 hover:bg-violet-50 hover:text-violet-700"><CalendarRange className="h-4 w-4" /></button>}<button type="button" aria-label="Редактировать операцию" title="Редактировать" onClick={() => group.chainId ? onOpen({ paymentId: p.id }) : onEdit(p)} className="tap rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600"><Pencil className="h-4 w-4"/></button><button type="button" aria-label={group.chainId ? "Отменить операцию" : "Удалить платёж"} title={group.chainId ? "Отменить операцию" : "Удалить платёж"} onClick={() => group.chainId ? onOpen({ paymentId: p.id }) : onDelete(p.id)} className="tap rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4"/></button></div></td>
            </tr>
            {!group.chainId && group.bankTransferId && open && <tr id={`transfer-${group.key}`}><td colSpan={8} className="px-5 py-3"><div className="space-y-2 rounded-lg border border-emerald-100 bg-emerald-50/40 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2"><p className="flex items-center gap-2 text-sm font-medium"><Link2 className="h-4 w-4 text-emerald-700"/>Связанный перевод между счетами</p><b className={group.linkedTransfers?.length === 2 && Math.round((group.linkedTransfers ?? []).reduce((sum,item)=>sum+item.amount,0)*100)===0?"text-emerald-700":"text-amber-700"}>{group.linkedTransfers?.length === 2 ? "Суммы сходятся" : "Вторая сторона ещё не проведена"}</b></div>
              {(group.linkedTransfers ?? []).map(item=><div key={item.id} className="grid gap-1 rounded-lg bg-white p-3 text-sm sm:grid-cols-[100px_110px_1fr_1fr] sm:items-center"><span className="text-slate-500">{formatDate(item.date)}</span><b className={item.amount<0?"text-red-600":"text-emerald-700"}>{formatMoney(item.amount)}</b><span>{companyName(item.companyId)} · {accountName(item.accountId)}</span><span className="text-slate-600">{item.category}</span></div>)}
            </div></td></tr>}
