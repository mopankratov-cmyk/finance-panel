...
function StaffSection({ title, employees, companies, canViewPrivate: _canViewPrivate, onEdit, defaultOpen = false }: { title: string; employees: PayrollEmployee[]; companies: DdsCompany[]; canViewPrivate: boolean; onEdit: (employee: PayrollEmployee) => void; defaultOpen?: boolean }) {
  ...
  return <Card><details open={defaultOpen} className="group"><summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-5 py-4 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-violet-300"><div className="min-w-0 flex-1"><h3 className="font-bold text-slate-950">{title}</h3><p className="mt-0.5 text-sm text-slate-500">{employees.length} сотрудников</p></div><ChevronDown className="h-5 w-5 text-slate-400 transition-transform group-open:rotate-180" /></summary><div className="border-t border-slate-100"><table className="w-full table-fixed text-sm"><thead><tr className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="w-[28%] px-5 py-2.5">Сотрудник</th><th className="w-[13%] px-3 py-2.5">Статус</th><th className="w-[18%] px-3 py-2.5">Оформление</th><th className="hidden w-[18%] px-3 py-2.5 lg:table-cell">Компания</th><th className="hidden w-[10%] px-3 py-2.5 xl:table-cell">Город</th><th className="w-[13%] px-3 py-2.5 text-right">Оклад</th><th className="w-14 px-2 py-2.5"><span className="sr-only">Открыть карточку</span></th></tr></thead><tbody className="divide-y divide-slate-100">{employees.length === 0 ? <tr><td colSpan={columns} className="px-5 py-8 text-center text-slate-400">Сотрудников нет</td></tr> : employees.map((employee) => <tr key={employee.id} className="hover:bg-slate-50/70"><td className="px-5 py-2"><button type="button" onClick={() => onEdit(employee)} className="flex min-h-11 w-full cursor-pointer flex-col justify-center text-left hover:text-violet-700"><span className="truncate font-semibold text-slate-950">{employee.fullName}</span><span className="truncate text-xs text-slate-500">{employee.position || "Без должности"}</span></button></td><td className="px-3 py-2"><span className={`inline-flex max-w-full rounded-full px-2 py-1 text-xs font-bold ${employee.employmentStatus === "active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{employee.employmentStatus === "active" ? "Работает" : "Уволен"}</span></td><td className="px-3 py-2 text-slate-700"><span className="line-clamp-2">{EMPLOYMENT_LABELS[employee.employmentType]}</span></td><td className="hidden px-3 py-2 text-slate-700 lg:table-cell"><span className="line-clamp-2">{companyNames(employee) || "Не назначена"}</span></td><td className="hidden truncate px-3 py-2 text-slate-700 xl:table-cell">{employee.city || "—"}</td><td className="px-3 py-2 text-right font-semibold tabular-nums text-slate-900">{employee.monthlySalary ? formatMoney(employee.monthlySalary) : "—"}</td><td className="px-2 py-2"><button type="button" aria-label={`Открыть карточку ${employee.fullName}`} onClick={() => onEdit(employee)} className="inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-lg text-slate-500 hover:bg-violet-50 hover:text-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-300"><Pencil className="h-4 w-4" /></button></td></tr>)}</tbody></table></div></details></Card>;
}
