...
import { allocatePayrollPayment, deletePayrollEmployee, importPayrollStaffFile, importPayrollStaffPrivateFile, loadPayrollData, savePayrollDebt, savePayrollEmployee, savePayrollPeriod } from "./payrollStore";
...
function SummaryEmployeeSection({ title, employees, data, payments, debtByEmployee, onEdit, defaultOpen = false }: { title: string; employees: PayrollEmployee[]; data: PayrollData; payments: Payment[]; debtByEmployee: Map<string, number>; onEdit: (employee: PayrollEmployee) => void; defaultOpen?: boolean }) {
  ...
  const debtYears = [...new Set([2025, Number(todayISO().slice(0, 4)), ...data.debts.map((debt) => debt.debtYear), ...periods.map((period) => Number(period.payDate.slice(0, 4)))])].sort((left, right) => left - right);
  const paymentDates = [...new Set(data.allocations.map((allocation) => paymentById.get(allocation.paymentId)?.date ?? allocation.confirmedAt.slice(0, 10)))];
  const timeline = [
    ...periods.map((period) => ({ id: `salary:${period.id}`, date: period.payDate, label: `ЗП ${period.payDate.slice(8, 10)}.${period.payDate.slice(5, 7)}`, period })),
    ...paymentDates.map((date) => ({ id: `payment:${date}`, date, label: `Оплата ${date.slice(8, 10)}.${date.slice(5, 7)}`, period: null })),
  ...
  const targetLabel = (allocation: PayrollData["allocations"][number]) => {
    const opening = data.debts.find((debt) => debt.id === allocation.debtOpeningId);
    if (opening) return `в долг ${opening.debtYear}`;
    const period = data.periods.find((item) => item.id === data.entries.find((entry) => entry.id === allocation.entryId)?.periodId);
    return period ? `за ${formatDate(period.payDate)}` : "в зарплату";
  };
  const compactColumnCount = 2 + debtYears.length + timeline.length;
  return <Card><details open={defaultOpen} className="group"><summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-5 py-4 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-violet-300"><div className="min-w-0 flex-1"><h3 className="font-bold text-slate-950">{title}</h3><p className="mt-0.5 text-sm text-slate-500">{employees.length} сотрудников · оплаты и начисления по датам</p></div><ChevronDown className="h-5 w-5 text-slate-400 transition-transform group-open:rotate-180" /></summary><div className="overflow-x-auto border-t border-slate-100"><table className="w-full min-w-max text-sm"><thead><tr className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="sticky left-0 z-20 max-w-[45vw] min-w-[7.5rem] border-r border-slate-200 bg-slate-50 px-3 py-3 sm:max-w-none sm:min-w-[230px] sm:px-5">Сотрудник</th>{debtYears.map((year) => <th key={year} className="min-w-[120px] px-3 py-3 text-right">Долг {year}</th>)}<th className="min-w-[130px] border-l border-slate-200 px-3 py-3 text-right">Общий долг</th>{timeline.map((column) => <th key={column.id} className="min-w-[135px] border-l border-slate-200 px-3 py-3 text-center">{column.label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{employees.length === 0 ? <tr><td colSpan={compactColumnCount} className="px-5 py-8 text-center text-slate-400">Сотрудников нет</td></tr> : employees.map((employee) => <tr key={employee.id} className="hover:bg-slate-50/70"><td className="sticky left-0 z-10 max-w-[45vw] border-r border-slate-100 bg-white px-3 py-3 sm:max-w-none sm:px-5"><button type="button" onClick={() => onEdit(employee)} className="min-h-11 cursor-pointer text-left font-semibold text-slate-950 hover:text-violet-700"><span className="block break-anywhere">{employee.fullName}</span><span className="mt-0.5 block text-xs font-normal text-slate-500">{employee.position || EMPLOYMENT_LABELS[employee.employmentType]}</span></button></td>{debtYears.map((year) => { const amount = debtForYear(employee.id, year); return <td key={year} className={`px-3 py-3 text-right font-semibold tabular-nums ${amount > 0 ? "text-rose-700" : "text-slate-400"}`}>{formatMoney(amount)}</td>; })}<td className={`border-l border-slate-200 px-3 py-3 text-right font-bold tabular-nums ${(debtByEmployee.get(employee.id) ?? 0) > 0 ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>{formatMoney(debtByEmployee.get(employee.id) ?? 0)}</td>{timeline.map((column) => { if (column.period) { const entry = data.entries.find((item) => item.employeeId === employee.id && item.periodId === column.period?.id); return <td key={column.id} className="border-l border-slate-100 px-3 py-3 text-right font-semibold tabular-nums">{entry ? formatMoney(payrollEntryTotal(employee, draftFromEntry(entry))) : "—"}</td>; } const allocations = data.allocations.filter((item) => item.employeeId === employee.id && (paymentById.get(item.paymentId)?.date ?? item.confirmedAt.slice(0, 10)) === column.date); return <td key={column.id} className="border-l border-slate-100 px-3 py-3 text-right tabular-nums">{allocations.length ? allocations.map((item) => <span key={item.id} className="mb-1 block text-emerald-700"><strong>{formatMoney(item.amount)}</strong><span className="block text-[11px] text-slate-500">{targetLabel(item)}</span></span>) : "—"}</td>; })}</tr>)}</tbody></table></div></details></Card>;
...
function StaffSection({ title, employees, companies, canViewPrivate: _canViewPrivate, onEdit, defaultOpen = false }: { title: string; employees: PayrollEmployee[]; companies: DdsCompany[]; canViewPrivate: boolean; onEdit: (employee: PayrollEmployee) => void; defaultOpen?: boolean }) {
  const columns = 7;
  ...
  return <Card><details open={defaultOpen} className="group"><summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-5 py-4 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-violet-300"><div className="min-w-0 flex-1"><h3 className="font-bold text-slate-950">{title}</h3><p className="mt-0.5 text-sm text-slate-500">{employees.length} сотрудников</p></div><ChevronDown className="h-5 w-5 text-slate-400 transition-transform group-open:rotate-180" /></summary><div className="table-cards-lg border-t border-slate-100 p-3 lg:p-0"><table className="w-full table-fixed text-sm"><thead><tr className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="w-[28%] px-5 py-2.5">Сотрудник</th><th className="w-[13%] px-3 py-2.5">Статус</th><th className="w-[18%] px-3 py-2.5">Оформление</th><th className="hidden w-[18%] px-3 py-2.5 lg:table-cell">Компания</th><th className="hidden w-[10%] px-3 py-2.5 xl:table-cell">Город</th><th className="w-[13%] px-3 py-2.5 text-right">Оклад</th><th className="w-14 px-2 py-2.5"><span className="sr-only">Открыть карточку</span></th></tr></thead><tbody className="divide-y divide-slate-100">{employees.length === 0 ? <tr><td colSpan={columns} className="px-5 py-8 text-center text-slate-400">Сотрудников нет</td></tr> : employees.map((employee) => <tr key={employee.id} className="hover:bg-slate-50/70"><td data-cell="title" className="px-5 py-2"><button type="button" onClick={() => onEdit(employee)} className="flex min-h-11 w-full cursor-pointer flex-col justify-center text-left hover:text-violet-700"><span className="font-semibold text-slate-950 lg:truncate">{employee.fullName}</span><span className="text-xs font-normal text-slate-500 lg:truncate">{employee.position || "Без должности"}</span></button></td><td data-label="Статус" className="px-3 py-2"><span className={`inline-flex max-w-full rounded-full px-2 py-1 text-xs font-bold ${employee.employmentStatus === "active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{employee.employmentStatus === "active" ? "Работает" : "Уволен"}</span></td><td data-label="Оформление" className="px-3 py-2 text-slate-700"><span className="line-clamp-2">{EMPLOYMENT_LABELS[employee.employmentType]}</span></td><td data-label="Компания" className="hidden px-3 py-2 text-slate-700 lg:table-cell"><span className="line-clamp-2">{companyNames(employee) || "Не назначена"}</span></td><td data-label="Город" className="hidden px-3 py-2 text-slate-700 xl:table-cell xl:truncate">{employee.city || "—"}</td><td data-label="Оклад" className="px-3 py-2 text-right font-semibold tabular-nums text-slate-900">{employee.monthlySalary ? formatMoney(employee.monthlySalary) : "—"}</td><td data-cell="actions" className="px-2 py-2"><button type="button" aria-label={`Открыть карточку ${employee.fullName}`} onClick={() => onEdit(employee)} className="inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-lg text-slate-500 hover:bg-violet-50 hover:text-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-300"><Pencil className="h-4 w-4" /></button></td></tr>)}</tbody></table></div></details></Card>;
}
...
function PayrollLinesTable({ employees, drafts, companies, accounts, onEdit, onChange }: { employees: PayrollEmployee[]; drafts: Record<string, PayrollDraftEntry>; companies: DdsCompany[]; accounts: Account[]; onEdit: (employee: PayrollEmployee) => void; onChange: (employeeId: string, lines: PayrollAccrualLine[]) => void }) {
  return <div className="overflow-x-auto rounded-xl border border-slate-200"><table className="w-full min-w-[960px] table-fixed text-sm xl:min-w-0"><colgroup><col className="w-[22%]" /><col className="w-[19%]" /><col className="w-[19%]" /><col className="w-[13%]" /><col className="w-[13%]" /><col className="w-[14%]" /></colgroup><thead><tr className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="px-3 py-3">Сотрудник</th><th className="px-2 py-3">Компания</th><th className="px-2 py-3">Вид начисления</th><th className="px-2 py-3">Зарплата</th><th className="px-2 py-3">Налог</th><th className="px-2 py-3">Способ оплаты</th></tr></thead><tbody className="divide-y divide-slate-100">{employees.length === 0 ? <tr><td colSpan={6} className="px-5 py-10 text-center text-slate-400">На выбранный период сотрудников нет</td></tr> : employees.flatMap((employee) => {
    ...
    const patchLine = (id: string, patch: Partial<PayrollAccrualLine>) => onChange(employee.id, lines.map((line) => {
      ...
      else if (patch.amount !== undefined || patch.paymentMethod !== undefined || patch.kind !== undefined) next.taxAmount = payrollLineTaxAmount(employee, next);
      return next;
    }));
    const addLine = () => {
...
function EmployeeForm({ employee, companies, canViewPrivate, onChange, onSave, onDelete, onCancel, saving, preview }: { employee: PayrollEmployee; companies: DdsCompany[]; canViewPrivate: boolean; onChange: (employee: PayrollEmployee) => void; onSave: () => Promise<void>; onDelete?: () => Promise<void>; onCancel: () => void; saving: boolean; preview: boolean }) {
  const [deleteError, setDeleteError] = useState("");
  const patch = (value: Partial<PayrollEmployee>) => onChange({ ...employee, ...value });
  const remove = async () => {
    if (!onDelete || !window.confirm(preview ? `Убрать ${employee.fullName} из предпросмотра?` : `Удалить сотрудника ${employee.fullName}? Это можно сделать только при отсутствии начислений, долгов и оплат.`)) return;
    setDeleteError("");
    try { await onDelete(); }
    catch (caught) { setDeleteError(caught instanceof Error ? caught.message : "Не удалось удалить сотрудника"); }
  };
  return <div className="space-y-4">
    ...
    {deleteError && <p role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{deleteError}</p>}
    <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between"><div>{onDelete && <button type="button" disabled={saving} onClick={() => void remove()} className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-lg px-3 text-sm font-semibold text-rose-700 hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-rose-300 disabled:cursor-not-allowed disabled:opacity-40"><Trash2 className="h-4 w-4" />{preview ? "Убрать из предпросмотра" : "Удалить сотрудника"}</button>}</div><div className="flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-11 rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700">Закрыть</button><button type="button" disabled={saving || !employee.fullName.trim()} onClick={() => void onSave()} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-bold text-white disabled:opacity-40">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{preview ? "Сохранить в предпросмотре" : "Сохранить"}</button></div></div>
  </div>;
}
