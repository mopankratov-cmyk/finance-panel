...
export function SalesPlanTable({
  ...
  const hasLegacyStockRows = plan.rows.some((row) => !row.marketplaceStocks?.[monthKey]);
  ...
  return (
    <>
      <tr className="group h-9 hover:bg-slate-50/60">
        <td style={stickyOffset(STICKY_LEFT.product, STICKY_WIDTHS.product)} className="sticky z-20 border-b border-r border-slate-200 bg-white px-1.5 py-0.5">
          <div className="flex items-center gap-1">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className={`block truncate text-[9px] ${stockRisk.shortageQty ? "text-rose-500" : "text-slate-400"}`}>{row.variant} · {marketplace === "wb" ? "WB" : "SKU"} {row.externalId || "—"} · {stockRisk.forecastAvailable ? `прогноз ${number(stockRisk.endingStock)}${stockRisk.shortageQty ? ` · Дефицит ${number(stockRisk.shortageQty)} шт.` : ""}` : "Прогноз недоступен: нет непрерывного плана"}</span>
            </span>
        ...
        >
          {row.marketplaceStocks?.[monthKey] ? number(stockRisk.marketplaceStock) : "—"}{stockRisk.marketplaceStale ? <span className="block text-[8px]">устарело</span> : null}
        </td>
        <td
