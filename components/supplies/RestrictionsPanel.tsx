...
export function RestrictionsPanel({ cabinetId, onRows }: { cabinetId: string; onRows?: (rows: RestrictionRow[]) => void }) {
  ...
  return (
    // Растягиваться на две колонки панель должна только там, где их две:
    // на телефоне сетка одноколоночная, и span 2 ломал её для всех соседей.
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:col-span-2">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button onClick={check} disabled={loading} className="flex min-h-11 items-center gap-1 text-xs text-violet-600 hover:underline disabled:opacity-50 lg:min-h-0">
      ...
      {rows && !error && (
        rows.length ? (
          // На телефоне вложенная прокрутка в 128 px внутри прокручиваемой
          // страницы отбирала жест себе; там список показываем целиком.
          <div className="space-y-1 overflow-y-auto overscroll-contain sm:max-h-32">
            {rows.map((r) => (
