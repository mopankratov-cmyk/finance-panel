          ...
          {cabinetHealth.map((cabinet) => (
            <article key={cabinet.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_260px]">
                ...
                <aside className="border-t border-slate-100 p-4 lg:border-l lg:border-t-0"><h4 className="text-xs font-semibold text-slate-700">Токены</h4>{cabinet.tokens.length ? <div className="mt-2 space-y-2">{cabinet.tokens.map((token) => <div key={token.scope} className="flex items-center justify-between gap-2"><span className="text-[11px] text-slate-500">{token.label}</span><span className="flex items-center gap-1"><span title={token.error || undefined} className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${token.available === true && (token.daysLeft == null || token.daysLeft > 30) ? "bg-emerald-50 text-emerald-700" : token.available === null ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>{token.available === false ? "нет доступа" : token.daysLeft != null ? `${token.daysLeft} дн.` : token.available === true ? "доступен" : "не проверен"}</span>{token.error ? <Hint label={`Почему токен «${token.label}» недоступен`}><span className="break-anywhere">{token.error}</span></Hint> : null}</span></div>)}</div> : <p className="mt-2 text-[11px] text-slate-400">Ежедневная проверка ещё не запускалась.</p>}</aside>
              </div>
            </article>
          ))}
        </div>
                ...
                return (
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2.5">
                      ...
                      {/* Обрезанная ошибка целиком жила в `title`: на касании
                          её было не прочитать. Полный текст — в подсказке, как
                          и в таблице источников выше. */}
                      {r.error && (
                        <span className="ml-2 inline-flex items-center gap-1 text-xs text-red-400" title={r.error}>
                          {r.error.slice(0, 40)}
                          {r.error.length > 40 ? <Hint label="Полный текст ошибки"><span className="break-anywhere">{r.error}</span></Hint> : null}
                        </span>
                      )}
                    </td>
