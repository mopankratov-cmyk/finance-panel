...
type TaxResponse = { companies: Company[]; selectedCompany: Company | null; payments: TaxPayment[]; taxRegisterAvailable: boolean; taxSettingsAvailable: boolean; vatEffectiveFrom: string | null; wbReportedInputVat: number | null; wbAdvertisingExpense: number | null; wbAdvertisingCoverageStart: string | null; yearSettings: { fixedInsuranceContributions: number; insuranceReductionLimitPercent: number; priorYearLoss: number; recognizedCogs: number; outputVatConfirmed: number | null; hasEmployees: boolean; note: string }; marketplaceTaxDocuments: MarketplaceTaxDocument[]; error?: string };
...
type MarketplaceTotals = { income: number; serviceExpenses: number; advertising: number; serviceExpensesExAdvertising: number; cogs: number; warnings: string[] };
...
function totals(source: MarketplaceResponse): MarketplaceTotals {
  ...
  const advertising = Number(wb?.ad ?? 0);
  const serviceExpenses = Number(wb?.commission ?? 0) + Number(wb?.acquiring ?? 0) + advertising + Number(wb?.other ?? 0) + Number(wb?.logistics ?? 0) + Number(wb?.storage ?? 0) + Number(ozon?.commission ?? 0) + Number(ozon?.delivery ?? 0) + Number(ozon?.services ?? 0);
  return {
    income: money(Number(wb?.revenue_after_spp ?? wb?.revenue_before_spp ?? 0) + Number(ozon?.revenue ?? 0)),
    serviceExpenses: money(serviceExpenses),
    advertising: money(advertising),
    serviceExpensesExAdvertising: money(serviceExpenses - advertising),
...
export function TaxesPage() {
  ...
  // WB/Ozon УПД подтверждают НДС и расходы, уже посчитанные по детализации;
  // повторно прибавлять их полную сумму означало бы задвоить расход. Вручную
  // добавляются только прочие документы, которых нет в выгрузках площадок.
  const additionalMpExpenses = money(documents.reduce((sum, document) => document.marketplace === "other" && document.usnExpenseStatus === "included" ? sum + document.grossExpenseAmount : sum, 0));
  // У Филиппова и Оптимы УПД выдаётся на кабинет целиком. Рекламу поэтому
  // берём отдельной полной суммой из «Истории затрат», а не складываем
  // брендовые доли из ОПиУ. До применения миграции остаётся безопасный fallback.
  ...
  const calculation = calculateTaxPeriod({
    ...
    marketplaceExpensesGross: (register?.yearSettings.recognizedCogs ?? 0) + automaticMarketplaceExpenses + additionalMpExpenses, marketplaceInputVatConfirmed: confirmedMpVat,
    marketplaceInputVatInUsnExpenses: mpVatInUsn, insuranceContributions: contributions,
    insuranceReductionLimitPercent: register?.yearSettings.hasEmployees ? 50 : 100,
  ...
  const syncWbDocuments = async () => {
    setSyncingDocuments(true); setError("");
    try {
      const result = await fetch("/api/sync/wb-tax-documents", { cache: "no-store" }).then(json<{ ok?: boolean; totals?: { imported: number; review: number; errors: number }; error?: string }>);
      const totals = result.totals;
      if (totals) setError(totals.errors ? `WB: загружено ${totals.imported}, требуют проверки ${totals.review}, ошибок ${totals.errors}` : "");
      setReload((value) => value + 1);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Не удалось получить документы WB");
    } finally {
