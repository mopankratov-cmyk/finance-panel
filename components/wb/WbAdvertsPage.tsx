                ...
                return (
                  ...
                  <button type="button" onClick={() => setSelectedId(campaign.id)} className="flex h-full min-w-0 flex-1 items-center gap-2 px-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500">
                    ...
                    <div className="min-w-0 flex-1 pr-2">
                      ...
                      */}
                      {/*
                        Перенос строки, а не выезд за край. Все элементы здесь
                        `shrink-0`, и при длинном артикуле с вердиктом строка
                    ...
                    {/*
                      Колонка была уже своего содержимого, а выравнивание по
                      правому краю уводило лишнее ВЛЕВО — цифры ложились поверх
                      названия товара. Ширина под самую длинную строку
                      («вчера 4 091 ₽ · 7 дн. 8 736 ₽») плюс overflow-hidden как
        ...
        <section className="min-w-0 rounded-xl border border-dashed border-slate-200 bg-white">
          {!selected ? (
            ...
            <div className="p-3 sm:p-5">
              ...
              {/*
                Действия стоят под шапкой, а не в конце карточки.
                Раньше «Пауза», ставка и пополнение бюджета лежали ПОСЛЕ
                посуточной таблицы: чтобы поставить кампанию на паузу, человек
                пролистывал два экрана метрик и тридцать строк статистики.
                Решение принимают по цифрам сверху, а нажимают здесь же —
                кнопки должны быть там, где взгляд, а не там, где кончилась
                разметка.
              */}
              {singleCabinet && cabinetMoney ? (
                <AdActionsPanel
