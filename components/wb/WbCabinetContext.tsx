...
export function WbCabinetProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<WbCabinetContextValue>(
    ...
    [
      ...
      // этого админ кабинета не видел ни своих кнопок, ни пункта «Команда», а
      // селлер не мог ставить задачи в собственном кабинете — хотя сервер их
      // разрешал. Права меняются редко, лишних перерисовок это не даёт.
      rights,
      setCabinetId,
      user,
    ],
