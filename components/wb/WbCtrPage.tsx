    ...
    <div className="min-h-[calc(100dvh-54px)] bg-[#f6f7f9] pb-16 md:pb-5">
      ...
      <div className="space-y-3 px-2 py-3 sm:px-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3 text-[10px] leading-5 text-slate-500">
          <b className="text-slate-700">Кто меняет фото.</b> Панель сама переставляет главное фото карточки
          на витрине WB, как только вариант набирает норму показов, — с момента запуска теста и до конца.
          Запись в карточку необратима, поэтому старт теста подтверждается отдельно. У видео-тестов вместо
          просмотров считается отношение заказов к открытиям карточки: просмотры конкретного ролика WB не
          отдаёт, и подставлять вместо них похожее число было бы обманом.
        </div>
      </div>
    </div>
  );
