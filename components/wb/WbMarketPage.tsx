...
import { ChartNoAxesCombined, Loader2, RefreshCw, TrendingDown, TrendingUp } from "lucide-react";
...
export function WbMarketPage() {
  ...
  const [granularity, setGranularity] = useDashboardFilter<"day" | "week">("gran", "week", ["day", "week"]);
  const [dateFrom, setDateFrom] = useDashboardFilter<string>("from", "");
  const [dateTo, setDateTo] = useDashboardFilter<string>("to", "");
  const [preset, setPreset] = useDashboardFilter("period", "", ["", "7", "30", "90"]);
  ...
  const loadPulse = useCallback(async () => {
    ...
    try {
      ...
      const period = dateFrom && dateTo ? `&date_from=${dateFrom}&date_to=${dateTo}` : "&weeks=4";
  ...
  }, [cabinetId, dateFrom, dateTo, granularity, nicheSelection, subject]);
  ...
  const applyPreset = (days: number) => {
    ...
    setDateTo(iso(end));
    setGranularity(days > 30 ? "week" : "day");
    setPreset(String(days) as typeof preset);
  };
  ...
  return <div className="min-h-[calc(100vh-54px)] bg-[#f6f7f9] pb-16 md:pb-5">
    ...
    <div className="px-2 py-3 sm:px-6">
      <div className="mb-3 rounded-xl border border-slate-200 bg-white p-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)]"><div className="grid gap-2 xl:grid-cols-[minmax(260px,1fr)_auto_auto_auto] xl:items-center"><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Ниша<select value={nicheSelection} onChange={(event) => setNicheSelection(event.target.value)} disabled={nichesLoading} className="mt-1 min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs font-normal normal-case tracking-normal text-slate-700 outline-none focus:border-violet-400 sm:min-h-9"><option value="">{nichesLoading ? "Определяем ниши…" : "Выберите нишу"}</option>{niches.map((niche) => <option key={niche.id} value={`id:${niche.id}`}>{niche.name} · {niche.sku_count} SKU</option>)}<option value="__custom">Свой путь предмета</option></select></label>{nicheSelection === "__custom" ? <label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Путь предмета<input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Игрушки/…" className="mt-1 min-h-11 w-full rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none focus:border-violet-400 sm:min-h-9 xl:w-80" /></label> : null}<div className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-100 p-0.5 sm:min-h-9 sm:gap-0">{(["week", "day"] as const).map((value) => <button type="button" key={value} onClick={() => setGranularity(value)} className={`min-h-11 rounded-md px-3 text-[10px] font-semibold sm:min-h-8 ${granularity === value ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}>{value === "week" ? "Недели" : "Дни"}</button>)}</div><div className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-100 p-0.5 sm:min-h-9 sm:gap-0">{[7, 30, 90].map((days) => <button type="button" key={days} onClick={() => applyPreset(days)} className={`min-h-11 rounded-md px-2.5 text-[10px] font-semibold sm:min-h-8 ${preset === String(days) ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}>{days}д</button>)}</div></div><div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center"><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">С<input type="date" value={dateFrom} max={dateTo || undefined} onChange={(event) => { setDateFrom(event.target.value); setPreset(""); }} className="ml-2 min-h-11 rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none sm:min-h-9" /></label><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">По<input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => { setDateTo(event.target.value); setPreset(""); }} className="ml-2 min-h-11 rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none sm:min-h-9" /></label>{dateFrom || dateTo ? <button type="button" onClick={() => { setDateFrom(""); setDateTo(""); setPreset(""); }} className="min-h-11 rounded-lg px-2 text-[10px] font-semibold text-slate-400 hover:text-violet-700 sm:min-h-9">Сбросить к 4 неделям</button> : <span className="text-[10px] text-slate-400">Без дат — последние 4 недели</span>}</div></div>

      {loading || nichesLoading ? <div className="grid min-h-[320px] place-items-center rounded-xl border border-slate-200 bg-white text-slate-400"><div className="text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /><div className="mt-2 text-xs">Собираем рыночный срез</div></div></div> : error ? <WbErrorState message={error} onRetry={() => void loadPulse()} /> : data ? <>
