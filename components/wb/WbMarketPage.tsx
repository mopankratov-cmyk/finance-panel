...
export function WbMarketPage() {
  ...
  return <div className="min-h-[calc(100dvh-54px)] bg-[#f6f7f9] pb-16 md:pb-5">
    <WbModuleHeader icon={ChartNoAxesCombined} title="Рынок" description={`Ниша против наших продаж · ${activeCabinet?.name ?? "все кабинеты"}`} actions={<button type="button" onClick={() => void loadPulse()} disabled={loading || (!nicheSelection.startsWith("id:") && !subject.trim())} className="inline-flex min-h-11 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-[10px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-40 lg:min-h-8"><RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} /> Обновить</button>} />
    <div className="px-2 py-3 sm:px-6">
      <div className="mb-3 rounded-xl border border-slate-200 bg-white p-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)]"><div className="grid gap-2 xl:grid-cols-[minmax(260px,1fr)_auto_auto_auto] xl:items-center"><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Ниша<select value={nicheSelection} onChange={(event) => setNicheSelection(event.target.value)} disabled={nichesLoading} className="mt-1 min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs font-normal normal-case tracking-normal text-slate-700 outline-none focus:border-violet-400 lg:min-h-9"><option value="">{nichesLoading ? "Определяем ниши…" : "Выберите нишу"}</option>{niches.map((niche) => <option key={niche.id} value={`id:${niche.id}`}>{niche.name} · {niche.sku_count} SKU</option>)}<option value="__custom">Свой путь предмета</option></select></label>{nicheSelection === "__custom" ? <label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Путь предмета<input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Игрушки/…" className="mt-1 min-h-11 w-full rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none focus:border-violet-400 lg:min-h-9 xl:w-80" /></label> : null}<div className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-100 p-0.5 lg:min-h-9 sm:gap-0">{(["week", "day"] as const).map((value) => <button type="button" key={value} aria-pressed={granularity === value} onClick={() => setGranularity(value)} className={`min-h-11 rounded-md px-3 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 lg:min-h-8 ${granularity === value ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}>{value === "week" ? "Недели" : "Дни"}</button>)}</div><div className="flex min-h-11 items-center gap-2 rounded-lg bg-slate-100 p-0.5 lg:min-h-9 sm:gap-0">{[7, 30, 90].map((days) => <button type="button" key={days} aria-pressed={preset === String(days)} onClick={() => applyPreset(days)} className={`min-h-11 rounded-md px-2.5 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 lg:min-h-8 ${preset === String(days) ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}>{days}д</button>)}</div></div><div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center"><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">С<input type="date" value={dateFrom} max={dateTo || undefined} onChange={(event) => { setDateFrom(event.target.value); setPreset(""); }} className="ml-2 min-h-11 rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none lg:min-h-9" /></label><label className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">По<input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => { setDateTo(event.target.value); setPreset(""); }} className="ml-2 min-h-11 rounded-lg border border-slate-200 px-3 text-xs font-normal normal-case tracking-normal outline-none lg:min-h-9" /></label>{dateFrom || dateTo ? <button type="button" onClick={() => applyPreset(WB_MARKET_DEFAULT_DAYS)} className="min-h-11 rounded-lg px-2 text-[10px] font-semibold text-slate-400 hover:text-violet-700 lg:min-h-9">Сбросить к 30 дням</button> : <span className="text-[10px] text-slate-400">Без дат — последние 30 дней</span>}</div></div>
      ...
      {loading || nichesLoading ? <><LoadingBanner seconds={elapsed} hint="Собираем рыночный срез" /><SkeletonTableRows /></> : error ? <WbErrorState message={error} onRetry={() => void loadPulse()} /> : data ? <>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-3"><div className="mb-2 flex flex-wrap items-center gap-4 text-[10px] text-slate-500"><span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-sky-500" />Ниша, ₽</span><span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-3 bg-violet-600" />Мы, ₽</span><span className="ml-auto">{shortDate(data.date_from)}–{shortDate(data.date_to)}</span></div><div className="min-h-[280px] min-w-0 w-full"><ResponsiveContainer width="100%" height={280} minWidth={0}><LineChart data={data.series} margin={{ top: 8, right: 8, left: 4, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="bucket" tickFormatter={shortDate} fontSize={10} minTickGap={24} stroke="#94a3b8" /><YAxis yAxisId="market" tickFormatter={(value) => `${Math.round(Number(value) / 1_000_000)}м`} fontSize={10} width={35} stroke="#94a3b8" /><YAxis yAxisId="ours" orientation="right" tickFormatter={(value) => `${Math.round(Number(value) / 1_000)}к`} fontSize={10} width={35} stroke="#94a3b8" /><Tooltip formatter={(value) => `${fmt(Number(value))} ₽`} labelFormatter={(label) => shortDate(String(label))} /><Line yAxisId="market" type="monotone" dataKey="niche" name="Ниша" stroke="#0ea5e9" strokeWidth={2} dot={false} /><Line yAxisId="ours" type="monotone" dataKey="ours" name="Мы" stroke="#7c3aed" strokeWidth={2} dot={{ r: 2 }} /></LineChart></ResponsiveContainer></div>
          {/* Числа ряда доступны только через подсказку recharts, а на касании
              она перекрывает сам график. Тот же ряд таблицей — единственный
              способ прочитать значения пальцем. */}
          <details className="mt-2 border-t border-slate-100 pt-2 lg:hidden">
            <summary className="tap-row flex cursor-pointer list-none items-center text-[11px] font-semibold text-slate-600">Значения по точкам</summary>
            <div className="scroll-x mt-1">
              <table className="min-w-full text-[10px]">
                <thead className="text-slate-400"><tr className="h-7"><th className="px-2 text-left font-semibold">Точка</th><th className="px-2 text-right font-semibold">Ниша, ₽</th><th className="px-2 text-right font-semibold">Мы, ₽</th></tr></thead>
                <tbody>{data.series.map((point) => <tr key={point.bucket} className="h-8 border-t border-slate-100"><td className="px-2 text-slate-600">{shortDate(point.bucket)}</td><td className="px-2 text-right tabular-nums text-sky-700">{fmt(point.niche)}</td><td className="px-2 text-right tabular-nums text-violet-700">{point.ours == null ? "—" : fmt(point.ours)}</td></tr>)}</tbody>
              </table>
            </div>
          </details>
        </section>
