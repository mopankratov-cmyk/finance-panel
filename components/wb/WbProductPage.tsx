...
export function WbProductPage() {
  ...
  // первыми, остальные — в прежнем порядке заполненности.
  const { orderIndex } = useCabinetSkuOrder(cabinetId && cabinetId !== "all" ? cabinetId : null);
  const ordered = useMemo(
    () => sortByCustomSkuOrder(filtered, (row) => Number(row.nmId), orderIndex),
    [filtered, orderIndex],
  );

  useEffect(() => setRowWindow({ start: 0, end: Math.min(18, ordered.length) }), [ordered.length, query, category, onlyIncomplete, statusFilter]);
  ...
  const updateWindow = (element: HTMLDivElement) => {
    ...
    const end = Math.min(ordered.length, first + visible + 6);
          ...
          <div className="h-[calc(100vh-300px)] min-h-[420px] overflow-auto rounded-xl border border-slate-200 bg-white" onScroll={(event) => updateWindow(event.currentTarget)}>
            <table className="min-w-[1380px] w-full border-collapse text-[10px]">
              ...
              <tbody>
                ...
                {ordered.slice(rowWindow.start, rowWindow.end).map((row) => {
