...
function createDraft(cabinetId: string): EditableOrder {
  ...
  return {
    ...
    items: [],
    paymentStages: [{ title: "Оплата фабрике", percent: 100, amount: 0, exchangeRate: null, dueDate: null, paidAt: null, status: "planned" }],
    logisticsStages: [],
    expenses: [],
  };
}
      ...
      >
        {form ? <div className="space-y-3 bg-[#f7f8fa] p-0.5">
          ...
          <EditorSection icon={Factory} title={`Товары · ${form.items.length}`} action={<div className="flex gap-2"><select value={pickerNm} onChange={(event) => setPickerNm(event.target.value)} className="h-9 max-w-56 rounded-lg border border-slate-200 bg-white px-2 text-[10px] outline-none focus:border-violet-400"><option value="">Выберите SKU</option>{availableSkus.map((sku) => <option key={sku.nmId} value={sku.nmId}>{sku.article || sku.nmId} · нужно {sku.need45}</option>)}</select><button type="button" onClick={addItem} disabled={!pickerNm} className={smallButton}><Plus className="h-3.5 w-3.5" /> Добавить</button></div>}>
            {form.receiptBatchId ? <div className="mb-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] text-amber-800"><AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Заказ уже передан в приёмку — себестоимость на складе уже посчитана и зафиксирована. Правка цены или количества здесь изменит только запись заказа, склад её не подхватит задним числом. Прежнее значение остаётся во вкладке «История изменений» ниже.</span></div> : null}
          ...
          <div className="grid gap-3 xl:grid-cols-2">
            <EditorSection icon={WalletCards} title="Этапы оплаты" action={<button type="button" onClick={() => mutate((current) => ({ ...current, paymentStages: [...current.paymentStages, { title: "Новый этап", percent: 0, amount: 0, exchangeRate: null, dueDate: null, paidAt: null, status: "planned" }] }))} className={smallButton}><Plus className="h-3.5 w-3.5" /> Этап</button>}>
              <div className="space-y-2">{form.paymentStages.map((stage, index) => <div key={index} className="rounded-lg border border-slate-100 bg-slate-50/60 p-2"><div className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_76px_104px_38px]"><input aria-label="Название этапа оплаты" value={stage.title} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, title: event.target.value } : row) }))} className={inputClass} /><input aria-label="Процент оплаты" type="number" min={0} max={100} value={stage.percent} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, percent: Number(event.target.value) } : row) }))} className={`${inputClass} text-right`} /><input aria-label="Сумма оплаты, ₽" title="Сумма в рублях" placeholder="₽" type="number" min={0} value={stage.amount} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, amount: Number(event.target.value) } : row) }))} className={`${inputClass} text-right`} /><button type="button" onClick={() => mutate((current) => ({ ...current, paymentStages: current.paymentStages.filter((_, rowIndex) => rowIndex !== index) }))} className="flex min-h-11 w-full items-center justify-end rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-600 sm:min-h-0 sm:w-auto sm:justify-center" aria-label="Удалить этап оплаты"><span className="mr-1.5 text-[10px] font-medium sm:hidden">Удалить</span><Trash2 className="h-4 w-4" /></button></div><div className="mt-2 grid grid-cols-3 gap-2"><label className="space-y-1 text-[9px] text-slate-400">Срок оплаты<input aria-label="Срок оплаты" type="date" value={stage.dueDate ?? ""} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, dueDate: event.target.value || null } : row) }))} className={inputClass} /></label><label className="space-y-1 text-[9px] text-slate-400" title="Курс на дату именно этого платежа — не курс заказа. Не пересчитывает сумму, только сохраняется для истории (§13 ТЗ).">Курс на дату оплаты<input aria-label="Курс на дату оплаты" type="number" min={0} step={0.0001} placeholder="как в заказе" value={stage.exchangeRate ?? ""} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, exchangeRate: event.target.value === "" ? null : Number(event.target.value) } : row) }))} className={inputClass} /></label><label className="space-y-1 text-[9px] text-slate-400">Статус<select aria-label="Статус оплаты" value={stage.status} onChange={(event) => mutate((current) => ({ ...current, paymentStages: current.paymentStages.map((row, rowIndex) => rowIndex === index ? { ...row, status: event.target.value as typeof row.status, paidAt: event.target.value === "paid" ? new Date().toISOString() : null } : row) }))} className={inputClass}><option value="planned">Запланирован</option><option value="paid">Оплачен</option><option value="cancelled">Отменён</option></select></label></div></div>)}</div>
