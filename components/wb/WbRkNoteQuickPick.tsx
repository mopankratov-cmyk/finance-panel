...
import { RK_BUDGET_AMOUNTS, RK_NOTE_PRESETS, rkBudgetNote, type RkBudgetPlacement } from "@/lib/wb/rkNotes";
...
export function WbRkNoteQuickPick({
  ...
  /**
   * Второй экран списка — сборщик бюджета.
   *
   * Бюджет с часом запуска это 24,5% всех решений по рабочей таблице, но
   * сочетаний суммы, площадки и биддера набирается больше двадцати. Пунктами их
   * не выложить: список стал бы длиннее экрана, а ради одной задачи в день
   * пришлось бы читать двадцать строк. Поэтому сумма выбирается на втором
   * экране, а площадка и биддер — переключателями рядом.
   */
  const [budgetMode, setBudgetMode] = useState(false);
  const [placement, setPlacement] = useState<RkBudgetPlacement>("all");
    ...
    <>
      ...
      >
        <div className="flex items-center gap-1 px-2 pb-1 pt-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
          {budgetMode ? (
            <button type="button" onClick={() => setBudgetMode(false)} className="inline-flex items-center gap-0.5 rounded text-slate-500 hover:text-violet-700">
              <ChevronLeft className="h-3 w-3" aria-hidden="true" /> Назад
            </button>
          ) : null}
          {budgetMode ? "Бюджет и запуск" : "Задача на день"}
        </div>

        {budgetMode ? (
          <>
            {/* Площадка и биддер — приписки к любой сумме, поэтому стоят
                переключателями, а не отдельными пунктами: иначе каждая сумма
                размножилась бы на четыре строки. */}
            <div className="flex gap-1 px-1 pb-1">
              <button
                type="button"
                onClick={() => setPlacement((value) => value === "shelf_cpm" ? "all" : "shelf_cpm")}
                aria-pressed={placement === "shelf_cpm"}
                className={`flex-1 rounded-lg px-2 py-1.5 text-[11px] font-semibold sm:text-[10px] ${placement === "shelf_cpm" ? "bg-violet-600 text-white" : "bg-slate-50 text-slate-600 hover:bg-slate-100"}`}
              >
                Только полки CPM
              </button>
              <button
                type="button"
                onClick={() => setBidder((value) => !value)}
                aria-pressed={bidder}
                className={`rounded-lg px-2 py-1.5 text-[11px] font-semibold sm:text-[10px] ${bidder ? "bg-violet-600 text-white" : "bg-slate-50 text-slate-600 hover:bg-slate-100"}`}
              >
                Биддер
              </button>
            </div>
            {RK_BUDGET_AMOUNTS.map((amount) => {
              const text = rkBudgetNote({ amountRub: amount, placement, bidder });
              return (
                <button
                  key={amount}
                  type="button"
                  onClick={() => void save(text, false, text)}
                  disabled={saving !== null}
                  className={`${itemClass} justify-between ${text === note ? "bg-violet-50 font-semibold text-violet-700" : "text-slate-700 hover:bg-slate-50"}`}
                >
                  <span className="truncate">{text}</span>
                  {saving === text ? <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-slate-400" /> : null}
                </button>
              );
            })}
          </>
        ...
        {budgetMode ? null : RK_NOTE_PRESETS.map((preset) => {
        ...
        {budgetMode ? null : (
          <button
            ...
            className={`${itemClass} justify-between text-slate-700 hover:bg-slate-50`}
          >
