...
export function WbTrendsPage() {
  ...
  return <div className="min-h-[calc(100dvh-54px)] bg-[#f6f7f9] pb-16 md:pb-5">
    <WbModuleHeader icon={TrendingUp} title="Динамика" description={`${period} · ${activeCabinet?.name ?? "все кабинеты"}`} actions={<div className="flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 bg-white p-0.5 lg:min-h-8 sm:gap-0">{[7, 14, 30].map((days) => <button key={days} type="button" onClick={() => setWindowDays(days)} className={`min-h-11 rounded-md px-3 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 lg:min-h-7 ${windowDays === days ? "bg-violet-600 text-white" : "text-slate-500 hover:bg-slate-50"}`}>{days}д vs {days}д</button>)}</div>} />
