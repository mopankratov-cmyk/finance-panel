...
- 2026-06-27 10:15 MSK - tightened the live run signal and Remotion transport resilience: `graph-run`/Studio now scope execution tails and run summaries to the active `run_id`, stale `render-poll` rows no longer leak into the worker/operator screens, single-clip placeholder warnings no longer fire after autofill has replaced `Control clip` copy, `graph-run` retries one Remotion upload-transport failure before falling back, and the VM `render-service` now falls back from Supabase SDK upload to direct Storage REST upload on `fetch failed` transport errors. A live recipe `#68` rerun finished with a real `renders/...mp4` output URL and without the old `Remotion error: upload ... fallback raw clip` warning.
