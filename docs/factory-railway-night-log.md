...
- 2026-06-28: добавлен no-FAL контур operator feedback. Новый `/api/factory/feedback-queue` отдаёт приоритетную очередь уже сохранённых video-memory кандидатов для ручной разметки `winner/reject`, а `POST` помечает `content_assets.analysis.memory_label`, winner seed в `viral_hooks` и reject anti-signal в `cf_signals` best-effort. `noPaidReadinessSmoke` теперь показывает размер этой очереди перед платным батчем.
- 2026-06-28: Studio экран `Обучение` получил блок `Очередь памяти видосов`: открыть сохранённый MP4, пометить `win` или `trash` без запуска генерации. `/api/factory/feedback-queue` теперь полагается на общий `proxy.ts` для session/cron auth, чтобы работали и browser UI, и no-paid CLI smoke.
- 2026-06-28: добавлен авто-разметчик памяти без FAL: `POST /api/factory/feedback-queue/auto`. Dry-run показывает, что будет помечено, `apply:true` применяет только объективные решения: trash для неиграбельных/низкий OTK/status fail, winner только при market views >=2500 или frames-grounded OTK >=8. Слабый OTK 6 остаётся usable, но не становится winner, чтобы завод не учился на мусоре.
- 2026-06-28: batch получил quality-first source gate `require_strong_source`. Обычный режим не сломан, но no-paid smoke и строгие запуски теперь требуют `prepared/real` источники, а WB-only draft возвращает `next_action: prepare_product`. Это не улучшает промпты, а режет главный источник плохих MP4: сырые WB-инфографики вместо подготовленных кадров товара.
- 2026-06-28: no-paid smoke получил `skip_balance_check` только для `dry_run`, чтобы при пустом FAL всё равно видеть quality/source blockers. Реальный запуск с `dry_run:false` по-прежнему fail-closed по балансам.
