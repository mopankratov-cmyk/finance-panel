...
## 0. Ориентируйся по графу, а не по grep

В репозитории настроен [Graphify](https://github.com/safishamsi/graphify) — кодовый граф зависимостей. Перед тем как grep'ать или читать файлы вслепую:

- если `graphify-out/graph.json` уже есть — используй `graphify query "<вопрос>"`, `graphify explain "<символ>"`, `graphify path "<A>" "<B>"` (это на порядки дешевле по токенам, чем чтение сырых файлов);
- если графа ещё нет в этом чекауте — построй его один раз бесплатно (без API-ключа, только AST): `graphify extract . --no-cluster` (или `graphify update .` при уже существующем графе);
- после изменений кода обнови граф: `graphify update .` (тоже бесплатно, только AST).

`graphify-out/` не коммитить — уже в `.gitignore`, локальный кэш на чекаут.
