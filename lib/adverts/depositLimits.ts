 ...
 *
 * Он ловит не объём, а ОПИСКУ: лишний ноль в поле суммы. Поэтому он всегда
 * заметно ниже суточного — иначе одно нажатие уносит дневной лимит целиком.
 *
 * 10 000 → 25 000 по требованию владельца 05.09.2026. Число не круглое от
 * балды: раскладка пополнения по аппетиту кампаний потребовала 22 500 ₽ на
 * HT-83-35 (она съедает треть расхода кабинета), и прежний потолок заставлял
 * дробить это на три нажатия.
 *
 * Чего этот потолок больше НЕ ловит: приписанный ноль к суммам 1 500 и 2 500 —
 * 15 000 и 25 000 теперь проходят. Это осознанная плата за одно нажатие вместо
 * трёх; последним рубежом остаётся суточный лимит и журнал, где видно каждую
 * операцию, включая отклонённые.
 */
export function depositMaxPerOperation(): number {
  return envNumber("WB_ADVERT_DEPOSIT_MAX_ONCE", 25_000);
}
