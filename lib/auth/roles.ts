...
const ACCESS: Record<Role, string[]> = {
  ...
  //
  // /planning и /abc раньше были в списке, но оба — retired-маршруты
  // (lib/wb/retiredRoutes.ts), которые редиректят в /wb/rnp. У закупщика
  // /wb не открыт вовсе, так что ссылка вела в тупик: свой же пункт меню
  // отвечал отказом. Не строим отдельный не-/wb экран взамен — под этими
  // путями нет собственного контента уже сейчас, только редирект.
  buyer: ["/", "/supplies", "/warehouse", "/costs", "/unit"],
