...
/**
 * Файл, который панель может удалить.
 *
 * Граница проходит по ХРАНИЛИЩУ, а не по папке: удалить можно то, что лежит в
 * нашем публичном бакете, — потому что там файлом распоряжаемся мы. Кадр
 * карточки живёт в WB: строку каталога снести можно, но сам кадр останется на
 * витрине и вернётся при ближайшем обходе, то есть кнопка обещала бы то, чего
 * не делает. Записи `yandex-disk:` — вообще не адрес, удалять там нечего.
 *
 * Раньше правило было уже: две папки, `panel-uploads/` и `covers/`. Оно
 * оставляло вне досягаемости 1 164 съёмки, переложенные с Диска в `shoots/`, и
 * файлы контент-завода — при том, что физически всё это наши объекты в нашем
 * бакете, и владелец попросил открыть удаление по всему, чем панель реально
 * распоряжается.
 *
 * ⚠️ У `gen/` и `prepared/` есть сосед: контент-завод в своём репозитории
 * ссылается на эти файлы. Удаление их сломает — поэтому экран предупреждает об
 * этом в подтверждении, а не молча стирает.
 */
export function isPanelOwned(url: string | null | undefined): boolean {
  return String(url ?? "").includes(`/storage/v1/object/public/${PANEL_UPLOAD_BUCKET}/`);
}

/** Папки завода: удалить можно, но на них ссылается соседний репозиторий. */
export function isFactoryFolder(url: string | null | undefined): boolean {
  const value = String(url ?? "");
  return isPanelOwned(value) && /\/(gen|prepared)\//.test(value);
}
