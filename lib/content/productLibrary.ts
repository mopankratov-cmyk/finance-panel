...
/**
 * Что вообще имеет смысл рисовать в сетке.
 *
 * У записей со статусами «файл недоступен» и «нет ссылки» показывать нечего:
 * в галерее это пустые серые плитки, которые ничего не сообщают и не
 * нажимаются. Их место — счётчик, а не сетка.
 *
 * Правило живёт здесь, а не в компонентах, потому что мест показа стало два:
 * подборщик вариантов теста и каталог контента. Разъехавшись, они показывали
 * бы по одному товару разное число файлов — и оба выглядели бы сломанными.
 */
export function displayableItems(items: ContentItem[]): ContentItem[] {
  return items.filter((item) => item.usability !== "unresolved" && item.usability !== "missing");
}
