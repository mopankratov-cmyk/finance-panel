...
ok(/const sourceReadyArticles = await resolveSourceReadyArticles\(db, selectedRecipes\.map/.test(route), "batch route resolves source-ready articles before launch");
...
ok(/next_action: \{ type: "prepare_drafts", route: "\/api\/factory\/prepare-drafts", count, niche: requestedNiche \}/.test(route), "batch route suggests draft preparation when queue is empty");
ok(/next_action: preflight\.missing_drafts > 0 \|\| preflight\.missing_source_drafts > 0 \? \{ type: "prepare_drafts", route: "\/api\/factory\/prepare-drafts", count, niche: requestedNiche \} : null/.test(route), "batch route suggests draft preparation when guarded launch is missing source-ready drafts");
...
ok(/const preflight = \{[\s\S]*ready: plannedRecipeIds\.length >= count && !cappedByBudget && sourceReadyDrafts >= count && \(!requireLearningGate \|\| learningGate\.ready\)/.test(route), "batch route returns launch readiness before enqueue");
