...
ok(/const preflight = \{[\s\S]*ready: plannedRecipeIds\.length >= count && !cappedByBudget && sourceReadyDrafts >= count && \(!requireStrongSource \|\| strongSourceDrafts >= count\) && providerReady && \(!requireLearningGate \|\| learningGate\.ready\)/.test(route), "batch route returns launch readiness before enqueue");
