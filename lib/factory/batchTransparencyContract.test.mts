...
ok(/const sourceReadiness = await loadSourceReadiness\(db, selectedRecipes\.map/.test(route), "batch route resolves source-ready articles before launch");
...
ok(/next_action: providerReady \? \{ type: "prepare_drafts", route: "\/api\/factory\/prepare-drafts", count, niche: requestedNiche \} : null/.test(route), "batch route suggests draft preparation when queue is empty");
ok(/preflight\.missing_drafts > 0 \|\| preflight\.missing_source_drafts > 0 \? \{ type: "prepare_drafts", route: "\/api\/factory\/prepare-drafts", count, niche: requestedNiche \} : sourcePrepNextAction/.test(route), "batch route suggests draft preparation when guarded launch is missing source-ready drafts");
...
ok(/const preflight = \{[\s\S]*ready: plannedRecipeIds\.length >= count && !cappedByBudget && sourceReadyDrafts >= count && \(!requireStrongSource \|\| strongSourceDrafts >= count\) && providerReady && \(!requireLearningGate \|\| learningGate\.ready\)/.test(route) || /const requireStrongSource = true;[\s\S]*ready: plannedRecipeIds\.length >= count && !cappedByBudget && sourceReadyDrafts >= count && \(!requireStrongSource \|\| strongSourceDrafts >= count\)/.test(route), "batch route returns launch readiness before enqueue");
