...
import { buildKatyaAnchorExpansionSafer } from "./bloggerAnchorExpansionSafer.ts";

const BASE = "https://api.heygen.com";
const DEFAULT_OUT_DIR = "/tmp/ugc-factory-katya-anchor-expansion-safer";
...
async function renderRun(run, apiKey, outDir) {
  ...
  return { ok: true, run_id: run.run_id, local_path: localPath, hypothesis: run.hypothesis };
}

async function main() {
  const generation = intArg("--generation", 15, 1, 99);
  const outRoot = argValue("--out-dir", DEFAULT_OUT_DIR);
  const outDir = join(outRoot, `generation-${String(generation).padStart(2, "0")}`);
  mkdirSync(outDir, { recursive: true });
  const plan = buildKatyaAnchorExpansionSafer(generation);
