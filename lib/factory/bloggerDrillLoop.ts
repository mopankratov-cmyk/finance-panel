...
// ПАЛИТРА-КАНОН (Библия Мани, батчи 5-8): верифицированные рецепты регистров.
...
export const DRILL_REGISTER_CANON: Record<string, { pose: string; voice: { style: number; speed: number }; motion: string; expr: "low" | "medium"; register_fit: number; source: string }> = {
  ...
  serious_expert: { pose: "sitting upright facing the camera, one forearm resting on the table, stern focused expression, slightly furrowed brow, lips firmly closed, absolutely no smile", voice: { style: 0.35, speed: 1.0 }, motion: "calm confident delivery, steady direct gaze, minimal head movement, occasional slow nod, no smiles", expr: "low", register_fit: 9, source: "b08_01 (medium-вариант b08_04 — альтернатива с напором)" },
  warm_friend: { pose: "holding a warm mug with both hands below her chin, warm gentle expression, soft kind eyes, relaxed closed-lip smile", voice: { style: 0.4, speed: 1.05 }, motion: CANON_MOTION, expr: "medium", register_fit: 8, source: "b07_02" },
  ...
  tired_evening: { pose: "holding a warm mug with both hands below her chin, visibly exhausted face, droopy half-closed heavy eyelids, faint dark circles under her eyes, slack relaxed features, no smile", voice: { style: 0.5, speed: 0.9 }, motion: QUIET_MOTION, expr: "low", register_fit: 7, source: "b08_05" },
  surprised: { pose: "holding a warm mug with both hands below her chin, genuinely surprised expression, raised eyebrows, mouth slightly open", voice: { style: 0.5, speed: 1.1 }, motion: "briefly looks off-camera mid-sentence as if someone distracted her, half-second pause, then returns to the camera and continues naturally, imperfect timing feel", expr: "medium", register_fit: 7, source: "b07_08 (мягкий вход — усиление в b9)" },
  // conspiratorial: НЕ канонизирована — b07_04 регистр 7/композиция b-roll, b08_08 композиция 9/регистр 4; финальный фикс в b9
};
