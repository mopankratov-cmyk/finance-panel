...
async function main() {
  ...
  // 1) один TTS на батч (скрипт общий) — кэш в леджере
  const ttsKey = `b${batch}_tts`;
  if (!ledger[ttsKey]) {
    const tts = await falQueue("fal-ai/elevenlabs/tts/eleven-v3", {
      text: plan.clips[0].script, voice: NASTYA,
      stability: 0.4, similarity_boost: 0.8,
      style: plan.clips[0].voice_emotion_speed.style, speed: plan.clips[0].voice_emotion_speed.speed,
      language_code: "ru",
    });
    const buf = Buffer.from(await (await fetchR(tts.audio.url)).arrayBuffer());
    writeFileSync(`${OUT}/b${batch}/voice.mp3`, buf);
    ledger[ttsKey] = await tgHostAudio(buf);
    saveLedger(ledger);
  }
  const audioUrl = ledger[ttsKey];
  ...
  for (const clip of plan.clips) {
    ...
    try {
      ...
      const dgKey = `${clip.look_name}::${clip.degloss_hint}`;
      if (!ledger.degloss_cache[dgKey]) {
        ...
        const dg = await falQueue("fal-ai/bytedance/seedream/v4.5/edit", {
          prompt: drillDeglossPrompt(clip.degloss_hint),
        ...
        ledger.degloss_cache[dgKey] = url;
        saveLedger(ledger);
      }
      ...
      if (!rec.video_done) {
        ...
        writeFileSync(`${OUT}/b${batch}/${clip.clip_id}-raw.mp4`, Buffer.from(await (await fetchR(videoUrl)).arrayBuffer()));
        rec.video_done = true;
        saveLedger(ledger);
      }
      ...
      if (ffbin && !rec.post_done) {
        ...
        ffmpeg(ffbin, ["-y", "-i", final, "-vf", "fps=1/1.2,scale=400:-1,tile=5x1", "-frames:v", "1", `${OUT}/b${batch}/${clip.clip_id}-strip.jpg`]);
        rec.post_done = true;
