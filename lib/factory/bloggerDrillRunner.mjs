...
import { buildDrillBatch, drillDeglossPrompt, buildIdentityLockDeglossPrompt } from "./bloggerDrillLoop.ts";
...
async function main() {
  ...
  // identity-lock: одно эталонное лицо на все сцены (вердикт «пять разных девушек»).
  const idAnchor = argValue("--identity-anchor");
  const anchorImg = idAnchor ? avatars.find((a) => a.name === idAnchor)?.image_url : null;
  if (idAnchor && !anchorImg) throw new Error(`identity-anchor ${idAnchor} not found in group`);
  ...
  for (const clip of plan.clips) {
    ...
    try {
      // 2) деглянц с кэшем по look+hint (+ эталон, если id-lock)
      const look = avatars.find((a) => a.name === clip.look_name);
      if (!look?.image_url) throw new Error(`look ${clip.look_name} not found`);
      const useIdLock = anchorImg && idAnchor !== clip.look_name;
      const imageUrls = useIdLock ? [anchorImg, look.image_url] : [look.image_url];
      const deglossPrompt = useIdLock
        ? buildIdentityLockDeglossPrompt(clip.degloss_hint, clip.pose_hint)
        : drillDeglossPrompt(clip.degloss_hint, clip.pose_hint);
      const dgKey = `${useIdLock ? `idlock:${idAnchor}→` : ""}${clip.look_name}::${clip.degloss_hint}::${clip.pose_hint || ""}`;
      if (!ledger.degloss_cache[dgKey]) {
        const dg = await falQueue("fal-ai/bytedance/seedream/v4.5/edit", {
          prompt: deglossPrompt,
          image_urls: imageUrls, num_images: 1,
        ...
        if (ffbin) {
          ...
          for (let attempt = 0; attempt < 3; attempt++) {
            ...
            const rg = await falQueue("fal-ai/bytedance/seedream/v4.5/edit", {
              prompt: deglossPrompt + " The photograph fills the entire canvas edge to edge: no black bars, no device bezel or notch, no rounded screen corners, no phone mockup frame.",
              image_urls: imageUrls, num_images: 1,
              image_size: { width: 1152, height: 2048 },
            });
      ...
      if (ffbin && !rec.post_done) {
        ...
        // реализм-пас 2026-07-03 (см. antiAiPost.ts): бочка-линза + дрейф экспозиции по t +
        // хромо-смаз/резкость-по-яркости + хромо-доминантный сенсорный шум + блокинг на движении.
        // Дрейф ББ НЕ вводим (warmthK 6400 только что откалиброван). Инлайн-копия канона
        // buildAntiAiVideoFilter — держать синхронно.
        const drift = "eq=contrast='" + (yavg < 105 ? 1.05 : 1.06) + "+0.006*sin(2*PI*t/5)':saturation=" + (yavg < 105 ? 1.06 : 1.05) + ":gamma=" + (yavg < 105 ? 1.06 : 1.0) + ":brightness='" + (yavg < 105 ? 0.04 : 0.01) + "+0.012*sin(2*PI*t/3.3)'";
        ...
        ffmpeg(ffbin, ["-y", "-i", raw, "-f", "lavfi", "-i", "anoisesrc=colour=brown:amplitude=0.0035:r=44100",
          "-filter_complex",
          `[0:v]fps=30,scale=iw*1.04:ih*1.04:flags=lanczos,lenscorrection=cx=0.5:cy=0.5:k1=-0.05:k2=-0.012:i=bilinear,crop=iw/1.04:ih/1.04:x='(in_w-out_w)/2+4*sin(t*1.3)+1.5*sin(t*7.9)':y='(in_h-out_h)/2+2*sin(t*1.7)+1*sin(t*9.3)',${drift},colortemperature=temperature=6400,${curves},hqdn3d=1.5:6:3:5,unsharp=5:5:0.4:5:5:0,rgbashift=rh=-1:bh=1,noise=c0s=5:c0f=t:c1s=12:c1f=t:c2s=12:c2f=t,vignette=PI/8[v];[0:a]highpass=f=120,lowpass=f=9000,acompressor=threshold=-18dB:ratio=3:attack=20:release=250:makeup=2,aecho=0.8:0.88:30|45:0.18|0.12[voice];[voice][1:a]amix=inputs=2:duration=first:normalize=0[a]`,
          "-map", "[v]", "-map", "[a]", "-c:v", "libx264", "-crf", "17", "-tune", "grain", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "128k", graded]);
        ffmpeg(ffbin, ["-y", "-i", graded, "-vf", "noise=c0s=3:c0f=t:c1s=7:c1f=t:c2s=7:c2f=t", "-c:v", "libx264", "-profile:v", "main", "-b:v", "3800k", "-maxrate", "4400k", "-bufsize", "7600k", "-preset", "veryfast", "-g", "250", "-bf", "3", "-x264-params", "scenecut=0:rc-lookahead=10", "-c:a", "aac", "-b:a", "96k", final]);
