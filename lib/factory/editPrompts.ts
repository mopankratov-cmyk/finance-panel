...
// (Nano Banana / Seedream через fal) лаконичный СТРУКТУРНЫЙ промпт по формуле Lock→Change→Amount→Constraints
...
// Детерминированный edit-промпт по формуле Lock→Change→Amount→Constraints (verb-first, явный preserve-клозет).
export function buildEditPrompt(ctx: EditPromptCtx): string {
  ...
  if (ctx.op === "clean") {
    return (
      ...
      `Amount: full background replacement, the product itself untouched. ` +
