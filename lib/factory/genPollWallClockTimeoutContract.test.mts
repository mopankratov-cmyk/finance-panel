...
ok(/const MAX_GEN_POLL_LOGS = 12;/.test(source), "gen-poll has an execution-log timeout backstop");
...
ok(/function stepLogCount\(plan: RunPlan, step: string\): number/.test(source), "graph-run can count repeated step logs");
...
ok(/const timedOutByWallClock = stepAgeMs\(plan, "gen-poll"\) >= MAX_GEN_POLL_MS \|\| stepLogCount\(plan, "gen-poll"\) >= MAX_GEN_POLL_LOGS;/.test(source), "gen-poll checks wall-clock and repeated-log timeout");
