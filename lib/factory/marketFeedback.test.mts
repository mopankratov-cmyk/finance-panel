...
ok(/let statusMarked = false;/.test(postMetrics) && /update\(\{ status: "posted", updated_at: new Date\(\)\.toISOString\(\) \}\)/.test(postMetrics) && /status_marked: statusMarked/.test(postMetrics), "post-metrics marks recipes as posted after market signal is accepted");
...
ok(/d\.status_marked\?"✓ опубликован · метрики записаны"/.test(studio) && /r\.status="posted"; _libCache=null;/.test(studio), "Studio reflects posted status after post-metrics accepts a market signal");
