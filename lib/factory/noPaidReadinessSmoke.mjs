...
function summarize(checks) {
  ...
  else if (batch.preflight?.ready === false) {
    if (batch.learning_gate?.ready === false && batch.learning_gate?.reason) blockers.push(`learning gate hold: ${batch.learning_gate.reason}`);
    else blockers.push("batch preflight not ready");
  }
...
function actionableNextActions(diagnostics, batch) {
  const sourceTiers = batch?.preflight?.source_tiers || diagnostics?.source_tiers || {};
  const wbOnly = Number(batch?.preflight?.wb_only_drafts || sourceTiers.wb || 0);
  const actions = Array.isArray(diagnostics?.next_actions) ? diagnostics.next_actions : [];
  return actions.filter((item) => {
    const text = String(item || "");
    if (/prepare_product|WB-only/i.test(text)) return wbOnly > 0 || batch?.next_action?.type === "prepare_product";
    return true;
  });
}
...
function buildMarkdown(payload) {
  ...
  const fal = Array.isArray(balances) ? balances.find((row) => row?.service === "fal") : null;
  const nextActions = actionableNextActions(d, b);
  const lines = [
    "# Factory No-Paid Readiness Smoke",
    "",
    ...
    b.next_action ? `- next_action: ${JSON.stringify(b.next_action)}` : "- next_action:",
    "",
    "## Next Actions",
    "",
    ...(nextActions.length ? nextActions.map((item) => `- ${item}`) : ["- no source-prep action left; start the next cycle with a fresh series_after once paid balance is ready"]),
    "",
  ];
  return lines.join("\n");
}
