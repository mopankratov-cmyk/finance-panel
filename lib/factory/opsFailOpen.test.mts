...
ok(/function enrichFallbackWorkerFromObserver[\s\S]*worker\.source !== "queue_fallback" \|\| worker\.last_seen[\s\S]*const staleMin = Number[\s\S]*staleMin >= 180\) return workerState[\s\S]*heartbeat\?\.last_activity_at[\s\S]*liveness\(lastSeen\)/.test(ops), "ops enriches queue fallback worker with observer pulse only while that pulse is fresh");
