...
export interface ProductTwinPreparationPlan {
  ...
  canonicalViews: {
    ...
    truth: "truthful_source" | "derived_from_source" | "synthetic_candidate";
    required: boolean;
    prompt: string;
  }[];
  serviceAssets: ProductTwinPreparationPlan["canonicalViews"];
  notes: string[];
}
...
export function normalizeTwinCategory(value: unknown, article = "", product = ""): ProductTwinCategory {
  ...
  if (/\bNV[-\w]*|jacket|куртк|ветровк|apparel/i.test(hay)) return "apparel";
  if (/\bCLR[-\w]*|bag|сумк/i.test(hay)) return "bag";
  return "other";
}
