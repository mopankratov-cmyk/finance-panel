...
async function resolveInputImage(body: ProductTwinBuildInput): Promise<{ image: string; sourceKind: string; sourcePath?: string; sourceUrl?: string; sourcePicked?: boolean } | { error: string }> {
  ...
  return { image: imageBufferToDataUrl(buf, contentType), sourceKind: sourcePicked ? "picked_disk_path" : "disk_path", sourcePath: diskPath, sourcePicked };
}
...
export async function buildProductTwin(input: ProductTwinBuildInput, db: SupabaseClient): Promise<{
  ...
  const twinId = cleanText(input.twin_id || input.twinId, 120) || buildTwinId({ article, source: resolved.sourcePath || resolved.sourceUrl || resolved.sourceKind });
