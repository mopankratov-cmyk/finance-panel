...
import { nicheFromArticle, type RubricNiche } from "./rubric";
...
function canonicalNiche(value: unknown): RubricNiche {
  const raw = text(value, 80).toLowerCase();
  if (raw === "clothing" || raw === "toys" || raw === "cosmetics" || raw === "default") return raw;
  return nicheFromArticle("", raw);
}
