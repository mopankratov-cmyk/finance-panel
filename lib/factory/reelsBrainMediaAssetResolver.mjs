...
function isProbablyImage(value) {
...
function envelopeAssetCandidate(value) {
  const assets = Array.isArray(value?.assets) ? value.assets : [];
  for (const asset of assets) {
    const url = String(asset?.url || "").trim();
    const field = String(asset?.field || "").toLowerCase();
...
function directAssetCandidate(video) {
  const fields = [video?.video_url, video?.download_url, video?.media_url, video?.audio_url, video?.url]
    ...
    .filter(Boolean);
  const directField = fields.slice(0, 4).find((candidate) => /^https?:\/\//i.test(candidate) && !isSocialPageUrl(candidate) && !isProbablyImage(candidate));
