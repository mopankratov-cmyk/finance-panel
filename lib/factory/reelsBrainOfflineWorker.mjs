...
const FAL_WHISPER_TIMEOUT_MS = timeoutMsEnv("REELS_BRAIN_FAL_WHISPER_TIMEOUT_MS", 120_000);
...
async function ytDlpCommand(binary, extraArgs = []) {
...
async function resolveMediaLocatorViaYtDlp(url) {
  ...
  try {
    const command = await ytDlpCommand(binary, ["-J", "--no-warnings", "--skip-download", target]);
...
async function downloadVideoViaYtDlp(sourceUrl) {
  ...
  try {
    const command = await ytDlpCommand(binary, [
      "--no-warnings",
      "--no-playlist",
      "-f", "mp4/best",
      "-o", outputTemplate,
...
async function transcribeFal(audioUrl, language = "ru", options = {}) {
  ...
  try {
    const response = await fetch("https://fal.run/fal-ai/whisper", {
      ...
      signal: AbortSignal.timeout(FAL_WHISPER_TIMEOUT_MS),
    });
