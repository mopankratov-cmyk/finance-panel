...
async function transcribeFal(audioUrl, language = "ru") {
  ...
  async function uploadToFalStorage(sourceUrl) {
    ...
    const init = await fetch("https://rest.alpha.fal.ai/storage/upload/initiate", {
      ...
      body: JSON.stringify({
        ...
        file_name: contentType.startsWith("audio/") ? "reels-brain-audio" : "reels-brain-video.mp4",
      }),
  ...
  if (!/fal\.(ai|media)|fal\.run/i.test(audioUrl)) {
    try {
      ...
      return { text: null, error: `fal_rehost_${String(error?.message || error).slice(0, 120)}` };
    }
  }
