...
Options:
  ...
  --priority <smart|fifo>      Queue ordering policy (default: smart)
  --deep-only <0|1>            Only process higher-priority expensive candidates
  --shard-index <n>            Shard index for parallel workers (default: 0)
  --shard-count <n>            Total shard count for parallel workers (default: 1)
...
function isDirectVideoLocator(value) {
...
function hasResolvedMediaLocatorsLocal(row) {
...
function hasTerminalMediaFailureLocal(row) {
  const analyzedFull = rec(row?.analyzed_full);
  const reelsSeed = rec(analyzedFull.reels_seed);
...
function scoreMediaCandidateLocal(input) {
  const virality = parseNumber(input?.virality_score) || 0;
  const views = parseNumber(input?.views) || 0;
  const createdAt = toText(input?.created_at);
  const createdTs = createdAt ? Date.parse(createdAt) : NaN;
  const hours = Number.isFinite(createdTs) ? Math.max(0, (Date.now() - createdTs) / (1000 * 60 * 60)) : null;
  ...
  if (/instagram\.com\/(reel|reels|tv|p)\//i.test(String(input?.url || ""))) score += 14;
  return Math.round(score * 10) / 10;
}

function widenedScanWindowLocal(scan, shardCount, hardCap = 800) {
  const multiplier = Math.max(4, shardCount * 4);
  return Math.max(scan, Math.min(hardCap, scan * multiplier));
}

function stableShardMatchLocal(id, shardIndex, shardCount) {
  if (!Number.isFinite(id) || shardCount <= 1) return true;
  return Math.abs(Number(id)) % shardCount === shardIndex;
}

function apifyActorPathLocal(actor) {
  return String(actor || "")
    .trim()
...
function normalizeInstagramPermalink(value) {
  ...
  try {
    ...
    if (match?.[1] && match?.[2]) return `https://www.instagram.com/${match[1].toLowerCase()}/${match[2]}/`;
...
function tiktokVideoIdFromUrl(value) {
  const target = toText(value);
  if (!target) return "";
  const byPath = target.match(/\/video\/(\d+)/i);
  if (byPath?.[1]) return byPath[1];
  const byEmbed = target.match(/\/v(?:ideo)?\/(\d+)\.html/i);
  if (byEmbed?.[1]) return byEmbed[1];
  try {
    const url = new URL(target);
    const shared = url.searchParams.get("share_item_id");
    if (shared && /^\d+$/.test(shared)) return shared;
  } catch {}
  return "";
}

function normalizeTiktokVideoUrl(value) {
  const id = tiktokVideoIdFromUrl(value);
  return id || toText(value);
}

async function fetchInstagramMediaMapViaApify(urls) {
  const token = toText(process.env.APIFY_TOKEN || "");
  if (!token || !Array.isArray(urls) || !urls.length) return new Map();
  const directUrls = Array.from(new Set(urls.map(normalizeInstagramPermalink).filter(Boolean))).slice(0, 10);
  if (!directUrls.length) return new Map();

  const actorCandidates = Array.from(new Set([
    process.env.APIFY_INSTAGRAM_REELS_ACTOR || "apify/instagram-scraper",
    "apify/instagram-scraper",
    "apify/instagram-hashtag-scraper",
  ].map(apifyActorPathLocal).filter(Boolean)));

  for (const actor of actorCandidates) {
    try {
      const requestUrl = new URL(`https://api.apify.com/v2/acts/${actor}/run-sync-get-dataset-items`);
      requestUrl.searchParams.set("token", token);
      requestUrl.searchParams.set("maxItems", String(directUrls.length));
      requestUrl.searchParams.set("maxTotalChargeUsd", "0.05");
      ...
      if (!response.ok) {
        throw new Error(`Apify ${actor} ${response.status}`);
      }
      ...
      for (const item of items) {
        const row = rec(item);
        const directUrl = normalizeInstagramPermalink(
          row.url
          || row.postPage
          || row.shortUrl
          || (row.shortCode ? `https://www.instagram.com/reel/${row.shortCode}/` : ""),
        );
        const mediaUrl = toText(
          row.videoUrl
          || row.video_url
          || rec(Array.isArray(row.childPosts) ? row.childPosts.find((item) => toText(rec(item).videoUrl || rec(item).video_url)) : null).videoUrl
          || rec(Array.isArray(row.childPosts) ? row.childPosts.find((item) => toText(rec(item).videoUrl || rec(item).video_url)) : null).video_url
          || row.downloadUrl
          || row.webVideoUrl
          || rec(Array.isArray(row.video_versions) ? row.video_versions[0] : null).url,
        );
        if (!directUrl || !mediaUrl || isImageLikeUrl(mediaUrl)) continue;
        mediaMap.set(directUrl, {
          media_url: mediaUrl,
          title: text(row.caption || row.title || row.text || "", 500),
          author: text(row.ownerUsername || row.username || rec(row.authorMeta).username || rec(row.authorMeta).name || "", 180),
          duration_sec: parseNumber(row.videoDuration || row.video_duration || row.duration || row.videoLength || row.length),
        });
      }
      if (mediaMap.size) {
        console.info(JSON.stringify({
          ok: true,
          stage: "instagram_apify_media_fallback",
          actor,
          requested: directUrls.length,
          resolved: mediaMap.size,
        }));
        return mediaMap;
      }
    } catch (error) {
      console.info(JSON.stringify({
        ok: false,
        stage: "instagram_apify_media_fallback_failed",
        actor,
...
async function fetchTiktokMediaMapViaApify(urls) {
  const token = toText(process.env.APIFY_TOKEN || "");
  if (!token || !Array.isArray(urls) || !urls.length) return new Map();
  const directUrls = Array.from(new Set(urls.map(toText).filter((url) => /tiktok\.com\//i.test(url)))).slice(0, 10);
  if (!directUrls.length) return new Map();

  const actorCandidates = Array.from(new Set([
    process.env.APIFY_TIKTOK_ACTOR || process.env.APIFY_ACTOR || "clockworks/tiktok-scraper",
    ...
    try {
      ...
      const response = await fetch(requestUrl, {
        ...
        body: JSON.stringify({
          ...
          proxy: { useApifyProxy: true },
          maxItems: directUrls.length,
          resultsLimit: directUrls.length,
          resultsPerPage: directUrls.length,
        }),
        signal: AbortSignal.timeout(60_000),
      });
      if (!response.ok) {
      ...
      for (const item of items) {
        const row = rec(item);
        const key = normalizeTiktokVideoUrl(
          row.submittedVideoUrl
          || row.webVideoUrl
          || row.url
          || row.videoUrl
          || row.video_url,
        ...
        if (!key || !mediaUrl || isImageLikeUrl(mediaUrl)) continue;
        mediaMap.set(key, {
          media_url: mediaUrl,
          title: text(row.text || row.caption || row.title || "", 500),
          author: text(rec(row.authorMeta).name || rec(row.authorMeta).uniqueId || rec(row.authorMeta).username || "", 180),
          duration_sec: parseNumber(rec(row.videoMeta).duration || row.videoDuration || row.video_duration || row.duration),
        });
      }
      if (mediaMap.size) {
        console.info(JSON.stringify({
          ok: true,
          stage: "tiktok_apify_media_fallback",
          actor,
          requested: directUrls.length,
          resolved: mediaMap.size,
        }));
      ...
      console.info(JSON.stringify({
        ok: false,
        stage: "tiktok_apify_media_fallback_failed",
        actor,
        error: String(error?.message || error).slice(0, 240),
      }));
    }
...
function shouldApplyInstagramApifyMediaFallback(platform, provider, runs) {
  if (platform !== "instagram" || provider !== "apify_instagram" || !Array.isArray(runs) || !runs.length) return false;
  return runs.every((run) => {
    const matchedWithMedia = Number(run?.matched_with_media || 0) || 0;
    return matchedWithMedia <= 0;
  });
}

async function fetchLocalMediaBackfillCandidates(config, platform, limit, scan) {
  const db = getSupabaseAdminClient();
  if (!db) return [];
  const niches = splitCsv(config.niches);
  if (!niches.length) return [];
  const queryScan = widenedScanWindowLocal(scan, config.shardCount, 800);
  ...
  if (error) {
    console.info(JSON.stringify({
      ok: false,
      stage: "local_media_candidate_fetch_failed",
      platform,
      error: String(error.message || error).slice(0, 240),
    }));
    return [];
  ...
  return (Array.isArray(data) ? data : [])
    .filter((row) => stableShardMatchLocal(row.id, config.shardIndex, config.shardCount))
    .filter((row) => toText(row.url) && !hasResolvedMediaLocatorsLocal(row) && !hasTerminalMediaFailureLocal(row))
    .map((row) => ({ row, priority: scoreMediaCandidateLocal(row) }))
    .sort((a, b) =>
    ...
    .map(({ row }) => ({
      ...
      niche: row.niche,
      url: row.url,
      provider: config.mediaProviders[platform] || config.provider,
      effective_provider: config.mediaProviders[platform] || config.provider,
      found: 1,
      matched: 1,
      matched_with_media: 0,
      used_local_resolver: false,
      resolver_sample: null,
      sample_found: {
        url: row.url,
        video_url: null,
        media_url: null,
...
async function hasYtDlpBinary() {
  ...
  const candidates = [
    ...
    "yt-dlp",
    ...NIX_BIN_DIRS.map((dir) => path.join(dir, "yt-dlp")),
...
function shouldUseYtDlpCookies(target) {
  ...
  if (!value) return false;
  if (/(^https?:\/\/)?([a-z0-9-]+\.)?(youtube\.com|youtu\.be)(\/|$)/i.test(value)) {
    return boolFlag(process.env.YT_DLP_ENABLE_YOUTUBE_COOKIES ?? "0", false);
  }
  if (/(^https?:\/\/)?([a-z0-9-]+\.)?(instagram\.com|instagr\.am)(\/|$)/i.test(value)) {
    return boolFlag(process.env.YT_DLP_ENABLE_INSTAGRAM_COOKIES ?? "0", false);
  }
  return false;
}

function hasConfiguredYtDlpCookies() {
  if (toText(process.env.YT_DLP_COOKIES_PATH)) return true;
  if (toText(process.env.YT_DLP_COOKIES_TXT)) return true;
  if (toText(process.env.YT_DLP_COOKIES_B64) || toText(process.env.YT_DLP_COOKIES_BASE64)) return true;
  if (toText(process.env.YT_DLP_COOKIES_GZ_B64)) return true;
...
function localResolverAllowedForPlatform(platform) {
  const value = toText(platform)?.toLowerCase() || "";
  if (value === "youtube") {
    return boolFlag(process.env.REELS_BRAIN_ENABLE_YOUTUBE_LOCAL_RESOLVER ?? "0", false)
      || hasConfiguredYtDlpCookies();
  }
  return true;
}

function shouldSkipDownloadForYtDlpProbe(target) {
  return !shouldUseYtDlpCookies(target);
}

function ytDlpProbeArgSets(target) {
  const base = ["-J", "--no-warnings"];
  if (shouldSkipDownloadForYtDlpProbe(target)) base.push("--skip-download");
...
function normalizeYtDlpTarget(url) {
  const target = toText(url);
  if (!target) return "";
  try {
    const parsed = new URL(target);
    if (/tiktok\.com$/i.test(parsed.hostname) && /\/v\/[^/]+\.html$/i.test(parsed.pathname)) {
      const shareItemId = toText(parsed.searchParams.get("share_item_id"));
...
async function resolveMediaLocatorViaYtDlp(url) {
  ...
  const probeArgSets = ytDlpProbeArgSets(target);
  let lastError = null;
  for (const probeArgs of probeArgSets) {
    try {
      const command = await ytDlpCommand(binary, target, probeArgs);
  ...
  console.info(JSON.stringify({
    ...
    error: lastError?.error || "yt_dlp_probe_failed",
    stderr: lastError?.stderr || null,
    args: lastError?.args || null,
  }));
  return null;
}
...
async function extractAudioFeaturesLocally(mediaUrl, options = {}) {
  ...
  if (!target) {
    return {
      ...
      audio_features: null,
      transcript: null,
      error: "media_locator_unresolved",
    };
  }
...
async function fetchJsonViaCurl(url, init = {}, timeoutMs = HTTP_JSON_TIMEOUT_MS) {
  const headers = new Headers(init.headers || {});
  const args = [
    "-sS",
    "-L",
  ...
  try {
    ...
    if (markerIndex < 0) throw new Error("curl missing status marker");
...
function isRetryableFetchJsonFailure(error) {
...
async function fetchJsonWithRetry(url, init = {}, options = {}) {
  const attempts = Math.max(1, Math.min(4, Number(options.attempts || 1) || 1));
  const timeoutMs = Math.max(5_000, Math.min(600_000, Number(options.timeoutMs || HTTP_JSON_TIMEOUT_MS) || HTTP_JSON_TIMEOUT_MS));
  const backoffMs = Math.max(250, Math.min(15_000, Number(options.backoffMs || 2_500) || 2_500));
  const subprocessFallback = Boolean(options.subprocessFallback);
  const retryOnStatus = Array.isArray(options.retryOnStatus) && options.retryOnStatus.length
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetchJson(url, init, timeoutMs);
      lastResponse = response;
      if (response.ok || !retryOnStatus.includes(Number(response.status || 0)) || attempt >= attempts) {
        return response;
      }
      console.info(JSON.stringify({
        ok: false,
        stage: "fetch_json_retryable_status",
        attempt,
        attempts,
        status: response.status,
      ...
      lastError = error;
      if (subprocessFallback && isRetryableFetchJsonFailure(error)) {
        console.info(JSON.stringify({
          ok: false,
          stage: "fetch_json_curl_fallback",
          attempt,
          attempts,
          error: String(error?.message || error).slice(0, 240),
          url: String(url).slice(0, 240),
        }));
        return await fetchJsonViaCurl(url, init, timeoutMs);
      }
      if (!isRetryableFetchJsonFailure(error) || attempt >= attempts) throw error;
      console.info(JSON.stringify({
        ok: false,
        stage: "fetch_json_retryable_error",
        attempt,
        attempts,
        error: String(error?.message || error).slice(0, 240),
        url: String(url).slice(0, 240),
...
async function runCycle(config, cycle) {
  ...
  url.searchParams.set("priority", config.priority);
  url.searchParams.set("deep_only", config.deepOnly ? "1" : "0");
  url.searchParams.set("shard_index", String(Math.min(config.shardIndex, Math.max(0, config.shardCount - 1))));
  url.searchParams.set("shard_count", String(config.shardCount));
  if (mode === "audio") {
    url.searchParams.set("transcribe", config.transcribe ? "1" : "0");
    url.searchParams.set("language", config.language);
  } else {
    url.searchParams.set("provider", provider);
    url.searchParams.set("use_local_resolver", config.useLocalResolver && localResolverAllowedForPlatform(platform) ? "1" : "0");
  }

  try {
    phase = "heartbeat_start";
    await postHeartbeat(
      config,
      `cycle ${cycle}: ${platform} ${mode} backfill`,
      mode === "audio"
        ? `transcribe=${config.transcribe ? "on" : "off"} limit=${limit} scan=${scan} shard=${config.shardIndex}/${config.shardCount} priority=${config.priority} deep=${config.deepOnly ? "on" : "off"}`
        : `provider=${provider} limit=${limit} scan=${scan} shard=${config.shardIndex}/${config.shardCount} priority=${config.priority} deep=${config.deepOnly ? "on" : "off"}`,
    );
    ...
    phase = "backfill_fetch";
    const result = await fetchJsonWithRetry(url.toString(), {
      headers: { Authorization: `Bearer ${config.secret}` },
    }, {
      attempts: mode === "media" && platform === "instagram" ? 3 : 2,
      backoffMs: mode === "media" && platform === "instagram" ? 4_000 : 2_000,
      timeoutMs: HTTP_JSON_TIMEOUT_MS,
      subprocessFallback: mode === "media" && platform === "instagram",
      retryOnStatus: [500, 502, 503, 504],
    });

    const body = result.json && typeof result.json === "object" ? result.json : {};
    let runs = Array.isArray(body.runs) ? body.runs : [];
    if (mode === "media" && runs.length === 0 && config.localResolverAvailable) {
      const fallbackRuns = await fetchLocalMediaBackfillCandidates(config, platform, limit, scan);
      if (fallbackRuns.length) {
        runs = fallbackRuns;
        console.info(JSON.stringify({
          ok: true,
          stage: "local_media_candidate_fallback",
          platform,
          provider,
    ...
    if (mode === "media" && shouldApplyInstagramApifyMediaFallback(platform, provider, runs)) {
      const mediaMap = await fetchInstagramMediaMapViaApify(runs.map((run) => run?.url));
      if (mediaMap.size) {
        runs = runs.map((run) => {
          const key = normalizeInstagramPermalink(run?.url);
          const media = mediaMap.get(key);
          if (!media?.media_url) return run;
          return {
            ...run,
            found: 1,
            matched: 1,
            matched_with_media: 1,
            effective_provider: "apify_instagram",
            media_url: media.media_url,
            sample_found: {
              ...(rec(run?.sample_found)),
              url: run?.url || rec(run?.sample_found).url || key,
              media_url: media.media_url,
              video_url: media.media_url,
              author: media.author || rec(run?.sample_found).author || null,
              duration_sec: media.duration_sec ?? rec(run?.sample_found).duration_sec ?? null,
              platform,
            },
            sample_matched: {
              ...(rec(run?.sample_matched)),
              url: run?.url || rec(run?.sample_matched).url || key,
              media_url: media.media_url,
              video_url: media.media_url,
              author: media.author || rec(run?.sample_matched).author || null,
              duration_sec: media.duration_sec ?? rec(run?.sample_matched).duration_sec ?? null,
              platform,
            },
          };
        });
    ...
    if (mode === "media" && platform === "tiktok" && provider === "apify_tiktok") {
      const unresolved = runs.filter((run) => Number(run?.matched_with_media || 0) <= 0);
      if (unresolved.length) {
        const mediaMap = await fetchTiktokMediaMapViaApify(unresolved.map((run) => run?.url));
        if (mediaMap.size) {
          runs = runs.map((run) => {
            const key = normalizeTiktokVideoUrl(run?.url);
            const media = mediaMap.get(key);
            if (!media?.media_url) return run;
            return {
              ...run,
              found: Math.max(1, Number(run?.found || 0) || 0),
              matched: Math.max(1, Number(run?.matched || 0) || 0),
              matched_with_media: 1,
              effective_provider: "apify_tiktok",
              media_url: media.media_url,
              sample_found: {
                ...(rec(run?.sample_found)),
                url: run?.url || rec(run?.sample_found).url || key,
    ...
    if (mode === "media" && config.useLocalResolver && config.localResolverAvailable) {
      phase = "local_resolver";
      for (const run of runs) {
        const targetUrl = toText(run?.url);
        const rowId = Number(run?.id || 0);
        const matchedWithMedia = Number(run?.matched_with_media || 0) || 0;
        const directMediaUrl = toText(
          run?.media_url
          || rec(run?.sample_matched).media_url
          || rec(run?.sample_matched).video_url
          || rec(run?.sample_found).media_url
          || rec(run?.sample_found).video_url,
        );
        if (!targetUrl || !rowId) continue;
        if (directMediaUrl && !isImageLikeUrl(directMediaUrl)) {
          phase = `media_commit_direct:${rowId}`;
          const commit = await fetchJson(`${config.baseUrl}/api/factory/jobs/reels-brain-media-commit`, {
            ...
            body: JSON.stringify({
              ...
              url: targetUrl,
              platform,
              media_url: directMediaUrl,
              title: toText(run?.title) || toText(rec(run?.sample_matched).title) || toText(rec(run?.sample_found).title),
              author: toText(run?.author) || toText(rec(run?.sample_matched).author) || toText(rec(run?.sample_found).author),
              duration_sec: parseNumber(run?.duration_sec ?? rec(run?.sample_matched).duration_sec ?? rec(run?.sample_found).duration_sec),
            }),
          });
        ...
        if (
          platform === "youtube"
          && /(youtube\.com\/shorts\/|youtube\.com\/watch\?|youtu\.be\/)/i.test(targetUrl)
        ) {
          phase = `media_commit_page:${rowId}`;
          const commit = await fetchJson(`${config.baseUrl}/api/factory/jobs/reels-brain-media-commit`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    if (mode === "audio" && config.localAudioEnabled) {
      ...
      for (const run of runs) {
        ...
        const sourceUrl = toText(run?.url);
        const rowId = Number(run?.id || 0);
        if ((!mediaUrl && !sourceUrl) || !rowId) continue;

        phase = `local_audio_extract:${rowId}`;
    ...
    const attempted = runs.length || Number(body.attempted || 0) || 0;
    ...
    console.log(JSON.stringify({
      ...
      local_resolver_enabled: config.useLocalResolver && localResolverAllowedForPlatform(platform),
      route_local_resolver_enabled: body.local_resolver_enabled || false,
    ...
    throw new Error(`runCycle phase=${phase} mode=${mode} platform=${platform}: ${message}`);
  }
}
...
const config = {
  ...
  audioScan: intFlag(opts["audio-scan"] || process.env.REELS_BRAIN_AUDIO_BACKFILL_SCAN || 18, 18, 1, 2000),
  ...
  priority: toText(opts.priority || process.env.REELS_BRAIN_QUEUE_PRIORITY || "smart") || "smart",
  deepOnly: boolFlag(opts["deep-only"] ?? process.env.REELS_BRAIN_DEEP_ONLY ?? "0", false),
  shardIndex: intFlag(opts["shard-index"] || process.env.REELS_BRAIN_WORKER_SHARD_INDEX || 0, 0, 0, 15),
  shardCount: intFlag(opts["shard-count"] || process.env.REELS_BRAIN_WORKER_SHARD_COUNT || 1, 1, 1, 16),
...
config.mediaProviders = {
  instagram: toText(process.env.REELS_BRAIN_MEDIA_BACKFILL_PROVIDER_INSTAGRAM || config.provider || "apify_instagram"),
