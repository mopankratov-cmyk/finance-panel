...
Options:
  ...
  --mode <media|audio>         Worker mode (default: media)
...
async function runCycle(config, cycle) {
  ...
  url.searchParams.set("scan", String(config.scan));
  ...
  await postHeartbeat(
    config,
    `cycle ${cycle}: ${config.platform} ${config.mode} backfill`,
    config.mode === "audio"
      ? `transcribe=${config.transcribe ? "on" : "off"} limit=${config.limit} scan=${config.scan}`
      : `provider=${config.provider} limit=${config.limit} scan=${config.scan}`,
  );
  ...
  if (config.mode === "media" && config.useLocalResolver && config.localResolverAvailable) {
...
const config = {
  ...
  mode: toText(opts.mode || process.env.REELS_BRAIN_OFFLINE_MODE || "media") === "audio" ? "audio" : "media",
