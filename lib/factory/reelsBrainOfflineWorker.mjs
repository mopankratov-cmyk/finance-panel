...
async function runCycle(config, cycle) {
  const url = new URL("/api/factory/jobs/reels-brain-media-backfill", config.baseUrl);
  ...
  await postHeartbeat(
    config,
    `cycle ${cycle}: ${config.platform} media backfill`,
  ...
  if (config.useLocalResolver && config.localResolverAvailable) {
  ...
  const note = `attempted=${attempted} with_media=${withMedia} enriched=${enriched} inserted=${inserted} local_commits=${locallyResolved}`;
