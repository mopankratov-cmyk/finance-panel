...
async function hasAudioToolchain() {
  const [ffprobe, ffmpeg] = await Promise.all([hasBinary("ffprobe"), hasBinary("ffmpeg")]);
...
async function transcribeFal(audioUrl, language = "ru") {
...
config.localAudioEnabled = config.localAudioPreferred ? await hasAudioToolchain() : false;
