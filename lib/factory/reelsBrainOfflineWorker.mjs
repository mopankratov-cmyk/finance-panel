...
async function fetchInstagramMediaMapViaApify(urls) {
  ...
  for (const actor of actorCandidates) {
    try {
      const response = await fetch(`https://api.apify.com/v2/acts/${actor}/run-sync-get-dataset-items?token=${token}&maxItems=${directUrls.length}`, {
        method: "POST",
...
function shouldApplyInstagramApifyMediaFallback(platform, provider, runs) {
  ...
  return runs.every((run) => {
    const effectiveProvider = toText(run?.effective_provider || run?.provider);
    const matchedWithMedia = Number(run?.matched_with_media || 0) || 0;
    const found = Number(run?.found || 0) || 0;
    const matched = Number(run?.matched || 0) || 0;
    return matchedWithMedia <= 0
      && found <= 0
      && matched <= 0
      && (!effectiveProvider || effectiveProvider === "bright_instagram_post");
  });
}
