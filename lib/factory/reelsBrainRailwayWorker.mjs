...
async function runStep(name, fn) {
  ...
  await heartbeat("working", `running ${name}`);
  try {
    ...
    await heartbeat("working", `finished ${name} in ${durationSec}s`, { note: compact(result, 700) });
    ...
    await heartbeat("working", `step ${name} failed; continuing`, { blocker: message });
...
async function runCycle() {
  ...
  await heartbeat("working", `cycle started ${cycleStarted}`);
  ...
  await heartbeat(ok ? "working" : "blocked", ok ? "cycle finished; waiting next tick" : "cycle finished with recoverable errors", {
...
while (true) {
  await runCycle().catch(async (error) => {
    ...
    await heartbeat("blocked", "cycle crashed; worker will retry", { blocker: message }).catch(() => {});
  });
