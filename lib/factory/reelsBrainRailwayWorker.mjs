...
const avProbeLimit = numberEnv("REELS_BRAIN_AV_PROBE_LIMIT", 10, 0, 20);
...
async function downloadAssetToTemp(assetUrl, maxBytes) {
  const res = await fetch(signedAssetUrl(assetUrl), { signal: AbortSignal.timeout(90_000) });
  ...
  if (enableAvProbe && avProbeLimit > 0) {
    results.push(await runStep("audio visual ffprobe", async () => {
      ...
      const todo = candidates
        .filter((row) => row?.video_id && row?.asset_url && row?.asset_kind === "video" && row?.media_probe?.ok !== true)
