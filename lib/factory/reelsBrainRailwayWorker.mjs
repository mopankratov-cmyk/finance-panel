...
const avProbeLimit = numberEnv("REELS_BRAIN_AV_PROBE_LIMIT", 3, 0, 20);
...
Env:
  ...
  REELS_BRAIN_AV_PROBE_LIMIT     Default/max: 3/20 assets per cycle
...
async function downloadAssetToTemp(assetUrl, maxBytes) {
  const res = await fetch(assetUrl, { signal: AbortSignal.timeout(90_000) });
  ...
  if (enableAvProbe && avProbeLimit > 0) {
    results.push(await runStep("audio visual ffprobe", async () => {
      ...
      const todo = candidates
        .filter((row) => row?.video_id && row?.asset_url && row?.asset_kind === "video")
