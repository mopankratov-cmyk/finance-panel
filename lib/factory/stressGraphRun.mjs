...
function buildMarkdownReport(payload) {
  ...
  const stressTargetMet = summary.completed === summary.totalRuns && !summary.failed && !summary.runFail && !summary.timeouts;
  const lines = [
    "# Stress Run Report",
    "",
...
const summary = {
  ...
  timeouts: results.filter((r) => r.error === "timeout waiting for completion").length,
  avgDurationSec: Math.round(results.reduce((acc, r) => acc + r.durationSec, 0) / Math.max(1, results.length)),
};
summary.targetMet = summary.completed === summary.totalRuns && !summary.failed && !summary.runFail && !summary.timeouts;
...
if (summary.completed !== runs || summary.failed || summary.runFail || summary.timeouts) {
  process.exitCode = 1;
}
