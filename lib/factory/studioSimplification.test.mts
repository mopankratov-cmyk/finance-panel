...
ok(/state==="alive" \? \{c:"var\(--ok\)", bg:"var\(--ok-bg\)", label:"на связи"\}/.test(studio) && /running:\{c:"var\(--acc-lt\)", bg:"var\(--vio-bg\)", label:"идёт"\}/.test(studio) && /run_fail:\{c:"var\(--err\)", bg:"var\(--err-bg\)", label:"сбой"\}/.test(studio), "worker status chips use product copy instead of raw backend enums");
...
ok(/const heartbeatSummary=heartbeatDiag\.issue==="table_missing"[\s\S]*фолбэк и не блокирует прогоны/.test(studio) && /"Следом: "/.test(studio), "worker heartbeat diagnostics stay compact and explain the factory impact first");
...
ok(/"Сигнал качества"/.test(studio) && /"категории сбоев"/.test(studio) && /"живые инциденты"/.test(studio) && /"тренд по часам"/.test(studio), "full observability sections are labeled in consistent Russian product language");
ok(/Если пульс уйдёт в `молчит` или `потерян`/.test(studio) && /"пульс "\+String\(heartbeatDiag\.issue\|\|heartbeatDiag\.title\|\|"unknown"\)/.test(studio) && /"всего "\+\(p\.total\|\|0\)\+" · ок "\+\(p\.passed\|\|0\)\+" · warning "\+\(p\.warning\|\|0\)\+" · сбой "\+\(p\.failed\|\|0\)/.test(studio), "assistant guidance and observability summaries use the same operator vocabulary");
