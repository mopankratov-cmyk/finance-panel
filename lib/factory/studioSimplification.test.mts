...
ok(/"шаги прогона"/.test(studio) && /"хвост "\+execLog\.length/.test(studio) && /st==="done"\?"готово":st==="warning"\?"предупр\.":"сбой"/.test(studio) && /"вход "\+inp/.test(studio) && /" · выход "\+out/.test(studio), "OTK execution log stays readable in product language");
...
ok(/function recipeStatusLabel\(status\)\{[\s\S]*warning:"предупр\."[\s\S]*running:"идёт"/.test(studio) && /"в банк — победитель ✓/.test(studio) && /"✓ рынок → хук в победители"/.test(studio) && /"нужна перепроверка"/.test(studio) && /"каталог: "/.test(studio), "library and operator toasts use product-facing recipe and winner language");
