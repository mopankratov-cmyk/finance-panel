...
ok(/const sourceLabel=workerSource==="heartbeat_db"\?"heartbeat":"фолбэк";/.test(studio) && !/infra note:/.test(studio) && /"пульс потерян"/.test(studio), "worker status copy stays human and hides infra-note fallback text");
...
ok(/header\("Пульс завода","пульс · прогоны · контроль"/.test(studio) && /loadingCard\("смотрю пульс и живые прогоны…"\)/.test(studio) && /const heartbeatSummary=heartbeatDiag\.issue==="table_missing"[\s\S]*фолбэк и не блокирует прогоны/.test(studio) && /heartbeatIssueLabel\(heartbeatDiag\.issue\)/.test(studio) && /"Следом: "/.test(studio), "worker heartbeat diagnostics stay compact and explain the factory impact first");
...
ok(/"Сигнал качества"/.test(studio) && /"категории сбоев"/.test(studio) && /"живые инциденты"/.test(studio) && /"тренд по часам"/.test(studio) && /"модель "\+String\(quality\.model_runs\|\|0\)/.test(studio) && /"текст "\+String\(quality\.text_runs\|\|0\)/.test(studio), "full observability sections are labeled in consistent Russian product language");
ok(/Если пульс уйдёт в `молчит` или `потерян`/.test(studio) && /"пульс "\+heartbeatIssueLabel\(heartbeatDiag\.issue\|\|heartbeatDiag\.title\|\|"unknown"\)/.test(studio) && /"всего "\+\(p\.total\|\|0\)\+" · ок "\+\(p\.passed\|\|0\)\+" · warning "\+\(p\.warning\|\|0\)\+" · сбой "\+\(p\.failed\|\|0\)/.test(studio) && /errorCategoryLabel\(r\.error_category\)/.test(studio), "assistant guidance and observability summaries use the same operator vocabulary");
