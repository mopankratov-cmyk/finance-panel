...
ok(/tgSendMessage/.test(source), "send-review route can fall back to link message");
...
ok(/if \(!sent\?\.ok\)/.test(source), "send-review route falls back when Telegram cannot fetch a video URL");
