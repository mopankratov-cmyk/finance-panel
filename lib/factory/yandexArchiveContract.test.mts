...
ok(/content_assets/.test(helper) && /ARCHIVE_DISKS = \["gen", "product_twin", "prepared"\]/.test(helper), "Yandex archive reads generated and product twin assets");
...
ok(/\.in\("disk", ARCHIVE_DISKS\)/.test(helper) && /\.in\("kind", ARCHIVE_KINDS\)/.test(helper), "Yandex archive scans configured disks and media kinds");
