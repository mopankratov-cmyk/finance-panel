...
ok(/\.env\.vercel\.local/.test(worker), "worker auto-loads local Vercel env exports");
ok(/\.env\.vercel\.production\.local/.test(worker), "worker auto-loads production Vercel env exports");
ok(/\/Users\/maksimpankratov\/finance-panel\/\.env\.production\.local/.test(worker), "worker checks the main project production env file");
