...
// Используем response_url из ответа сабмита (собственные URL fal) — надёжнее, чем конструировать.
const SUBMIT = "https://queue.fal.run/fal-ai/flux/dev";

function key(): string | null { return process.env.FAL_KEY || null; }

// Сабмит. Возвращает токен (base64 от response_url) или null. faceUrl не используется (дефолтная модель).
export async function falPulidSubmit(prompt: string, _faceUrl?: string): Promise<string | null> {
  ...
  try {
    ...
    if (!r.ok) return null;
    const j = (await r.json()) as { response_url?: string; request_id?: string };
    if (!j.response_url) return null;
    return Buffer.from(j.response_url).toString("base64url");
  } catch { return null; }
}

export interface FalStatus { status: "in_progress" | "done" | "error"; imageUrl?: string; error?: string }

// token = base64url(response_url). status = response_url + "/status", результат = response_url.
export async function falPulidStatus(token: string): Promise<FalStatus> {
  const k = key();
