...
export function validateExpenseCategory(input: Record<string, unknown>): { name: string; opiuArticleId: string | null } {
  ...
  const opiuArticleId = input.opiuArticleId == null || input.opiuArticleId === "" ? null : String(input.opiuArticleId);
  if (!name || name.length > 160 || /[\u0000-\u001f]/.test(name)) throw new Error("Укажите название статьи от 1 до 160 символов");
