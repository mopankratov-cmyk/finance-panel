...
async function collectTarget(context, target) {
  ...
  try {
    ...
    // Ставим ДО первого перехода, иначе главная успеет отдать прежний кабинет.
    ...
    await randomPause();
    // WB мог не принять переключение (сессия чужая, кабинет отвязан). Тогда на
    // странице будут выплаты ДРУГОГО кабинета — снимать их под этим cabinetId
    // нельзя: в календаре это выглядело бы как обычная выплата.
    const switched = verifySupplierSwitch(target, await context.cookies("https://seller.wildberries.ru"));
    if (!switched.ok) throw new Error(`SUPPLIER_MISMATCH: ${switched.reason}`);
    ...
    if (table) {
      ...
      log("table_gave_nothing", { marketplace: target.marketplace, cabinetId: target.cabinetId, headers: table.headers.slice(0, 24) });
    }
