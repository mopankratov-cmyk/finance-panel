...
const REPORT_HEADERS = [
  /номер\s+документа|№\s*документа/i,
  /№\s*отч[её]та|номер\s+отч[её]та/i,
  /id\s+заявки|№\s*заявки|номер\s+заявки/i,
];
/** Фактический день перечисления — сильнее планируемого, когда он проставлен. */
const SENT_DATE_HEADERS = [/дата\s+отправки|дата\s+перечисления\s+средств/i];
 ...
 * @returns {{ reportId: number, period: number, amount: number, date: number, sentDate: number, status: number, dateIsWeak: boolean }}
 */
export function mapColumns(headers) {
  ...
  return {
    sentDate: bestIndex(headers, SENT_DATE_HEADERS).index,
 ...
 *
 * Ozon кладёт в ячейку суммы ДВЕ строки — начислено и к выплате: «22 948 ₽
 * 22 948 ₽». Пока они равны, это одно и то же число и брать его безопасно;
 * как только они разойдутся, ячейка неоднозначна, и молча взять одну из двух
 * нельзя — строка пропускается с причиной.
 */
export function cellAmount(cell) {
  const text = String(cell ?? "").replace(/[  ]/g, " ").trim();
  if (!text) return null;
  const found = [...text.matchAll(/(-|−|–)?\s*(\d{1,3}(?:\s\d{3})*(?:[.,]\d{1,2})?|\d+(?:[.,]\d{1,2})?)\s*(?:₽|руб\.?)?/gi)]
    .map((match) => {
  ...
  return found.every((value) => value === found[0]) ? found[0] : null;
}

/** Номер документа/отчёта: «№319211 от 10.08.2026» → «319211». */
export function cellReportId(cell) {
  const text = String(cell ?? "").replace(/\s+/g, " ").trim();
  if (!text || text === "—") return null;
  const numbered = text.match(/№\s*([\wА-Яа-я/-]+)/);
  if (numbered) return numbered[1];
  return /\d/.test(text) ? text.slice(0, 80) : null;
}
...
export function parsePayoutTableRow(cells, columns, target, options = {}) {
  ...
  // Отрицательное «Итого к оплате» — не выплата, а долг продавца: удержания за
  // период перекрыли реализацию. В календарь поступлений такому не место, и
  // панель такие снимки отвергает. Отсеиваем здесь, с понятной причиной.
  if (amount < 0) return { skipped: "сумма отрицательная — это удержание, а не выплата" };

  const period = cellPeriod(at(columns.period));
  const reportId = cellReportId(at(columns.reportId));
  ...
  // Фактический день отправки денег сильнее планируемого; планируемый берём,
  // пока выплата не ушла. День формирования отчёта не годится ни в каком виде.
  const plannedDate = cellDate(at(columns.sentDate))
    ?? (columns.dateIsWeak ? null : cellDate(at(columns.date)));
