...
export function bonusType(row: WbReportRow): string {
  return String(row.bonus_type_name ?? "").toLowerCase().trim();
}
...
export function buildCostLookup(
  ...
  for (const d of deliveryCosts) {
    if (d.barcode) {
      if (!byBarcode.has(d.barcode)) byBarcode.set(d.barcode, d.cost_rub);
      if (!packagingByBarcode.has(d.barcode)) packagingByBarcode.set(d.barcode, d.packaging_rub);
    }
  }
  return { byArticle, byBarcode, packagingByArticle, packagingByBarcode };
}

export type CostLookup = ReturnType<typeof buildCostLookup>;

function unitCost(row: WbReportRow, lookup: CostLookup): number {
...
function unitPackaging(row: WbReportRow, lookup: CostLookup): number {
  const article = String(row.sa_name ?? "").trim().toUpperCase();
  const barcode = String(row.barcode ?? "");
  return lookup.packagingByArticle.get(article) ?? lookup.packagingByBarcode.get(barcode) ?? 0;
}

export function cogsForSales(sales: WbReportRow[], lookup: CostLookup): number {
...
export function packagingForSales(sales: WbReportRow[], lookup: CostLookup): number {
  return sales.filter(isSale).reduce((sum, row) => {
    const qty = Math.abs(num(row.quantity) || 1);
