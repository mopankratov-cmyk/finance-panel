...
import { normalizeWbFinanceReports, optionalMoney, scheduledWbPayouts, strictWbPaymentDate } from "./wbPayoutStatus";
...
test("scheduled payout is not invented when WB did not return an exact date", () => {
  ...
  assert.deepEqual(scheduledWbPayouts(reports), [{ reportId: "12", date: "2026-08-29", amount: 1900 }]);
});
