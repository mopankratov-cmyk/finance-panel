...
export interface UnitRefreshQuery {
  forceRefresh: boolean;
  backgroundRefresh: boolean;
}

export function parseUnitRefreshQuery(searchParams: URLSearchParams): UnitRefreshQuery {
  const parseFlag = (name: "refresh" | "background"): boolean => {
  ...
  const forceRefresh = parseFlag("refresh");
  const backgroundRefresh = parseFlag("background");
  if (forceRefresh && backgroundRefresh) {
    throw new Error("Параметры refresh и background нельзя включать одновременно");
  }
  return { forceRefresh, backgroundRefresh };
}
