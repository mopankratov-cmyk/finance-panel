 ...
 */
export async function attachKizEntities(db: SupabaseClient): Promise<{ byProduct: number; byCabinet: number; left: number } | null> {
  const { data, error } = await db.rpc("kiz_attach_legal_entity");
  if (error || !data) return null;
  const row = data as Record<string, unknown>;
  return {
    byProduct: Number(row.byProduct ?? 0) || 0,
    byCabinet: Number(row.byCabinet ?? 0) || 0,
