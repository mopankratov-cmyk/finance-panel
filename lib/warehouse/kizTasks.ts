...
/** Имя фоновой задачи в журнале синхронизаций. */
export const KIZ_NIGHTLY_JOB = "kiz-withdrawals";
...
/** Ошибка отсутствующей миграции — вызывающий решает, каким текстом отвечать. */
export class KizTasksMigrationError extends Error {}
 ...
 *
 * Живёт в lib, потому что делают это двое: человек кнопкой «Обновить» и ночной
 * прогон. Две копии одного правила однажды разошлись бы молча.
 */
export async function collectKizFromTasks(db: SupabaseClient, cabinetIds: string[]): Promise<KizTasksResult> {
  ...
  try {
    ...
    if (missingMigration(code)) throw new KizTasksMigrationError("Примените миграцию 202608240026_kiz_srid.sql");
    throw error;
  }
