...
export type WriteAbility =
  /** Токен пишет: WB пустил запрос дальше проверки прав. */
  | { can: true }
  /** Токен выпущен «только на чтение» — WB сказал это прямым текстом. */
  | { can: false; reason: "read-only"; message: string }
  /** Категории «Продвижение» нет или ключ истёк. */
  | { can: false; reason: "no-access"; message: string }
  /** Спросить не удалось: сеть, лимит, невнятный ответ. Это НЕ «нельзя». */
  | { can: false; reason: "unknown"; message: string };

/**
 * Может ли ключ писать — спросив у самого WB, а не гадая по токену.
 *
 * Долго это считалось непроверяемым: признака «только на чтение» в payload
 * JWT нет, и панель честно писала «право записи выяснится на первой попытке».
 * Живая проверка 01.09.2026 дала недостающий факт — на запрещённый запрос WB
 * отвечает 403 с `detail: "read-only token cannot perform non-readonly
 * requests"`, причём отвечает ГЕЙТВЕЕМ, до всякой бизнес-логики.
 *
 * Отсюда безопасный зонд: пишущий метод с заведомо пустой работой. Пустой
 * массив ставок менять нечего — применить такой запрос не к чему даже
 * теоретически, — но по классификации WB он остаётся non-readonly, и ключ без
 * права записи спотыкается о гейтвей. Ключ с правом проходит дальше и получает
 * обычный ответ на пустой список.
 *
 * Ни одна кампания при этом не трогается. Именно поэтому зонд не переименовывает
 * кампанию «в то же имя» — тот вариант тоже сработал бы, но оставил бы след в
 * истории кабинета WB, а проверять права ценой записи в чужой журнал нельзя.
 */
export async function probeWriteAbility(token: string): Promise<WriteAbility> {
  const probe = await advertCall<unknown>({
    token,
  ...
  if (probe.ok) return { can: true };

  if (probe.status === 403) {
    const raw = typeof probe.raw === "string" ? probe.raw : JSON.stringify(probe.raw ?? "");
    // Отличаем «ключ только на чтение» от «нет доступа к этому объекту».
    // Первое чинится перевыпуском ключа, второе — нет, и путать их вредно.
    if (/read-?only/i.test(raw)) {
      return {
        can: false,
        reason: "read-only",
        message: "Ключ выпущен «Только на чтение». Перевыпустите токен Продвижения без этой галочки.",
      };
    }
    return { can: false, reason: "no-access", message: probe.message };
  }
  if (probe.status === 401) return { can: false, reason: "no-access", message: probe.message };

  // 400 на пустой список — это уже ответ бизнес-логики, то есть проверку прав
  // запрос прошёл. Такой отказ означает «писать можно», а не «нельзя».
  if (probe.status === 400) return { can: true };

  return { can: false, reason: "unknown", message: probe.message };
}
