...
function rebuildTone(value: string) {
  if (value === "aligned") return { label: "aligned", bg: "#dcfce7", bd: "#86efac", fg: "#166534" };
  if (value === "partial") return { label: "partial", bg: "#ecfeff", bd: "#67e8f9", fg: "#0f766e" };
  if (value === "mismatch") return { label: "mismatch", bg: "#fee2e2", bd: "#fca5a5", fg: "#991b1b" };
  return { label: "unknown", bg: "#f8fafc", bd: "#cbd5e1", fg: "#64748b" };
}

function rebuildModeLabel(value: unknown) {
  const raw = String(value || "").trim().toLowerCase();
  if (raw === "brief_bundle_completion") return "brief bundle";
  if (raw === "ship_ready_generation") return "ship-ready";
  if (raw === "high_trust_generation") return "high-trust";
  if (!raw) return "general memory";
  return raw.replace(/_/g, " ");
}

function rebuildBiasSummary(context: JsonRecord) {
  const tags = [
    context.brief_bundle_biased ? "brief" : "",
    context.ship_ready_biased ? "ship" : "",
    context.high_trust_generation_biased ? "high-trust" : "",
    context.exact_proof_biased ? "exact-proof" : "",
    context.output_ready_biased ? "output-ready" : "",
  ].filter(Boolean);
  return tags.length ? tags.join(" · ") : "общая память";
}
...
export default function ReelsBrainPixelCockpit({ initialData }: { initialData?: BootPayload }) {
  ...
  const vm = useMemo(() => {
    ...
    const segmentOutputCards = ((segmentOutputBanks.briefs || []) as JsonRecord[]).slice(0, 6).map((row, index) => {
      ...
      const memoryContext = (actionPrimary.memory_context || primary.memory_context || {}) as JsonRecord;
      const memoryAlignment = (memoryContext.rebuild_alignment || {}) as JsonRecord;
      ...
      return {
        ...
        policyReason,
        memoryContext,
        memoryTone: rebuildTone(String(memoryAlignment.status || "unknown")),
        memoryNote: String(memoryContext.memory_note || ""),
      };
    });
    ...
    const segmentDecisionCards = ((segmentDecisionDeck.items || []) as JsonRecord[]).slice(0, 6).map((row) => {
      ...
      const memoryContext = (row.memory_context || {}) as JsonRecord;
      const memoryAlignment = (memoryContext.rebuild_alignment || {}) as JsonRecord;
      return {
        ...row,
        gradeTone: grade,
    ...
    const portfolioSummary = {
      highTrustCoverage: num(portfolioReadiness.summary?.high_trust_coverage_pct || mission.portfolio_readiness?.high_trust_coverage_pct),
      generationReadyCoverage: num(portfolioReadiness.summary?.generation_ready_coverage_pct || mission.portfolio_readiness?.generation_ready_coverage_pct),
      audioBackedGenerationReadyCoverage: num(portfolioReadiness.summary?.audio_backed_generation_ready_pct || mission.portfolio_readiness?.audio_backed_generation_ready_pct),
      publishableExactCoverage: num(portfolioReadiness.summary?.publishable_exact_coverage_pct || mission.portfolio_readiness?.publishable_exact_coverage_pct),
      generationReadySegments: num(portfolioReadiness.summary?.generation_ready_segments || mission.portfolio_readiness?.generation_ready_segments),
      audioBackedGenerationReadySegments: num(portfolioReadiness.summary?.audio_backed_generation_ready_segments || mission.portfolio_readiness?.audio_backed_generation_ready_segments),
      publishableExactSegments: num(portfolioReadiness.summary?.publishable_exact_segments || mission.portfolio_readiness?.publishable_exact_segments),
      stableSegments: num(portfolioReadiness.summary?.stable_segments || mission.portfolio_readiness?.stable_segments),
      marketConfirmedSegments: num(portfolioReadiness.summary?.market_confirmed_segments || mission.portfolio_readiness?.market_confirmed_segments),
      weakAudioSegments: num(portfolioReadiness.summary?.weak_audio_segments || mission.portfolio_readiness?.weak_audio_segments),
      expectedSegments: num(portfolioReadiness.summary?.expected_segments || mission.portfolio_readiness?.expected_segments),
      verdict: String(portfolioReadiness.summary?.verdict || mission.portfolio_readiness?.verdict || "still_building"),
    };
    ...
    const trustMatrix = NICHES.map((niche) => {
      ...
      return {
        ...
        cells: ["tiktok", "instagram", "youtube"].map((platform) => {
          ...
          const audioSupport = (trust.audio_support || {}) as JsonRecord;
          return {
            platform,
            ...
            audioScore: num(audioSupport.score),
            audioStatus: String(audioSupport.status || "weak"),
            transcriptRate: num(audioSupport.with_transcript_rate),
    ...
    const trustHotspots = summaries
      ...
      .slice(0, 6);
    const memoryFitCards = NICHES.map((niche) => {
      const summary = summaries.find((item) => item.niche === niche) || {};
      const context = (summary.pattern_rebuild_context || {}) as JsonRecord;
      const mode = rebuildModeLabel(context.execution_mode);
      const focusPlatform = String(context.focus_platform || "").trim().toLowerCase();
      const biasSummary = rebuildBiasSummary(context);
      const score = Math.max(0, Math.min(100,
        (context.brief_bundle_biased ? 26 : 0)
        + (context.ship_ready_biased ? 22 : 0)
        + (context.high_trust_generation_biased ? 24 : 0)
        + (context.exact_proof_biased ? 12 : 0)
        + (context.output_ready_biased ? 10 : 0)
        + (focusPlatform ? 6 : 0),
      ));
      const status = score >= 40 ? "aligned" : score >= 18 ? "partial" : Object.keys(context).length ? "mismatch" : "unknown";
      const tone = rebuildTone(status);
      return {
        niche,
        mode,
        status,
        tone,
        score,
        focusPlatform: focusPlatform || "all",
        sourceVideos: num(context.source_videos),
        requestedLimit: num(context.requested_limit),
        persistedAt: safeDateLabel(context.persisted_at),
        biasSummary,
      };
    });
                  ...
                  {(row.cells || []).map((cell: JsonRecord) => (
                    <div key={cell.platform} style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 14, background: "#f8fafc" }}>
                      ...
                      <div style={{ marginTop: 8, color: "#334155", lineHeight: 1.6 }}>
                        ...
                        <div>audio trust: {compact(cell.audioScore)}% · {cell.audioStatus}</div>
                        <div>transcript depth: {compact(cell.transcriptRate)}%</div>
                      </div>
      ...
      <div className="rb-shell">
              ...
              <div className="rb-card" key={`segment-output:${item.niche}:${item.platform}`}>
                <div className="rb-two" style={{ gridTemplateColumns: "1fr auto", gap: 14 }}>
                  ...
                  <div style={{ display: "grid", gap: 8, justifyItems: "end" }}>
                    ...
                    {item.memoryContext?.rebuild_alignment?.status ? (
                      <div className="rb-live-pill" style={{ background: item.memoryTone.bg, borderColor: item.memoryTone.bd, color: item.memoryTone.fg }}>
                        <i style={{ background: item.memoryTone.fg }} />
                        memory {item.memoryTone.label}
                      </div>
                    ) : null}
                  </div>
                </div>
              ...
              <div className="rb-card" key={`segment-decision:${item.niche}:${item.platform}`}>
                <div className="rb-two" style={{ gridTemplateColumns: "1fr auto", gap: 14 }}>
                  ...
                  <div style={{ display: "grid", gap: 8, justifyItems: "end" }}>
                    ...
                    {item.memory_context?.rebuild_alignment?.status ? (
                      <div className="rb-live-pill" style={{ background: item.memoryTone.bg, borderColor: item.memoryTone.bd, color: item.memoryTone.fg }}>
                        <i style={{ background: item.memoryTone.fg }} />
                        memory {item.memoryTone.label}
                      </div>
        ...
        <section>
          ...
          <div className="rb-three" style={{ marginBottom: 16 }}>
            {vm.memoryFitCards.map((item) => (
              <div className="rb-card" key={`memory-fit:${item.niche}`}>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  <div className="rb-pill">{NICHE_LABELS[item.niche] || item.niche}</div>
                  <div className="rb-pill" style={{ background: item.tone.bg, borderColor: item.tone.bd, color: item.tone.fg }}>
                    {item.tone.label}
                  </div>
                  <div className="rb-pill">fit {compact(item.score)}</div>
                </div>
                <h3 style={{ font: "600 24px/1.15 'Space Grotesk'", margin: "14px 0 10px" }}>{item.mode}</h3>
                <p style={{ margin: 0, color: "#334155" }}>
                  Focus: {item.focusPlatform} · Bias: {item.biasSummary}
                </p>
                <p style={{ margin: "8px 0 0", color: "#475569" }}>
                  source videos {compact(item.sourceVideos)} · requested limit {compact(item.requestedLimit)}
                </p>
                <p style={{ margin: "8px 0 0", color: "#64748b", fontSize: 13 }}>
                  Последняя пересборка: {item.persistedAt}
                </p>
              </div>
            ))}
          </div>
              ...
              <>
                <div className="rb-pattern" style={{ borderColor: "#a7f3d0", background: "#f0fdf4" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    ...
                    {vm.actionPack.primary.memory_context?.rebuild_alignment?.status ? (
                      <div className="rb-pill" style={{
                        background: rebuildTone(String(vm.actionPack.primary.memory_context?.rebuild_alignment?.status || "unknown")).bg,
                        borderColor: rebuildTone(String(vm.actionPack.primary.memory_context?.rebuild_alignment?.status || "unknown")).bd,
                        color: rebuildTone(String(vm.actionPack.primary.memory_context?.rebuild_alignment?.status || "unknown")).fg,
                      }}>
                        memory {rebuildTone(String(vm.actionPack.primary.memory_context?.rebuild_alignment?.status || "unknown")).label}
                      </div>
                    ) : null}
                  </div>
                  ...
                  {vm.actionPack.primary.memory_context?.memory_note ? (
                    <p style={{ marginTop: 8, color: "#0f766e", fontWeight: 600 }}>{String(vm.actionPack.primary.memory_context.memory_note)}</p>
                  ) : null}
                  {vm.actionPack.primary.evidence?.trust_decision?.audio_support?.note ? (
                    <p style={{ marginTop: 8, color: "#0f172a", fontWeight: 600 }}>
                      Audio trust: {String(vm.actionPack.primary.evidence?.trust_decision?.audio_support?.status || "unknown")} · {compact(vm.actionPack.primary.evidence?.trust_decision?.audio_support?.with_transcript_rate)}% transcript
                    </p>
                  ) : null}
            ...
            <div className="rb-card" style={{ marginTop: 16 }}>
              ...
              <div className="rb-pattern" style={{ borderColor: "#bae6fd", background: "#f0f9ff" }}>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  ...
                  {vm.briefPack.decision_pack?.rebuild_alignment?.status ? (
                    <div className="rb-pill" style={{
                      background: rebuildTone(String(vm.briefPack.decision_pack?.rebuild_alignment?.status || "unknown")).bg,
                      borderColor: rebuildTone(String(vm.briefPack.decision_pack?.rebuild_alignment?.status || "unknown")).bd,
                      color: rebuildTone(String(vm.briefPack.decision_pack?.rebuild_alignment?.status || "unknown")).fg,
                    }}>
                      memory {rebuildTone(String(vm.briefPack.decision_pack?.rebuild_alignment?.status || "unknown")).label}
                    </div>
                  ) : null}
                </div>
                ...
                {vm.briefPack.decision_pack?.memory_note ? (
                  <p style={{ marginTop: 10, color: "#0f766e", fontWeight: 600 }}>{String(vm.briefPack.decision_pack.memory_note)}</p>
                ) : null}
                {vm.briefPack.decision_pack?.audio_note ? (
                  <p style={{ marginTop: 8, color: "#0f172a", fontWeight: 600 }}>
                    Audio trust: {String(vm.briefPack.decision_pack?.audio_support?.status || "unknown")} · {compact(vm.briefPack.decision_pack?.audio_support?.with_transcript_rate)}% transcript
                  </p>
                ) : null}
              </div>
        ...
        <section>
          ...
          <div className="rb-two">
            <div className="rb-card">
              <div className="rb-pill">{vm.measurementPlan.status || "waiting"} · candidates {compact(vm.measurementPlan.total_candidates || 0)} · audio weak {compact(vm.measurementPlan.audio_weak_candidates || 0)}</div>
              ...
              {((vm.measurementPlan.items || []) as JsonRecord[]).length ? ((vm.measurementPlan.items || []) as JsonRecord[]).slice(0, 4).map((item, index) => (
                <div className="rb-pattern" key={`measurement:${item.measurement_id || index}`} style={{ marginTop: index ? 10 : 0 }}>
                  ...
                  <p>{item.reason}</p>
                  <p><strong>Audio foundation:</strong> {String(item.audio_foundation_status || "ok")} · {String(item.audio_foundation_note || "audio status not specified")}</p>
        ...
        <section>
          ...
          <div className="rb-two" style={{ marginTop: 16 }}>
            <div className="rb-card">
              ...
              <h3 style={{ font: "700 26px/1.08 'Space Grotesk'", margin: "10px 0" }}>
                {compact(vm.portfolioSummary.highTrustCoverage)}% high-trust coverage
              </h3>
              <p style={{ color: "#64748b", lineHeight: 1.5 }}>
                {compact(vm.portfolioSummary.generationReadyCoverage)}% generation-ready · {compact(vm.portfolioSummary.audioBackedGenerationReadyCoverage)}% audio-backed gen-ready · {compact(vm.portfolioSummary.publishableExactCoverage)}% publishable exact · {String(vm.portfolioSummary.verdict || "still_building").replaceAll("_", " ")}
              </p>
              <div className="rb-three" style={{ marginTop: 14 }}>
                <div className="rb-brief-block"><b>Gen-ready</b><p>{compact(vm.portfolioSummary.generationReadySegments)}</p></div>
                <div className="rb-brief-block"><b>Audio-backed</b><p>{compact(vm.portfolioSummary.audioBackedGenerationReadySegments)}</p></div>
                <div className="rb-brief-block"><b>Exact-ready</b><p>{compact(vm.portfolioSummary.publishableExactSegments)}</p></div>
              </div>
              <div className="rb-three" style={{ marginTop: 12 }}>
                <div className="rb-brief-block"><b>Weak audio</b><p>{compact(vm.portfolioSummary.weakAudioSegments)}</p></div>
                <div className="rb-brief-block"><b>High trust</b><p>{compact(vm.portfolioSummary.marketConfirmedSegments)}</p></div>
                <div className="rb-brief-block"><b>Expected</b><p>{compact(vm.portfolioSummary.expectedSegments)}</p></div>
              </div>
              <div className="rb-brief-block" style={{ marginTop: 12 }}>
                <b>Что это значит</b>
                <p>Stable и high-trust показывают глубину понимания ниши. Publishable exact показывает, где уже закрыт точный доказательный слой. Audio-backed generation-ready показывает самый честный слой: где мозг уже может отдавать high-trust генеративные решения без хрупкого audio/transcript fallback.</p>
              </div>
              <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
                {vm.portfolioCoverageCards.map((row: JsonRecord) => (
                  <div className="rb-pattern" key={`portfolio-platform:${row.platform}`}>
                    <h3>{row.label}</h3>
                    <p>{compact(row.high_trust_pct || 0)}% high-trust · {compact(row.generation_ready_pct || 0)}% gen-ready · {compact(row.audio_backed_generation_ready_pct || 0)}% audio-backed · {compact(row.publishable_exact_pct || 0)}% publishable exact · coverage {compact(row.coverage_pct || 0)}%</p>
                    <p style={{ marginTop: 8 }}>{String(row.readinessLabel || "weak")} · weak audio {compact(row.weak_audio_segments || 0)} · next gap {String(row.next_gap || "none")}</p>
                  </div>
                ))}
              </div>
            </div>
