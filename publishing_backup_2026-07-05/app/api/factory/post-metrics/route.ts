...
import { normalizeDistributionPlatform } from "@/lib/factory/distribution";
...
function metricPlatformLabel(value: unknown): string {
  const platform = normalizeDistributionPlatform(value);
...
export async function POST(req: NextRequest) {
  ...
  // 1) запись метрик в post_metrics (оживляем мёртвую таблицу)
    ...
    const metricPlatform = normalizeDistributionPlatform(b.platform || b.target_platform || recipePlan.target_platform);
    ...
    try {
      const { error } = await db.from("post_metrics").insert({
        ...
        platform: metricPlatformLabel(metricPlatform).slice(0, 20),
