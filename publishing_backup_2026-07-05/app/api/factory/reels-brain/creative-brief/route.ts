...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: corpusRows } = await db
      .from("viral_videos")
      .select("platform,analyzed,analyzed_full")
      .eq("niche", niche)
      .limit(1000);
    ...
    const trustDecision = selectCreativeBriefBrainWithTrust({
      playbook,
      platform,
      feedbackRows: ((feedbackRows || []) as ReelsBrainMetricRow[]),
      corpusRows: ((corpusRows || []) as Array<{ platform?: string | null; analyzed?: boolean | null; analyzed_full?: unknown }>),
    });
    ...
    return NextResponse.json(
      normalizeLegacyCreativeBrief({
        ...
        memory_context: {
          rebuild_context: trustDecision.rebuild_context,
          rebuild_alignment: trustDecision.rebuild_alignment,
        },
        ...best,
