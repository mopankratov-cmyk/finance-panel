...
### 2026-07-05 — Publishing Cockpit first UI pass

- Ветка: `feat/product-broll-operator-get-clean`
- Цель: поднять первый read-only Publishing Cockpit в `app/inferno` поверх реальных `factory_*` данных и закрыть основные экранные состояния.
- Изменено:
  - `lib/factory/publishingCockpit.ts`: новый fail-open aggregator для `overview / bank / calendar / fleet / runs / metrics / channels / alerts`, включая worker pulse и derived alerts.
  - `app/api/factory/publishing-cockpit/route.ts`: read-only boot endpoint для cockpit с глобальным `stream` filter.
  - `app/inferno/publishing/page.tsx`: новый inferno route.
  - `app/inferno/publishing/PublishingCockpit.tsx`: первый hifi-ish UI-pass на 8 вкладок + Rita side panel + demo states `loading/empty/partial/error/ratelimited/banned/success/salvo`.
- Проверки:
  - `npm run dev` стартует чисто.
  - `npm run build` всё ещё падает на существующем webpack hashing crash (`WasmHash._updateWithBuffer`) даже при smoke-странице, то есть блокер не локализуется в новом publishing-коде.
- Результат:
  - В репо теперь есть первый live экран `/inferno/publishing` и серверный read-only payload для него.
  - Следующий шаг после починки общего build-crash: dogfood UI в браузере, подтянуть точные handoff-состояния по каждому дроверу и добавить targeted actions, когда владелец разрешит write-path.
