...
function memoryContext(pattern: ReelsBrainActionPattern) {
  const rebuildAlignment = pattern.evidence?.trust_decision?.rebuild_alignment || null;
  const rebuildContext = pattern.evidence?.trust_decision?.rebuild_context || null;
  return {
    ...
    memory_note: rebuildAlignment
      ? text(rebuildAlignment.status) === "aligned"
        ? "Память под этот rollout пересобиралась под близкий output context."
        : text(rebuildAlignment.status) === "partial"
          ? "Память частично совпадает с задачей: rollout лучше вести как control/guarded lane."
          : text(rebuildAlignment.status) === "mismatch"
            ? "Память смещена под другой context: перед scale лучше дособрать focused rebuild."
...
export function buildReelsBrainActionPack(
  ...
  const cards = sortActionCards([...patterns]
    .map((pattern, index) => ({
      ...
      memory_context: memoryContext(pattern),
  ...
  return {
    ...
    summary: {
      ...
      rebuild_alignment: cards[0]?.memory_context?.rebuild_alignment || null,
      memory_note: cards[0]?.memory_context?.memory_note || "",
