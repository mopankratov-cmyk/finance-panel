...
function normalizeRecipe(
  ...
  const rebuildAlignment = recipe.evidence?.trust_decision?.rebuild_alignment || null;
  const rebuildContext = recipe.evidence?.trust_decision?.rebuild_context || null;
  ...
  return {
    ...
    memory_context: {
      rebuild_alignment: rebuildAlignment ? {
        status: text(rebuildAlignment.status, "unknown"),
        score: num(rebuildAlignment.score),
        reasons: list(rebuildAlignment.reasons, 4),
      } : null,
      rebuild_context: rebuildContext ? {
        execution_mode: text(rebuildContext.execution_mode),
        focus_platform: text(rebuildContext.focus_platform),
      } : null,
      memory_note: rebuildAlignment
        ? text(rebuildAlignment.status) === "aligned"
          ? "Память под этот brief пересобиралась под близкий output context."
          : text(rebuildAlignment.status) === "partial"
            ? "Память частично совпадает с задачей: годится для control/brief режима."
            : text(rebuildAlignment.status) === "mismatch"
              ? "Память смещена под другой context: нужен focused rebuild."
              : "Контекст пересборки памяти пока не зафиксирован."
        : "Контекст пересборки памяти пока не зафиксирован.",
    },
  };
}
...
export function buildReelsBrainBriefPack(
  ...
  return {
    ...
    decision_pack: {
      rebuild_alignment: normalizedRanked[0]?.memory_context?.rebuild_alignment || null,
      memory_note: normalizedRanked[0]?.memory_context?.memory_note || "",
    },
  };
}
...
export function buildGroupedReelsBrainBriefPacks(input: {
  ...
  const sortGroups = <T extends { primary?: { segment_priority_mode?: string; segment_priority_score?: number; effective_op_score?: number; confidence?: string; title?: string; high_trust_generation_ready?: boolean; publishable_exact?: boolean; proof_quality?: string } | null }>(rows: T[]) =>
    rows.sort((a, b) =>
