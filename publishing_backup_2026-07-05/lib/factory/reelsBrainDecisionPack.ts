...
export type ReelsBrainDecisionTrust = {
  ...
  trust: {
    ...
    audio_support?: {
      ...
      confidence?: "high" | "medium" | "low";
      with_audio?: number;
      with_audio_rate?: number;
      with_transcript?: number;
      with_transcript_rate?: number;
      feature_depth?: {
        pause_map_ready?: number;
        pause_map_ready_rate?: number;
        pacing_ready?: number;
        pacing_ready_rate?: number;
        beat_hint_ready?: number;
        beat_hint_ready_rate?: number;
      } | null;
      why_ready?: string[];
      why_not_yet?: string[];
      note?: string;
  ...
  return {
    ...
    evidence: {
      ...
      trust_decision: {
        ...
        audio_support: input.trustDecision.trust.audio_support
          ? {
              score: num(input.trustDecision.trust.audio_support.score),
              status: text(input.trustDecision.trust.audio_support.status),
              confidence: text(input.trustDecision.trust.audio_support.confidence),
              with_audio: num(input.trustDecision.trust.audio_support.with_audio),
              with_audio_rate: num(input.trustDecision.trust.audio_support.with_audio_rate),
              with_transcript: num(input.trustDecision.trust.audio_support.with_transcript),
              with_transcript_rate: num(input.trustDecision.trust.audio_support.with_transcript_rate),
              feature_depth: input.trustDecision.trust.audio_support.feature_depth
                ? {
                    pause_map_ready: num(input.trustDecision.trust.audio_support.feature_depth.pause_map_ready),
                    pause_map_ready_rate: num(input.trustDecision.trust.audio_support.feature_depth.pause_map_ready_rate),
                    pacing_ready: num(input.trustDecision.trust.audio_support.feature_depth.pacing_ready),
                    pacing_ready_rate: num(input.trustDecision.trust.audio_support.feature_depth.pacing_ready_rate),
                    beat_hint_ready: num(input.trustDecision.trust.audio_support.feature_depth.beat_hint_ready),
                    beat_hint_ready_rate: num(input.trustDecision.trust.audio_support.feature_depth.beat_hint_ready_rate),
                  }
                : null,
              why_ready: Array.isArray(input.trustDecision.trust.audio_support.why_ready) ? input.trustDecision.trust.audio_support.why_ready : [],
              why_not_yet: Array.isArray(input.trustDecision.trust.audio_support.why_not_yet) ? input.trustDecision.trust.audio_support.why_not_yet : [],
              note: text(input.trustDecision.trust.audio_support.note) || null,
            }
          : null,
        ...
        rebuild_alignment: input.trustDecision.rebuild_alignment
          ? {
              status: input.trustDecision.rebuild_alignment.status,
              score: num(input.trustDecision.rebuild_alignment.score),
              reasons: Array.isArray(input.trustDecision.rebuild_alignment.reasons)
                ? input.trustDecision.rebuild_alignment.reasons
                : [],
            }
          : null,
        rebuild_context: input.trustDecision.rebuild_context
          ? {
              focus_platform: text(input.trustDecision.rebuild_context.focus_platform) || null,
              source_discovery_mode: text(input.trustDecision.rebuild_context.source_discovery_mode) || null,
              execution_mode: text(input.trustDecision.rebuild_context.execution_mode) || null,
              field_focus: text(input.trustDecision.rebuild_context.field_focus) || null,
              family_focus: text(input.trustDecision.rebuild_context.family_focus) || null,
              platform_biased: bool(input.trustDecision.rebuild_context.platform_biased),
              exact_proof_biased: bool(input.trustDecision.rebuild_context.exact_proof_biased),
              brief_bundle_biased: bool(input.trustDecision.rebuild_context.brief_bundle_biased),
              ship_ready_biased: bool(input.trustDecision.rebuild_context.ship_ready_biased),
              high_trust_generation_biased: bool(input.trustDecision.rebuild_context.high_trust_generation_biased),
              output_ready_biased: bool(input.trustDecision.rebuild_context.output_ready_biased),
              requested_limit: num(input.trustDecision.rebuild_context.requested_limit) || null,
              source_videos: num(input.trustDecision.rebuild_context.source_videos) || null,
              persisted_at: text(input.trustDecision.rebuild_context.persisted_at) || null,
            }
          : null,
      },
...
export function buildReelsBrainDecisionPack(input: {
  ...
  return {
    ...
    decision_pack: {
      ...
      memory_note: input.trustDecision.rebuild_alignment?.status === "aligned"
        ? "Память паттернов недавно пересобиралась под близкий output context, поэтому trust-layer ближе к рабочему бою."
        : input.trustDecision.rebuild_alignment?.status === "partial"
          ? "Память частично совпадает с нужным output context: использовать как ускоренный research/control слой."
          : input.trustDecision.rebuild_alignment?.status === "mismatch"
            ? "Последняя пересборка памяти была смещена под другой context; перед production-решением лучше дособрать нужный focused loop."
            : "Контекст последней пересборки памяти не зафиксирован, поэтому решение оценивается только по trust/evidence слоям.",
      audio_note: input.trustDecision.trust.audio_support?.status === "ready"
        ? "Voice/audio evidence уже достаточно плотный, поэтому decision-pack можно оценивать и по механике речи/ритма."
        : input.trustDecision.trust.audio_support?.status === "warming"
          ? "Voice/audio слой уже полезен, но пока лучше держать решения в control ladder."
          : input.trustDecision.trust.audio_support?.status === "weak"
            ? "Паттерны ещё слабо подкреплены voice/audio evidence: не масштабировать без контрольного теста."
            : "Voice/audio evidence пока не подключён к этому decision-pack.",
    },
  };
}
