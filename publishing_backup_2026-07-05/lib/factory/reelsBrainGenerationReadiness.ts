...
export function buildReelsBrainGenerationReadiness(input: {
  ...
  const items = Array.from(keys)
    .map((key) => {
      ...
      const audioState = audioBlockState(blockers);
      ...
      const highTrustGenerationReady = publishableExact && briefReady && contentSolutionReady && policyMode !== "research_only" && audioState !== "weak";
      return {
        niche,
        platform,
        ...
        audio_foundation_status: audioState,
