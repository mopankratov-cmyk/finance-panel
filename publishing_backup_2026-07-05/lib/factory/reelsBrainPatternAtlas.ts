...
function audioFoundationState(row: SegmentReadinessRow | undefined) {
  if (!row) return { status: "unknown", penalty: 0, note: "readiness не собран" };
  const audioRate = num(row.audio_rate);
  const transcriptRate = num(row.transcript_ready_rate);
  if (audioRate >= 55 && transcriptRate >= 45) {
    return { status: "ready", penalty: 0, note: "voice/audio foundation уже подтверждён" };
  }
  if (audioRate >= 30 && transcriptRate >= 20) {
    return { status: "warming", penalty: 6, note: "voice/audio foundation ещё в стадии прогрева" };
  }
  return { status: "weak", penalty: 16, note: "voice/audio foundation слишком тонкий для уверенного атласа" };
}
...
export function buildReelsBrainPatternAtlas(input: {
  ...
  const bySegment = (input.nicheSummaries || []).flatMap((nicheRow) =>
    platforms.map((platform) => {
      ...
      const audioFoundation = audioFoundationState(readiness);
      const mode = readinessThin
        ? "research_only"
        : audioFoundation.status === "weak"
          ? "research_only"
          : audioFoundation.status === "warming" && recommendationMode(text(nicheTrust.status), text(platformTrust.status)) === "primary"
            ? "control_only"
            : recommendationMode(text(nicheTrust.status), text(platformTrust.status));
      const baseStatus = atlasStatus(Math.max(0, avgStableScore - audioFoundation.penalty), stablePatterns.length, analyzed);
      const status = readinessThin
        ...
        : audioFoundation.status === "weak"
          ? "thin"
          : audioFoundation.status === "warming" && baseStatus === "stable"
            ? "forming"
        : baseStatus;
      ...
      return {
        ...
        audio_foundation_status: audioFoundation.status,
        audio_foundation_note: audioFoundation.note,
