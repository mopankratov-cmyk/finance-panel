...
test("buildReelsBrainPortfolioReadiness keeps audio-blocked segments out of generation-ready coverage", () => {
  const result = buildReelsBrainPortfolioReadiness({
    niches: ["ru_toys"],
    platforms: ["tiktok", "instagram"],
    segmentStabilityAudit: {
      items: [
        { niche: "ru_toys", platform: "tiktok", evidence_band: "stable", stability_score: 92, high_trust_segment: true, blockers: ["audio foundation missing"] },
        { niche: "ru_toys", platform: "instagram", evidence_band: "stable", stability_score: 89, high_trust_segment: true },
      ],
    },
    segmentSolutionMatrix: {
      by_segment: [
        { niche: "ru_toys", platform: "tiktok", publishable_exact: true, high_trust_generation_ready: true, trust_summary: { audio_foundation_status: "weak" } },
        { niche: "ru_toys", platform: "instagram", publishable_exact: true, high_trust_generation_ready: true, trust_summary: { audio_foundation_status: "ok" } },
      ],
    },
  });

  assert.equal(result.summary.generation_ready_segments, 1);
  assert.equal(result.summary.audio_backed_generation_ready_segments, 1);
  assert.equal(result.summary.generation_ready_coverage_pct, 50);
  assert.equal(result.summary.audio_backed_generation_ready_pct, 50);
  assert.equal(result.summary.weak_audio_segments, 1);
  assert.equal(result.summary.verdict, "still_building");
  assert.equal(result.by_platform[0]?.platform, "instagram");
  assert.equal(result.by_platform[1]?.weak_audio_segments, 1);
});
