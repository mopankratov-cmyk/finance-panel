...
function audioFoundationStatus(solution: JsonRecord | null | undefined, fallbackBlockers: unknown) {
  const trustSummary = (solution?.trust_summary && typeof solution.trust_summary === "object"
    ? solution.trust_summary
    : {}) as JsonRecord;
  const explicit = text(trustSummary.audio_foundation_status);
  if (explicit) return explicit;
  const blockers = list(trustSummary.blockers, 10);
  const fallback = list(fallbackBlockers, 10);
  const joined = [...blockers, ...fallback].join(" ").toLowerCase();
  return joined.includes("audio") || joined.includes("transcript") ? "weak" : "ok";
}
...
export function buildReelsBrainPortfolioReadiness(input: {
  ...
  const bySegment = niches.flatMap((niche) =>
    platforms.map((platform) => {
      ...
      const audioFoundation = audioFoundationStatus(solution, row.blockers);
      const generationReady = Boolean(solution.high_trust_generation_ready) && audioFoundation !== "weak";
      return {
        niche,
        platform,
        ...
        audio_foundation_status: audioFoundation,
  ...
  const byNiche = niches.map((niche) => {
    ...
    const audioBackedGenerationReady = items.filter((row) => row.high_trust_generation_ready && row.audio_foundation_status !== "weak").length;
    ...
    const weakAudio = items.filter((row) => row.audio_foundation_status === "weak").length;
    return {
      niche,
      stable,
      high_trust: highTrust,
      generation_ready: generationReady,
      audio_backed_generation_ready: audioBackedGenerationReady,
      forming,
      thin,
      missing,
      publishable_exact: publishableExact,
      ...
      audio_backed_generation_ready_pct: pct(audioBackedGenerationReady, items.length),
      publishable_exact_pct: pct(publishableExact, items.length),
      weak_outcome_segments: weakOutcome,
      weak_audio_segments: weakAudio,
      ...
      readiness: publishableExact === items.length
        ? "publishable_exact"
        : audioBackedGenerationReady === items.length && highTrust === items.length
          ? "covered"
  ...
  const audioBackedGenerationReady = bySegment.filter((row) => row.high_trust_generation_ready && row.audio_foundation_status !== "weak").length;
  ...
  const weakAudio = bySegment.filter((row) => row.audio_foundation_status === "weak").length;
  const coverageKnown = stable + forming + thin;
  const verdict = publishableExact === expectedTotal && expectedTotal > 0 && weakAudio === 0
    ? "ready_for_publishable_exact_generation"
    : highTrust === expectedTotal && publishableExact < expectedTotal
      ? "high_trust_but_exact_gaps"
    : audioBackedGenerationReady === expectedTotal && expectedTotal > 0
      ? "ready_for_high_trust_generation"
    : audioBackedGenerationReady >= Math.ceil(expectedTotal * 0.6) && publishableExact >= Math.ceil(expectedTotal * 0.35)
      ? "high_trust_but_exact_gaps"
    : audioBackedGenerationReady >= Math.ceil(expectedTotal * 0.6) && forming + stable >= Math.ceil(expectedTotal * 0.85)
  ...
  return {
    summary: {
      ...
      audio_backed_generation_ready_segments: audioBackedGenerationReady,
      ...
      audio_backed_generation_ready_pct: pct(audioBackedGenerationReady, expectedTotal),
