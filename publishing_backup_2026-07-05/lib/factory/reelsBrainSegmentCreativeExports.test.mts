...
function testBuildReelsBrainSegmentCreativeExportsSplitsShipAndValidateLanes() {
      ...
      items: [
        {
          ...
          memory_context: {
            ...
            memory_note: "Память под этот rollout пересобиралась под близкий output context.",
          },
  ...
  assert.equal(result.items[0]?.memory_context?.rebuild_alignment?.status, "aligned");
  assert.equal(result.summary.rebuild_alignment?.status, "aligned");
  assert.match(String(result.summary.memory_note), /близкий output context/i);
}
