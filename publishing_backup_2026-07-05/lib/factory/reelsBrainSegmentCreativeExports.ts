...
export function buildReelsBrainSegmentCreativeExports(input: {
    ...
    .map((row) => {
      ...
      return {
        ...
        memory_context: row.memory_context ? {
          rebuild_alignment: row.memory_context.rebuild_alignment ? {
            status: text(row.memory_context.rebuild_alignment.status, "unknown"),
            score: num(row.memory_context.rebuild_alignment.score),
            reasons: list(row.memory_context.rebuild_alignment.reasons, 4),
          } : null,
          rebuild_context: row.memory_context.rebuild_context ? {
            execution_mode: text(row.memory_context.rebuild_context.execution_mode),
            focus_platform: text(row.memory_context.rebuild_context.focus_platform),
          } : null,
          memory_note: text(row.memory_context.memory_note),
        } : null,
        trust: {
  ...
  return {
    summary: {
      ...
      rebuild_alignment: rows[0]?.memory_context?.rebuild_alignment || null,
      memory_note: text(rows[0]?.memory_context?.memory_note),
    },
