    ...
    .map((item) => {
      ...
      return {
        ...
        memory_context: item.memory_context ? {
          rebuild_alignment: item.memory_context.rebuild_alignment ? {
            status: text(item.memory_context.rebuild_alignment.status, "unknown"),
            score: num(item.memory_context.rebuild_alignment.score),
            reasons: list(item.memory_context.rebuild_alignment.reasons, 4),
          } : null,
          rebuild_context: item.memory_context.rebuild_context ? {
            execution_mode: text(item.memory_context.rebuild_context.execution_mode),
            focus_platform: text(item.memory_context.rebuild_context.focus_platform),
          } : null,
          memory_note: text(item.memory_context.memory_note),
        } : null,
