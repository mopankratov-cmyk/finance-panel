...
function audioFoundationState(row: SegmentReadinessRow | undefined) {
  ...
  if (audioRate >= 55 && transcriptRate >= 45) {
    return { status: "ready", penalty: 0, note: "voice/audio foundation уже достаточно плотный" };
  }
  if (audioRate >= 30 && transcriptRate >= 20) {
    return { status: "warming", penalty: 8, note: "voice/audio foundation ещё прогревается" };
  }
  return { status: "weak", penalty: 20, note: "voice/audio foundation пока слишком слабый" };
}
...
export function buildReelsBrainSegmentPriorityQueue(input: {
  ...
  const priorities = gapRows.map((row) => {
    ...
    ) && !marketBlocked && !readinessSoftBlocked && audioFoundation.status !== "weak";
    ...
    const urgencyBase = Math.round(
      ...
      + (audioFoundation.status === "ready" ? 10 : audioFoundation.status === "warming" ? 2 : -18)
      - (marketBlocked ? 36 : 0)
