...
test("buildReelsBrainSegmentSolutionMatrix groups solutions by niche and platform with trust-aware primary rows", () => {
  ...
  assert.equal(result.by_niche[0]?.audio_foundation_status, "ok");
...
test("buildReelsBrainSegmentSolutionMatrix does not treat audio-blocked segment as real generation-ready leader", () => {
  const result = buildReelsBrainSegmentSolutionMatrix({
    segmentSolutions: {
      items: [
        {
          ...
          trust_band: "high",
          production_state: "ready_now",
          creative_brief: { hook: "Hook A" },
          content_decision: { next_step: "Ship A" },
          trust_summary: {
            evidence_band: "stable",
            stability_score: 87,
            blockers: ["audio foundation ещё слабый для production-ready запуска"],
        ...
        },
        {
          ...
          trust_band: "medium",
          production_state: "ready_now",
          creative_brief: { hook: "Hook B" },
          content_decision: { next_step: "Ship B" },
          trust_summary: {
            evidence_band: "stable",
            stability_score: 72,
            blockers: [],
            proof_quality: "exact_segment",
            audio_foundation_status: "ok",
          },
        },
      ],
    },
    briefGapProgress: { top_candidates: [] },
    niches: ["ru_toys"],
