...
function generationReady(row: JsonRecord | null | undefined) {
  if (!row) return false;
  const trustSummary = (row.trust_summary && typeof row.trust_summary === "object" ? row.trust_summary : {}) as JsonRecord;
  const foundation = text(trustSummary.audio_foundation_status) || (list(trustSummary.blockers, 10).some((item) => item.toLowerCase().includes("audio") || item.toLowerCase().includes("transcript")) ? "weak" : "ok");
  return Boolean(row.high_trust_generation_ready) && foundation !== "weak";
}
...
function itemSort(a: JsonRecord, b: JsonRecord) {
  return policyModeRank(b.segment_priority_mode) - policyModeRank(a.segment_priority_mode)
    ...
    || Number(generationReady(b)) - Number(generationReady(a))
...
function summarizeGroup(groupKey: string, items: JsonRecord[], dimension: "niche" | "platform", gapCandidates: JsonRecord[]) {
  ...
  const generationReadyCount = sorted.filter((row) => generationReady(row)).length;
  ...
  return {
    ...
    audio_foundation_status: primary
      ? (
          text((primary.trust_summary as JsonRecord | null)?.audio_foundation_status)
          || (list((primary.trust_summary as JsonRecord | null)?.blockers, 10).some((item) => item.toLowerCase().includes("audio") || item.toLowerCase().includes("transcript")) ? "weak" : "ok")
        )
      : "unknown",
    high_trust_generation_ready: Boolean(primary && generationReady(primary)),
    ...
    generation_ready_segments: generationReadyCount,
...
export function buildReelsBrainSegmentSolutionMatrix(input: {
  ...
  return {
    summary: {
      ...
      generation_ready_segments: items.filter((row) => generationReady(row)).length,
