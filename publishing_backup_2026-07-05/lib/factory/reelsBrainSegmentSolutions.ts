...
function audioFoundationState(blockers: string[]) {
  const joined = blockers.join(" ").toLowerCase();
  const hasAudio = joined.includes("audio");
  const hasTranscript = joined.includes("transcript");
  if (hasAudio || hasTranscript) {
    return {
      status: "weak",
      note: "voice/audio foundation ещё не закрыт для production-ready сегмента",
    };
  }
  return {
    status: "ok",
    note: "явных audio/transcript blockers в readiness-аудите нет",
  };
}
...
export function buildReelsBrainSegmentSolutions(input: {
  ...
  const rows = records(input.decisionSnapshot?.items)
    .map((row) => {
      ...
      const blockers = list(stability.blockers, 4);
      const audioFoundation = audioFoundationState(blockers);
