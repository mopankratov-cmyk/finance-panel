...

{
  ...
  eq(trust.audio_support, null, "trust: audio support stays null when no audio evidence passed");
}
...

{
  const trust = buildPatternTrustSummary({
    ...
    generator_ready_patterns: new Array(4).fill(null).map((_, index) => ({ pattern_id: `r${index}` })),
    anti_patterns: [],
    top_hooks: [],
    quality_summary: { generator_ready: 4, needs_cleanup: 2, noise: 2, avg_relevance_score: 72 },
    generated_at: "2026-07-04T00:00:00.000Z",
  } as any, {
    platform: "tiktok",
    ready: true,
  ...
  eq(trust.audio_support?.status, "ready", "trust: strong audio evidence becomes ready");
  ok((trust.audio_support?.score || 0) >= 70, "trust: audio support scores high");
  ok(trust.why_ready.some((item) => item.includes("audio/transcript")), "trust: audio evidence is surfaced in why_ready");
}

{
  const nicheTrust = buildNicheTrustSummary({
    ...
    platforms: [
      { platform: "tiktok", trust: { score: 88, status: "ready", confidence: "high", generator_ready_patterns: 2, total_patterns: 4, anti_patterns: 1, high_risk_anti_patterns: 0, medium_risk_anti_patterns: 1, avg_relevance_score: 74, ready_pattern_rate: 50, risk_pressure: 9, audio_support: null, why_ready: [], why_not_yet: [], top_risks: [], note: "" } },
      { platform: "instagram", trust: { score: 54, status: "warming", confidence: "medium", generator_ready_patterns: 1, total_patterns: 3, anti_patterns: 1, high_risk_anti_patterns: 0, medium_risk_anti_patterns: 0, avg_relevance_score: 61, ready_pattern_rate: 33, risk_pressure: 4, audio_support: null, why_ready: [], why_not_yet: [], top_risks: [], note: "" } },
      { platform: "youtube", trust: { score: 20, status: "weak", confidence: "low", generator_ready_patterns: 0, total_patterns: 0, anti_patterns: 0, high_risk_anti_patterns: 0, medium_risk_anti_patterns: 0, avg_relevance_score: 0, ready_pattern_rate: 0, risk_pressure: 0, audio_support: null, why_ready: [], why_not_yet: [], top_risks: [], note: "" } },
    ],
  });
