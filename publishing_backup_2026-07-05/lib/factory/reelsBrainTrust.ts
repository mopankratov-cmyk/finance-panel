...
export interface ReelsBrainPatternTrust {
  ...
  audio_support: ReelsBrainAudioSupport | null;
  why_ready: string[];
  why_not_yet: string[];
  top_risks: string[];
  note: string;
...
export interface ReelsBrainAudioCoverageInput {
  corpus_videos?: number;
...
export interface ReelsBrainAudioSupport {
  score: number;
  status: "ready" | "warming" | "weak";
  confidence: "high" | "medium" | "low";
  analyzed_basis: number;
  with_audio: number;
...
export function buildAudioSupportSummary(input?: ReelsBrainAudioCoverageInput | null): ReelsBrainAudioSupport | null {
  if (!input) return null;

  const analyzedBasis = Math.max(
    1,
    num(input.analyzed_videos)
    || num(input.with_audio)
    || num(input.with_transcript)
    || num(input.corpus_videos),
  );
  const withAudio = num(input.with_audio);
  const withTranscript = num(input.with_transcript);
  const pauseReady = num(input.pause_map_ready);
  const pacingReady = num(input.pacing_ready);
  const beatReady = num(input.beat_hint_ready);

  const withAudioRate = pct(withAudio, analyzedBasis);
  const withTranscriptRate = pct(withTranscript, analyzedBasis);
  const pauseRate = pct(pauseReady, Math.max(1, withAudio));
  const pacingRate = pct(pacingReady, Math.max(1, withAudio));
  const beatRate = pct(beatReady, Math.max(1, withAudio));
  const featureCoverage = (pauseRate + pacingRate + beatRate) / 3;

  const rawScore =
    Math.min(38, (withAudio / analyzedBasis) * 38)
    + Math.min(36, (withTranscript / analyzedBasis) * 36)
    + Math.min(18, (featureCoverage / 100) * 18)
    + Math.min(8, Math.log(withAudio + 1) * 3.5);
  const score = clamp(rawScore);
  const status: ReelsBrainAudioSupport["status"] = score >= 72 && withAudio >= 12 && withTranscript >= 8
    ? "ready"
    : score >= 40 && withAudio >= 4
      ? "warming"
      : "weak";
  const confidence: ReelsBrainAudioSupport["confidence"] = withTranscript >= 20 && featureCoverage >= 45
    ? "high"
    : withAudio >= 6
      ? "medium"
      : "low";
  ...
  if (withAudioRate >= 45) whyReady.push(`audio-ready покрытие ${withAudioRate}%`);
  else whyNotYet.push(`audio-ready покрытие пока ${withAudioRate}%`);

  if (withTranscriptRate >= 35) whyReady.push(`voice/transcript слой уже ${withTranscriptRate}%`);
  else whyNotYet.push(`transcript покрытие пока ${withTranscriptRate}%`);

  if (pauseRate >= 25 || pacingRate >= 25 || beatRate >= 25) {
    whyReady.push(`feature-depth уже есть: pause ${pauseRate}% · pacing ${pacingRate}% · beat ${beatRate}%`);
  } else {
    whyNotYet.push(`мало deep-audio signals: pause ${pauseRate}% · pacing ${pacingRate}% · beat ${beatRate}%`);
  }

  const note = status === "ready"
    ? "Сегмент уже можно оценивать не только по визуалу, но и по voice/audio mechanics."
    : status === "warming"
      ? "Voice/audio слой уже полезен для контроля качества, но ещё не везде статистически плотный."
      : "Пока мало audio/transcript evidence: выводы по голосу и ритму ещё рано считать устойчивыми.";

  return {
    score,
    status,
    confidence,
    analyzed_basis: analyzedBasis,
    with_audio: withAudio,
    with_audio_rate: withAudioRate,
    with_transcript: withTranscript,
    with_transcript_rate: withTranscriptRate,
    feature_depth: {
      pause_map_ready: pauseReady,
      pause_map_ready_rate: pauseRate,
      pacing_ready: pacingReady,
      pacing_ready_rate: pacingRate,
      beat_hint_ready: beatReady,
      beat_hint_ready_rate: beatRate,
    },
    why_ready: whyReady,
...
export function buildPatternTrustSummary(
  ...
  audio?: ReelsBrainAudioCoverageInput | null,
  ...
  const audioSupport = buildAudioSupportSummary(audio);
  const audioScore = !audioSupport
    ? 0
    : audioSupport.status === "ready"
      ? 8
      : audioSupport.status === "warming"
        ? 3
        : analyzed >= Math.max(8, Math.floor(minAnalyzed * 0.5))
          ? -6
          : -2;
  const outcomeScore = outcome?.status === "proven"
    ...
    : outcome?.status === "promising"
      ...
      : outcome?.status === "weak"
        ? -8
        : 0;
  const rawScore = coverageScore + patternScore + readyScore + winnerScore + relevanceScore + outcomeScore + audioScore - Math.min(22, riskPressure * 0.45);
  const score = clamp(rawScore);
  ...
  const status: ReelsBrainPatternTrust["status"] = score >= 76 && readyState
    ...
    : score >= 48
      ? "warming"
      : "weak";
  let confidence: ReelsBrainPatternTrust["confidence"] = score >= 82 && generatorReady >= minReadyPatterns
    ? "high"
    : score >= 56
      ? "medium"
      : "low";
  if (audioSupport?.status === "weak" && confidence === "high") confidence = "medium";
  if (audioSupport?.status === "weak" && confidence === "medium" && analyzed >= minAnalyzed) confidence = "low";
  ...
  if (audioSupport?.status === "ready") whyReady.push(`audio/transcript слой подтверждён: ${audioSupport.with_transcript_rate}% transcript`);
  else if (audioSupport) whyNotYet.push(`audio/transcript слой ещё тонкий: ${audioSupport.with_audio_rate}% audio · ${audioSupport.with_transcript_rate}% transcript`);
...
export function buildNicheTrustSummary(input: {
  ...
  const base = buildPatternTrustSummary(input.meta_brain, input.readiness, input.gates, input.outcome, input.audio);
