-- WB РНП: единая цена продавца до СПП, только реальные продажи в выкупах
-- и постоянный бренд-контур NORVIA для Retail Family (ИП Филиппов).

update public.wb_cabinets
set brand_filters = array['norvia']::text[]
where marketplace = 'wb'
  and (
    inn = '330573647518'
    or lower(coalesce(name, '') || ' ' || coalesce(trade_mark, '')) like '%retail family%'
  )
  and brand_filters is distinct from array['norvia']::text[];
...
language sql stable as $$
  ...
  full join a on coalesce(o.d, s.d) = a.d
    and coalesce(o.nm_id, s.nm_id) = a.nm_id;
$$;
...
o as (
  select
    ...
    coalesce(sum(coalesce(w.total_price, 0) * (1 - coalesce(w.discount_percent, 0) / 100.0)) filter (where w.date >= b.today_start), 0) as s_today,
    count(*) filter (where w.date >= b.yesterday_start and w.date < b.today_start)::int as c_yesterday,
    coalesce(sum(coalesce(w.total_price, 0) * (1 - coalesce(w.discount_percent, 0) / 100.0)) filter (where w.date >= b.yesterday_start and w.date < b.today_start), 0) as s_yesterday,
    count(*) filter (where w.date >= b.week_start)::int as c_week,
    coalesce(sum(coalesce(w.total_price, 0) * (1 - coalesce(w.discount_percent, 0) / 100.0)) filter (where w.date >= b.week_start), 0) as s_week,
    count(*)::int as c_month,
    coalesce(sum(coalesce(w.total_price, 0) * (1 - coalesce(w.discount_percent, 0) / 100.0)), 0) as s_month
...
left join meta on meta.nm_id = coalesce(o.nm_id, s.nm_id, st.nm_id)
left join public.product_costs pc on pc.article = coalesce(nullif(o.article, ''), meta.article)
left join ad on ad.nm_id = coalesce(o.nm_id, s.nm_id, st.nm_id);
$$;
