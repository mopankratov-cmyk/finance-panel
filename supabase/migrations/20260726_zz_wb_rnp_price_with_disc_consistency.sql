-- WB RNP consistency after SPP sync.
--
-- 20260726_wb_spp_sync.sql starts persisting wb_orders.price_with_disc.
-- RNP SQL aggregates must prefer that stored WB value instead of recalculating
-- total_price * (1 - discount_percent/100), otherwise scoped dashboards can
-- drift from the seller cabinet on rounding/API nuances.
--
-- Also make rnp_report "today/yesterday/week/month" boundaries Moscow-based:
-- user-facing WB analytics is read in MSK, while Supabase now() is UTC.
