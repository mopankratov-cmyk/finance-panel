-- Брак при приёмке и ручное списание.
--
-- Флоу владельца: брак сообщает фулфилмент в момент приёмки — «принято 100, из них
-- 3 брак». Поэтому брак — поле строки приёмки, а не отдельный процесс: проведение
-- приходует всё принятое и тут же списывает брак с той же себестоимостью. Регистр
-- честен: пришло 100, списано 3, остаток 97 — и деньги брака видны отдельной строкой.
--
-- Ручное списание — для потерь, всплывающих позже приёмки: порча, недостача.
-- Устроено как отгрузка: проверка остатка до записи, списание по средневзвешенной,
-- в минус уйти нельзя.

alter table public.purchase_receipts
  add column if not exists defect_qty integer not null default 0 check (defect_qty >= 0);

-- ---------------------------------------------------------------------------
-- Проведение приёмки: брак списывается тем же документом
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Третий проход: приход всего принятого + списание брака той же себестоимостью.
  for v_row in
...
-- Ручное списание
-- ---------------------------------------------------------------------------
-- Как отгрузка, только без кабинета и с обязательной причиной: проверка всего
-- документа до записи, списание по средневзвешенной, в минус уйти нельзя.
...
begin
  ...
  for v_line in
    select (item->>'productId')::uuid as product_id, sum((item->>'qty')::integer) as qty
  ...
  for v_line in
    select (item->>'productId')::uuid as product_id, (item->>'qty')::integer as qty
    from jsonb_array_elements(p_lines) as item
  loop
    select * into v_product from public.products where id = v_line.product_id;
    ...
    insert into public.stock_moves (
      ...
      p_legal_entity_id, null, p_warehouse_id, v_line.product_id, v_product.nm_id, v_product.article,
