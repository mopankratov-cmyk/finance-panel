-- Переход состояния CTR-теста с учётом автоматической ротации.
--
-- Отличие от 202609010001 ровно одно: вместо безусловного запрета
-- `live swap must remain disabled` вызывается ctr_auto_gate — он пускает
-- автоматику к тестам с включённой сменой и НЕ пускает к ним человека.
-- Остальное тело перенесено дословно, чтобы правила финала, порога знаменателя
-- и потолка расхода не разъехались с предыдущей версией.
--
-- Одна plpgsql-функция на файл: редактор Supabase молча не применяет файл,
-- где их несколько.
...
begin
  ...
  -- Хозяин ротации у теста ровно один: либо человек, либо крон. Разворот
  -- запрета в обе стороны не даёт им переставлять фото навстречу друг другу.
  perform public.ctr_auto_gate(v_test.id, coalesce((p_input->>'auto')::boolean, false));
