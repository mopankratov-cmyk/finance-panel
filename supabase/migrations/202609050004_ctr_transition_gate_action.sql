-- Переход состояния CTR-теста: гейт теперь знает, КАКОЕ действие проверяет.
--
-- Отличие от 202609050002 ровно одно: в ctr_auto_gate передаётся v_action.
-- Без него запрет накрывал весь тест, и на экране автоматика отбирала у
-- человека даже кнопку «Отменить» — см. 202609050003.
--
-- Тело перенесено дословно, чтобы правила финала, порога знаменателя и
-- потолка расхода не разъехались с предыдущей версией.
--
...
begin
  ...
  perform public.ctr_auto_gate(v_test.id, coalesce((p_input->>'auto')::boolean, false), v_action);
