...
comment on table public.access_audit_log is
  'Журнал действий по ТЗ о правах §17: входы, учётные записи и роли, зарплата, себестоимость, цены, рекламные бюджеты, поставки, финансовые отчёты и классификация, складские операции, выгрузки. Права на удаление отзываются отдельной миграцией 202609100003 — одного grant для этого недостаточно.';
...
-- Пишет только сервер.
--
-- ВНИМАНИЕ: одного этого grant НЕДОСТАТОЧНО, чтобы запретить удаление, и
-- первая версия комментария здесь обещала обратное. GRANT только добавляет
-- права, а Supabase раздаёт service_role полный набор на новые таблицы схемы
-- public через default privileges — живая проверка показала, что сервисная
-- роль спокойно удаляла и переписывала записи. Явный отзыв делает следующая
-- миграция, 202609100003_access_audit_log_immutable.sql.
