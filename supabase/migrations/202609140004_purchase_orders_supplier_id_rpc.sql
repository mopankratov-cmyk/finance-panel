-- Заказ фабрике запоминает ссылку на suppliers, не только текст.
--
-- Колонка purchase_orders.supplier_id уже добавлена (202609140003_suppliers.sql),
-- но save_purchase_order её не писал — только текстовое поле supplier. Сама
-- по себе колонка ничего не значила: заказ, сохранённый из формы, приходил
-- с полем supplierId, которое функция молча теряла.
--
-- Только функция, без DDL — по тому же правилу, что развело схему и
-- процедуры в 202609040002/0003: редактор Supabase путает DDL и тело
-- процедуры в одном файле.
