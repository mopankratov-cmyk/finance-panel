-- История заказа: правка цены товара или платёжного графика переставала
-- быть видна — audit_log уже писал before/after заказа, но только его
-- собственные колонки. Позиции и графики удалялись и вставлялись заново
-- НИЖЕ по той же функции, и их прежнее состояние никуда не снималось —
-- «редакция» технически стиралась в момент правки, а не запоминалась.
--
-- Фикс снимает срез items/paymentStages/logisticsStages/expenses В v_before
-- ДО того, как их удалит следующий блок функции — так же, как v_before уже
-- снимает срез самого заказа до update. Новой таблицы не заводим: то же
-- назначение полей (было → стало) уже есть в operation_audit_log, разница
-- была только в полноте снимка.
--
-- Только функция, без DDL — тот же файл-на-функцию, что и 202609140004.
...
begin
  ...
  -- Снимок дочерних строк — только когда заказ уже существовал (иначе
  -- снимать нечего, v_before и так null и в историю уйдёт action='created').
  if v_before is not null then
    v_before := v_before || jsonb_build_object(
      'items', coalesce((
        select jsonb_agg(jsonb_build_object(
          'nmId', i.nm_id, 'article', i.article, 'name', i.name,
          'quantity', i.quantity, 'unitPrice', i.unit_price
        ) order by i.article, i.nm_id)
        from public.purchase_order_items i where i.order_id = v_id
      ), '[]'::jsonb),
      'paymentStages', coalesce((
        select jsonb_agg(jsonb_build_object(
          'title', s.title, 'percent', s.percent, 'amount', s.amount,
          'dueDate', s.due_date, 'paidAt', s.paid_at, 'status', s.status
        ) order by s.position)
        from public.purchase_payment_stages s where s.order_id = v_id
      ), '[]'::jsonb),
      'logisticsStages', coalesce((
        select jsonb_agg(jsonb_build_object(
          'title', l.title, 'provider', l.provider, 'dueDate', l.due_date,
          'completedAt', l.completed_at, 'cost', l.cost, 'status', l.status
        ) order by l.position)
        from public.purchase_logistics_stages l where l.order_id = v_id
      ), '[]'::jsonb),
      'expenses', coalesce((
        select jsonb_agg(jsonb_build_object(
          'title', e.title, 'amount', e.amount, 'currency', e.currency
        ) order by e.position)
        from public.purchase_expenses e where e.order_id = v_id
      ), '[]'::jsonb)
    );
  end if;
