-- post_receipt_batch: проставляет supplier_id/country партии из заказа.
-- v_order объявлена как public.purchase_orders%rowtype — Postgres обновляет
-- композитный тип таблицы при каждом вызове функции, поэтому v_order.supplier_id
-- уже доступен сам по себе, без правки declare. country берёт отдельным
-- скалярным подзапросом к suppliers — сама функция их раньше не читала.
--
-- Тело функции без изменений, кроме одной строки INSERT INTO stock_batches
-- (добавлены supplier_id, country) — сверено построчно с действующей версией
-- в 202608230015_variant_functions.sql.
...
begin
  ...
  insert into public.stock_batches (
    id, legal_entity_id, receipt_batch_id, order_id, supplier, supplier_id, country,
    goods_amount, logistics_amount, extra_amount, total_amount, total_qty,
    cost_basis, cost_note, posted_by
  ) values (
    v_batch_id, v_entity_id, p_batch_id, v_order.id, coalesce(v_order.supplier, ''), v_order.supplier_id,
    (select s.country from public.suppliers s where s.id = v_order.supplier_id),
