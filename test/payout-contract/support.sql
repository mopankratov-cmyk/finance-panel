...
create table if not exists test_support.barriers (
  barrier_key text primary key,
  target integer not null check (target > 0),
  arrived integer not null default 0
);

create or replace function test_support.reset_barrier(p_key text, p_target integer)
returns void language plpgsql set search_path = ''
as $$
begin
  insert into test_support.barriers(barrier_key,target,arrived)
  values(p_key,p_target,0)
  on conflict(barrier_key) do update set target=excluded.target,arrived=0;
end
$$;

create or replace function test_support.wait_barrier(p_key text)
returns void language plpgsql set search_path = ''
as $$
declare v_target integer; v_arrived integer; v_until timestamptz;
begin
  update test_support.barriers set arrived=arrived+1 where barrier_key=p_key;
  v_until:=pg_catalog.clock_timestamp()+interval '5 seconds';
  loop
    select target,arrived into v_target,v_arrived
      from test_support.barriers where barrier_key=p_key;
    exit when v_arrived>=v_target;
    if pg_catalog.clock_timestamp()>v_until then
      raise exception using errcode='P0001',message='TEST_BARRIER_TIMEOUT';
    end if;
...
begin
  if pg_catalog.current_setting('application_name',true)='fault-second-payment'
    and (select pg_catalog.count(*) from public.payments
      where comment='[marketplace-payout-v2]')>=1 then
...
grant delete on table public.app_users, public.wb_cabinets, public.companies,
  public.accounts, public.payments, public.marketplace_payout_routes,
  public.marketplace_payout_series, public.marketplace_payout_forecast_revisions,
...
begin
  ...
  delete from public.payments where id::text like '95%' or comment='[marketplace-payout-v2]';
  ...
  delete from test_support.barriers;
end
$$;
...
begin
  ...
  values('97000000-0000-4000-8000-000000000001','wb',
    ...
    '9714053621','507401001','ООО «РВБ»','shared',true,
    '91000000-0000-4000-8000-000000000002');
end
$$;

alter function test_support.clean() owner to payout_rpc_owner;
alter function test_support.install_fixtures() owner to payout_rpc_owner;
grant usage on schema test_support to payout_rpc_owner, payout_rpc_executor;
grant execute on all functions in schema test_support to payout_rpc_executor;
