...
test("кабинеты заведённого сотрудника выбираются отметками", () => {
  const page = read("app/users/page.tsx");
  const list = read("app/api/users/route.ts");
  // Раньше `cabinet_ids` уходил на сервер ТОЛЬКО при заведении, а панель
  // показывала лишь уже выданные кабинеты: набор замерзал в момент создания
  // учётки — ни добавить, ни убрать.
  assert.match(page, /patch\(u\.id, \{ cabinet_ids: next \}\)/, "кабинеты существующего сотрудника не сохраняются");
  assert.match(page, /type="checkbox"/, "кабинеты выбираются не отметками");
  assert.match(list, /scope_cabinet_ids: scope/, "сервер не отдаёт область выбора");
  // Кнопка показывалась только тому, у кого кабинет уже был, — человеку без
  // кабинетов выдать их было нечем вовсе.
  assert.match(page, /rolesOf\(u\)\.some\(isCabinetScopedRole\) \? \(/, "панель кабинетов скрыта от тех, у кого их пока нет");
});
