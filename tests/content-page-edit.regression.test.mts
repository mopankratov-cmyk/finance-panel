...
test("перечитывание после правки не прячет список", () => {
  const page = read("components/wb/WbContentPage.tsx");
  // Полноэкранный баннер уместен только на первой загрузке. После каждого
  // добавления и удаления он убирал бы с глаз ровно то, ради чего человек
  // открыл экран, — и выглядел бы как сброс страницы.
  assert.match(page, /loading && !data \?/);
  assert.match(page, /reloadKey/, "после правки каталог перечитывается");
});
