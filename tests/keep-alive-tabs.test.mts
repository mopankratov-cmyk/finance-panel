...
test("своё фото можно загрузить и убрать", () => {
  ...
  // Принадлежность проверяется настоящим URL и сравнением pathname по границе
  // сегмента — не подстрочным target.includes(...), который пропускал чужую
  // ссылку, если нужная подстрока просто где-то встречалась в строке.
  assert.match(route, /pathname = new URL\(target\)\.pathname/, "путь разбирается как настоящий URL, а не строкой");
  assert.match(route, /pathname\.startsWith\(ownPrefix\)/, "сравнение по границе сегмента пути, а не по вхождению подстроки");
  assert.doesNotMatch(route, /target\.includes\(/, "подстрочной проверки владения быть не должно");
  ...
  // Хранилище трогаем только когда каталог реально что-то удалил: иначе урл,
  // которого нет в каталоге, мог снести чужой объект по случайному совпадению
  // пути.
  assert.match(route, /\.delete\(\)\.eq\("url", target\)\.select\("id"\)/, "delete() должен вернуть удалённые строки, а не просто отсутствие ошибки");
  assert.match(route, /removed\.data\.length === 0/, "storage\\.remove\\(\\) не должен звать, если каталог ничего не удалил");
  // IDOR: nmId из тела запроса POST обязан реально стоять в карточках ИМЕННО
