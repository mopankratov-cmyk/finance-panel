...
test("шапка колонок «Полок» прилипает под верхней панелью", async () => {
  const page = await read("../components/wb/WbShelfPage.tsx");
  // 54px — высота фиксированной панели WbShell, плюс вырез экрана: в
  // горизонтальной ориентации iPhone панель опускается на его высоту, и шапка
  // без этой добавки уезжает под неё.
  assert.match(page, /sticky top-\[calc\(54px\+var\(--safe-t\)\)\] z-20/);
  // Фон обязателен и непрозрачен: сквозь полупрозрачный читались цифры
  // уезжающих под шапку карточек.
  assert.match(page, /bg-\[#f6f7f9\] px-10/);
