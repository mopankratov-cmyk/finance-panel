...
test("AI agent uses its own concise sidebar", () => {
  for (const path of ["/agent", "/agent/history"]) {
    assert.equal(isAgentSidebarPath(path), true, path);
  }

  for (const path of ["/", "/wb/rnp", "/ozon", "/summary", "/cabinets", "/agent-old"]) {
    assert.equal(isAgentSidebarPath(path), false, path);
  }
});
