...
test("внешний селлер переходит между своими модулями, не упираясь в отказ", () => {
  ...
  // Адрес выхода считается один раз и берётся обеими ветками — сайдбаром и
  // мобильной шапкой. Раньше вторая вела на «/» жёстко, и селлер с телефона
  // упирался в отказ ровно там, где на широком экране всё работало.
  assert.match(shell, /const exitHref = isSeller \? "\/wb\/rnp" : "\/"/, "из склада селлера уводит на закрытую витрину");
  assert.equal((shell.match(/href=\{exitHref\}/g) ?? []).length, 2, "адрес выхода взят не во всех ветках");
});
