...
test("UI distinguishes campaign spend from comparable SKU spend and preserves DRR severity", () => {
  ...
  // Формат числа менялся (₽ в середине строки съедал ширину у артикула) —
  // сторожу важно, что расход за закрытую неделю в строке ЕСТЬ.
  assert.match(page, /7д \{(?:rub|int)\(campaign\.spent_7_closed\)\}/, "расход кампании за закрытую неделю");
  ...
  // 05.09.2026 отдельная строка «по артикулу» свёрнута в пометку у числа за
  // семь дней: строка растила высоту, а список наезжал сам на себя. Все три
  // состояния сохранены — они переехали в подсказку строки, а не пропали.
  assert.match(page, /По этому артикулу тратит не только эта кампания: \$\{rub\(article\.spent_sku_7_closed\)\}/);
  assert.match(page, /Math\.round\(article\.spent_sku_7_closed\) !== Math\.round\(campaign\.spent_7_closed\)/);
  assert.match(page, /Разбивка расхода по артикулам за период не собрана/, "нет данных отличается от нуля");
  assert.match(page, /Весь расход по этому артикулу за 7 дней — из этой кампании/, "совпадение — тоже ответ");
