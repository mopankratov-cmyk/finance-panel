...
test("WB navigation exposes only the approved dashboards as direct links", () => {
  assert.deepEqual(WB_NAVIGATION_ITEMS.map((item) => item.href), [
    ...
    "/wb/funnel",
    "/wb/adverts",
    // Управление рекламой — отдельный пункт, а не вкладка внутри «Рекламы».
    // Тот экран показывает, этот меняет и тратит деньги; смешивать чтение и
    // запись в одном пункте значит приводить человека к кнопкам с деньгами
    // тогда, когда он шёл просто посмотреть цифры.
    "/wb/ads",
    "/wb/rk",
    "/wb/supplies",
    "/wb/unit",
