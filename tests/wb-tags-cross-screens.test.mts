...
test("меню ярлыков не проваливает клики в таблицу", () => {
  ...
  // Слушать прокрутку надо в фазе перехвата (третий аргумент true): уезжает
  // не окно, а контейнер таблицы, и всплывающего события от него не будет.
  //
  // Закрывается меню не на любом сдвиге, а на заметном: пальцем почти всякое
  // касание даёт микропрокрутку, и меню захлопывалось ровно в момент выбора
  // ярлыка. До порога оно переставляется за уехавшей кнопкой.
  assert.match(hook, /getBoundingClientRect\(\)/);
  assert.match(hook, /addEventListener\("scroll", \w+, true\)/);
  assert.match(hook, /setOpen\(false\)/);
});
