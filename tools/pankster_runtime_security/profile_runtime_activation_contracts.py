"""Disabled-by-default profile runtime activation contracts for Phase 1E.

This module prepares a secret-free profile runtime activation manifest from
...
state, activate profiles, start runtime processes, start subprocesses, create
...
REQUIRED_PROFILE_RUNTIME_ACTIVATION_CAPABILITIES = frozenset(
    {
        "profile_worker_binding_ready",
...
class ProfileRuntimeActivationIdentity:
    activation_name: str
...
@dataclass(frozen=True)
class ProfileRuntimeActivationConfig:
    """Disabled-by-default profile runtime activation controls."""
...
@dataclass(frozen=True)
class ProfileRuntimeActivationRequest:
    activation_identity: ProfileRuntimeActivationIdentity
    profile_worker_binding_request: ProfileWorkerBindingRequest
    ...
    profile_runtime_activation_policy_id: str


@dataclass(frozen=True)
class ProfileRuntimeActivationDecision:
    allowed: bool
    reason: str
    profile_worker_binding_decision: ProfileWorkerBindingDecision | None = None
    profile_runtime_activation_manifest: Mapping[str, str] | None = None
...
def prepare_profile_runtime_activation(
    *,
    request: ProfileRuntimeActivationRequest,
    config: ProfileRuntimeActivationConfig | None = None,
    audit_sink: AuditSinkState,
    broker_available: bool,
) -> ProfileRuntimeActivationDecision:
    """Prepare profile runtime activation manifest without activating profile."""

    resolved_config = config or ProfileRuntimeActivationConfig()
    config_reason = _config_denial_reason(resolved_config)
    if config_reason is not None:
        return _deny(config_reason)

    identity_reason = _activation_identity_reason(request.activation_identity)
    ...
    worker_decision = prepare_profile_worker_binding(
        request=request.profile_worker_binding_request,
        config=ProfileWorkerBindingConfig(
    ...
    if worker_decision.reason != "PROFILE_WORKER_BINDING_CONTRACT_READY_NO_WORKER_BOUND":
        return ProfileRuntimeActivationDecision(False, worker_decision.reason, worker_decision)

    worker_manifest = worker_decision.profile_worker_binding_manifest or {}
    activation_manifest = {
        "activation_name": request.activation_identity.activation_name,
        "activation_version": request.activation_identity.activation_version,
        "activation_contract_version": request.activation_identity.activation_contract_version,
        "worker_binding_name": worker_manifest["worker_binding_name"],
        "gateway_binding_name": worker_manifest["gateway_binding_name"],
        "wiring_name": worker_manifest["wiring_name"],
        "executor_name": worker_manifest["executor_name"],
        "host_adapter_name": worker_manifest["host_adapter_name"],
        "adapter_name": worker_manifest["adapter_name"],
        ...
        "profile_runtime_activation_policy_id": request.profile_runtime_activation_policy_id,
        "profile_runtime_activation_state": "disabled_contract_ready_no_profile_activated",
    }
    scan = scan_secret_shapes(activation_manifest)
    if not scan.allowed:
        return ProfileRuntimeActivationDecision(False, "PROFILE_RUNTIME_ACTIVATION_MANIFEST_SECRET_SCAN_FAILED", worker_decision)
    return ProfileRuntimeActivationDecision(
        allowed=False,
        reason="PROFILE_RUNTIME_ACTIVATION_CONTRACT_READY_NO_PROFILE_ACTIVATED",
        profile_worker_binding_decision=worker_decision,
        profile_runtime_activation_manifest=activation_manifest,
    )


def _config_denial_reason(config: ProfileRuntimeActivationConfig) -> str | None:
    if not config.profile_runtime_activation_contract_enabled:
        return "PROFILE_RUNTIME_ACTIVATION_DISABLED"
...
def _activation_identity_reason(identity: ProfileRuntimeActivationIdentity) -> str | None:
    for field, value in (
        ("activation_name", identity.activation_name),
        ("activation_version", identity.activation_version),
        ("activation_contract_version", identity.activation_contract_version),
    ):
        if not isinstance(value, str) or not value.strip():
            return f"PROFILE_RUNTIME_ACTIVATION_IDENTITY_FIELD_MISSING:{field}"
    missing = sorted(REQUIRED_PROFILE_RUNTIME_ACTIVATION_CAPABILITIES - set(identity.capabilities))
    if missing:
        return f"PROFILE_RUNTIME_ACTIVATION_CAPABILITY_MISSING:{missing[0]}"
    return None


def _request_reason(request: ProfileRuntimeActivationRequest) -> str | None:
    worker_request = request.profile_worker_binding_request
    if request.expected_profile_id != worker_request.expected_profile_id:
        return "EXPECTED_PROFILE_MISMATCH"
    if request.expected_policy_version != worker_request.expected_policy_version:
        return "EXPECTED_POLICY_VERSION_MISMATCH"
    if request.expected_runtime_backend != worker_request.expected_runtime_backend:
        return "EXPECTED_RUNTIME_BACKEND_MISMATCH"
    if request.expected_rollback_policy_id != worker_request.expected_rollback_policy_id:
        return "EXPECTED_ROLLBACK_POLICY_MISMATCH"
    if request.expected_wiring_policy_id != worker_request.expected_wiring_policy_id:
        return "EXPECTED_WIRING_POLICY_MISMATCH"
    if request.expected_gateway_binding_policy_id != worker_request.expected_gateway_binding_policy_id:
        return "EXPECTED_GATEWAY_BINDING_POLICY_MISMATCH"
    if request.expected_profile_worker_binding_policy_id != worker_request.profile_worker_binding_policy_id:
        return "EXPECTED_PROFILE_WORKER_BINDING_POLICY_MISMATCH"
    if not isinstance(request.profile_runtime_activation_policy_id, str) or not request.profile_runtime_activation_policy_id.strip():
        return "PROFILE_RUNTIME_ACTIVATION_POLICY_MISSING"
    return None


def _deny(reason: str) -> ProfileRuntimeActivationDecision:
    return ProfileRuntimeActivationDecision(False, reason)
