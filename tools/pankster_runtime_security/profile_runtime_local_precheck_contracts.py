"""Disabled-by-default profile runtime local precheck contracts.

This module prepares a secret-free local precheck manifest from local contract
objects only. It does not import Hermes runtime code, import or edit gateway.py,
import or edit web_server.py, mutate profile worker runtime state, perform a
local precheck, perform synthetic dry-runs, perform synthetic invocation, invoke
profiles, execute profile activation, start runtime processes, start
subprocesses, create sandboxes, call provider/model APIs, read process
...
REQUIRED_PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITIES = frozenset(
    {
        "profile_runtime_synthetic_dry_run_contract_ready",
...
class ProfileRuntimeLocalPrecheckIdentity:
    local_precheck_name: str
    local_precheck_version: str
    local_precheck_contract_version: str
    capabilities: tuple[str, ...]


@dataclass(frozen=True)
class ProfileRuntimeLocalPrecheckConfig:
    """Disabled-by-default profile runtime local precheck controls."""
...
@dataclass(frozen=True)
class ProfileRuntimeLocalPrecheckRequest:
    local_precheck_identity: ProfileRuntimeLocalPrecheckIdentity
    profile_runtime_synthetic_dry_run_request: ProfileRuntimeSyntheticDryRunRequest
    ...
    profile_runtime_local_precheck_policy_id: str


@dataclass(frozen=True)
class ProfileRuntimeLocalPrecheckDecision:
    allowed: bool
    reason: str
    profile_runtime_synthetic_dry_run_decision: ProfileRuntimeSyntheticDryRunDecision | None = None
    profile_runtime_local_precheck_manifest: Mapping[str, str] | None = None
...
def prepare_profile_runtime_local_precheck(
    *,
    request: ProfileRuntimeLocalPrecheckRequest,
    config: ProfileRuntimeLocalPrecheckConfig | None = None,
    audit_sink: AuditSinkState,
    broker_available: bool,
) -> ProfileRuntimeLocalPrecheckDecision:
    """Prepare local precheck manifest without performing local precheck."""

    resolved_config = config or ProfileRuntimeLocalPrecheckConfig()
    config_reason = _config_denial_reason(resolved_config)
    if config_reason is not None:
        return _deny(config_reason)

    identity_reason = _local_precheck_identity_reason(request.local_precheck_identity)
    ...
    dry_run_decision = prepare_profile_runtime_synthetic_dry_run(
        request=request.profile_runtime_synthetic_dry_run_request,
        config=ProfileRuntimeSyntheticDryRunConfig(
    ...
    if dry_run_decision.reason != "PROFILE_RUNTIME_SYNTHETIC_DRY_RUN_CONTRACT_READY_NO_SYNTHETIC_DRY_RUN_PERFORMED":
        return ProfileRuntimeLocalPrecheckDecision(False, dry_run_decision.reason, dry_run_decision)

    dry_run_manifest = dry_run_decision.profile_runtime_synthetic_dry_run_manifest or {}
    precheck_manifest = {
        "local_precheck_name": request.local_precheck_identity.local_precheck_name,
        "local_precheck_version": request.local_precheck_identity.local_precheck_version,
        "local_precheck_contract_version": request.local_precheck_identity.local_precheck_contract_version,
        "synthetic_dry_run_name": dry_run_manifest["synthetic_dry_run_name"],
        "synthetic_invocation_name": dry_run_manifest["synthetic_invocation_name"],
        "invocation_name": dry_run_manifest["invocation_name"],
        "activation_execution_name": dry_run_manifest["activation_execution_name"],
        "activation_name": dry_run_manifest["activation_name"],
        "worker_binding_name": dry_run_manifest["worker_binding_name"],
        "gateway_binding_name": dry_run_manifest["gateway_binding_name"],
        "wiring_name": dry_run_manifest["wiring_name"],
        "executor_name": dry_run_manifest["executor_name"],
        "host_adapter_name": dry_run_manifest["host_adapter_name"],
        "adapter_name": dry_run_manifest["adapter_name"],
        ...
        "profile_runtime_local_precheck_policy_id": request.profile_runtime_local_precheck_policy_id,
        "profile_runtime_local_precheck_state": "disabled_contract_ready_no_local_precheck_performed",
    }
    scan = scan_secret_shapes(precheck_manifest)
    if not scan.allowed:
        return ProfileRuntimeLocalPrecheckDecision(
            False,
            "PROFILE_RUNTIME_LOCAL_PRECHECK_MANIFEST_SECRET_SCAN_FAILED",
            dry_run_decision,
        )
    return ProfileRuntimeLocalPrecheckDecision(
        allowed=False,
        reason="PROFILE_RUNTIME_LOCAL_PRECHECK_CONTRACT_READY_NO_LOCAL_PRECHECK_PERFORMED",
        profile_runtime_synthetic_dry_run_decision=dry_run_decision,
        profile_runtime_local_precheck_manifest=precheck_manifest,
    )


def _config_denial_reason(config: ProfileRuntimeLocalPrecheckConfig) -> str | None:
    if not config.profile_runtime_local_precheck_contract_enabled:
        return "PROFILE_RUNTIME_LOCAL_PRECHECK_DISABLED"
...
def _local_precheck_identity_reason(identity: ProfileRuntimeLocalPrecheckIdentity) -> str | None:
    for field, value in (
        ("local_precheck_name", identity.local_precheck_name),
        ("local_precheck_version", identity.local_precheck_version),
        ("local_precheck_contract_version", identity.local_precheck_contract_version),
    ):
        if not isinstance(value, str) or not value.strip():
            return f"PROFILE_RUNTIME_LOCAL_PRECHECK_IDENTITY_FIELD_MISSING:{field}"
    missing = sorted(REQUIRED_PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITIES - set(identity.capabilities))
    if missing:
        return f"PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITY_MISSING:{missing[0]}"
    return None


def _request_reason(request: ProfileRuntimeLocalPrecheckRequest) -> str | None:
    dry_run_request = request.profile_runtime_synthetic_dry_run_request
    if request.expected_profile_id != dry_run_request.expected_profile_id:
        return "EXPECTED_PROFILE_MISMATCH"
    if request.expected_policy_version != dry_run_request.expected_policy_version:
        return "EXPECTED_POLICY_VERSION_MISMATCH"
    if request.expected_runtime_backend != dry_run_request.expected_runtime_backend:
        return "EXPECTED_RUNTIME_BACKEND_MISMATCH"
    if request.expected_rollback_policy_id != dry_run_request.expected_rollback_policy_id:
        return "EXPECTED_ROLLBACK_POLICY_MISMATCH"
    if request.expected_wiring_policy_id != dry_run_request.expected_wiring_policy_id:
        return "EXPECTED_WIRING_POLICY_MISMATCH"
    if request.expected_gateway_binding_policy_id != dry_run_request.expected_gateway_binding_policy_id:
        return "EXPECTED_GATEWAY_BINDING_POLICY_MISMATCH"
    if request.expected_profile_worker_binding_policy_id != dry_run_request.expected_profile_worker_binding_policy_id:
        return "EXPECTED_PROFILE_WORKER_BINDING_POLICY_MISMATCH"
    if request.expected_profile_runtime_activation_policy_id != dry_run_request.expected_profile_runtime_activation_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_ACTIVATION_POLICY_MISMATCH"
    if request.expected_profile_runtime_activation_execution_policy_id != dry_run_request.expected_profile_runtime_activation_execution_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_ACTIVATION_EXECUTION_POLICY_MISMATCH"
    if request.expected_profile_runtime_invocation_policy_id != dry_run_request.expected_profile_runtime_invocation_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_INVOCATION_POLICY_MISMATCH"
    if request.expected_profile_runtime_synthetic_invocation_policy_id != dry_run_request.expected_profile_runtime_synthetic_invocation_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_SYNTHETIC_INVOCATION_POLICY_MISMATCH"
    if request.expected_profile_runtime_synthetic_dry_run_policy_id != dry_run_request.profile_runtime_synthetic_dry_run_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_SYNTHETIC_DRY_RUN_POLICY_MISMATCH"
    if not isinstance(request.profile_runtime_local_precheck_policy_id, str) or not request.profile_runtime_local_precheck_policy_id.strip():
        return "PROFILE_RUNTIME_LOCAL_PRECHECK_POLICY_MISSING"
    return None


def _deny(reason: str) -> ProfileRuntimeLocalPrecheckDecision:
    return ProfileRuntimeLocalPrecheckDecision(False, reason)
