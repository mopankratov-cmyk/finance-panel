"""Disabled-by-default profile runtime readiness gate contracts.

This module prepares a secret-free readiness gate manifest from local contract
objects only. It does not import Hermes runtime code, import or edit gateway.py,
import or edit web_server.py, mutate profile worker runtime state, open a
readiness gate, perform local precheck execution, perform a local precheck,
...
from tools.pankster_runtime_security.profile_runtime_local_precheck_execution_contracts import (
    ProfileRuntimeLocalPrecheckExecutionConfig,
    ProfileRuntimeLocalPrecheckExecutionDecision,
    ProfileRuntimeLocalPrecheckExecutionRequest,
    prepare_profile_runtime_local_precheck_execution,
)
from tools.pankster_runtime_security.secret_scan import scan_secret_shapes


REQUIRED_PROFILE_RUNTIME_READINESS_GATE_CAPABILITIES = frozenset(
    {
        "profile_runtime_local_precheck_execution_contract_ready",
...
class ProfileRuntimeReadinessGateIdentity:
    readiness_gate_name: str
    readiness_gate_version: str
    readiness_gate_contract_version: str
    capabilities: tuple[str, ...]


@dataclass(frozen=True)
class ProfileRuntimeReadinessGateConfig:
    """Disabled-by-default profile runtime readiness gate controls."""

    profile_runtime_readiness_gate_contract_enabled: bool = False
    ...
    profile_runtime_readiness_gate_enabled: bool = False
...
@dataclass(frozen=True)
class ProfileRuntimeReadinessGateRequest:
    readiness_gate_identity: ProfileRuntimeReadinessGateIdentity
    profile_runtime_local_precheck_execution_request: ProfileRuntimeLocalPrecheckExecutionRequest
    ...
    expected_profile_runtime_local_precheck_execution_policy_id: str
    profile_runtime_readiness_gate_policy_id: str


@dataclass(frozen=True)
class ProfileRuntimeReadinessGateDecision:
    allowed: bool
    reason: str
    profile_runtime_local_precheck_execution_decision: ProfileRuntimeLocalPrecheckExecutionDecision | None = None
    profile_runtime_readiness_gate_manifest: Mapping[str, str] | None = None
    ...
    readiness_gate_opened: bool = False
...
def prepare_profile_runtime_readiness_gate(
    *,
    request: ProfileRuntimeReadinessGateRequest,
    config: ProfileRuntimeReadinessGateConfig | None = None,
    audit_sink: AuditSinkState,
    broker_available: bool,
) -> ProfileRuntimeReadinessGateDecision:
    """Prepare readiness gate manifest without opening readiness gate."""

    resolved_config = config or ProfileRuntimeReadinessGateConfig()
    config_reason = _config_denial_reason(resolved_config)
    if config_reason is not None:
        return _deny(config_reason)

    identity_reason = _readiness_gate_identity_reason(request.readiness_gate_identity)
    ...
    execution_decision = prepare_profile_runtime_local_precheck_execution(
        request=request.profile_runtime_local_precheck_execution_request,
        config=ProfileRuntimeLocalPrecheckExecutionConfig(
            profile_runtime_local_precheck_execution_contract_enabled=resolved_config.profile_runtime_local_precheck_execution_contract_enabled,
            ...
            profile_runtime_local_precheck_execution_enabled=resolved_config.profile_runtime_local_precheck_execution_enabled,
    ...
    if execution_decision.reason != "PROFILE_RUNTIME_LOCAL_PRECHECK_EXECUTION_CONTRACT_READY_NO_LOCAL_PRECHECK_EXECUTION_PERFORMED":
        return ProfileRuntimeReadinessGateDecision(False, execution_decision.reason, execution_decision)

    execution_manifest = execution_decision.profile_runtime_local_precheck_execution_manifest or {}
    readiness_manifest = {
        "readiness_gate_name": request.readiness_gate_identity.readiness_gate_name,
        "readiness_gate_version": request.readiness_gate_identity.readiness_gate_version,
        "readiness_gate_contract_version": request.readiness_gate_identity.readiness_gate_contract_version,
        "local_precheck_execution_name": execution_manifest["local_precheck_execution_name"],
        "local_precheck_name": execution_manifest["local_precheck_name"],
        "synthetic_dry_run_name": execution_manifest["synthetic_dry_run_name"],
        "synthetic_invocation_name": execution_manifest["synthetic_invocation_name"],
        "invocation_name": execution_manifest["invocation_name"],
        "activation_execution_name": execution_manifest["activation_execution_name"],
        ...
        "profile_runtime_local_precheck_execution_policy_id": request.expected_profile_runtime_local_precheck_execution_policy_id,
        "profile_runtime_readiness_gate_policy_id": request.profile_runtime_readiness_gate_policy_id,
        "profile_runtime_readiness_gate_state": "disabled_contract_ready_no_readiness_gate_opened",
    }
    scan = scan_secret_shapes(readiness_manifest)
    if not scan.allowed:
        return ProfileRuntimeReadinessGateDecision(
            False,
            "PROFILE_RUNTIME_READINESS_GATE_MANIFEST_SECRET_SCAN_FAILED",
            execution_decision,
        )
    return ProfileRuntimeReadinessGateDecision(
        allowed=False,
        reason="PROFILE_RUNTIME_READINESS_GATE_CONTRACT_READY_NO_GATE_OPENED",
        profile_runtime_local_precheck_execution_decision=execution_decision,
        profile_runtime_readiness_gate_manifest=readiness_manifest,
    )


def _config_denial_reason(config: ProfileRuntimeReadinessGateConfig) -> str | None:
    if not config.profile_runtime_readiness_gate_contract_enabled:
        return "PROFILE_RUNTIME_READINESS_GATE_DISABLED"
    if config.profile_runtime_readiness_gate_enabled:
        return "PROFILE_RUNTIME_READINESS_GATE_OUT_OF_SCOPE"
...
def _readiness_gate_identity_reason(identity: ProfileRuntimeReadinessGateIdentity) -> str | None:
    for field, value in (
        ("readiness_gate_name", identity.readiness_gate_name),
        ("readiness_gate_version", identity.readiness_gate_version),
        ("readiness_gate_contract_version", identity.readiness_gate_contract_version),
    ):
        if not isinstance(value, str) or not value.strip():
            return f"PROFILE_RUNTIME_READINESS_GATE_IDENTITY_FIELD_MISSING:{field}"
    missing = sorted(REQUIRED_PROFILE_RUNTIME_READINESS_GATE_CAPABILITIES - set(identity.capabilities))
    if missing:
        return f"PROFILE_RUNTIME_READINESS_GATE_CAPABILITY_MISSING:{missing[0]}"
    return None


def _request_reason(request: ProfileRuntimeReadinessGateRequest) -> str | None:
    execution_request = request.profile_runtime_local_precheck_execution_request
    ...
    if request.expected_profile_runtime_activation_execution_policy_id != execution_request.expected_profile_runtime_activation_execution_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_ACTIVATION_EXECUTION_POLICY_MISMATCH"
    if request.expected_profile_runtime_invocation_policy_id != execution_request.expected_profile_runtime_invocation_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_INVOCATION_POLICY_MISMATCH"
    if request.expected_profile_runtime_synthetic_invocation_policy_id != execution_request.expected_profile_runtime_synthetic_invocation_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_SYNTHETIC_INVOCATION_POLICY_MISMATCH"
    if request.expected_profile_runtime_synthetic_dry_run_policy_id != execution_request.expected_profile_runtime_synthetic_dry_run_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_SYNTHETIC_DRY_RUN_POLICY_MISMATCH"
    if request.expected_profile_runtime_local_precheck_policy_id != execution_request.expected_profile_runtime_local_precheck_policy_id:
        return "EXPECTED_PROFILE_RUNTIME_LOCAL_PRECHECK_POLICY_MISMATCH"
    if (
        request.expected_profile_runtime_local_precheck_execution_policy_id
        != execution_request.profile_runtime_local_precheck_execution_policy_id
    ):
        return "EXPECTED_PROFILE_RUNTIME_LOCAL_PRECHECK_EXECUTION_POLICY_MISMATCH"
    if not isinstance(request.profile_runtime_readiness_gate_policy_id, str) or not request.profile_runtime_readiness_gate_policy_id.strip():
        return "PROFILE_RUNTIME_READINESS_GATE_POLICY_MISSING"
    return None


def _deny(reason: str) -> ProfileRuntimeReadinessGateDecision:
    return ProfileRuntimeReadinessGateDecision(False, reason)
