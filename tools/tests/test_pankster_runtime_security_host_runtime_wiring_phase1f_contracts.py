...
from tools.pankster_runtime_security.host_runtime_wiring_phase1f_contracts import (
    PHASE_1F_A20_APPROVAL_COMMAND_SHA256,
    PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_FILE_ALLOWLIST,
    Phase1FVersionedHostRuntimeWiringConfig,
    Phase1FVersionedHostRuntimeWiringRequest,
    Phase1FVersionedHostRuntimeWiringScopeAttestation,
    phase_1f_versioned_host_runtime_wiring_scope_manifest,
    prepare_phase_1f_versioned_host_runtime_wiring,
    validate_phase_1f_versioned_host_runtime_wiring_scope,
)
...
def adapter_identity(**overrides):
    values = {
        ...
        "adapter_version": "1",
        "adapter_contract_version": "phase-1f-a21",
...
def host_identity(**overrides):
    values = {
        ...
        "host_contract_version": "phase-1f-a21",
...
def execution_identity(**overrides):
    values = {
        ...
        "executor_version": "1",
        "execution_contract_version": "phase-1f-a21",
...
def wiring_identity(**overrides):
    values = {
        ...
        "wiring_version": "1",
        "wiring_contract_version": "phase-1f-a21",
...
def base_wiring_request(**overrides):
    base_execution = overrides.pop("execution_request", execution_request())
    values = {
        "wiring_identity": wiring_identity(),
        "execution_request": base_execution,
        "expected_profile_id": "dev-director",
        "expected_policy_version": "policy-v1",
        "expected_runtime_backend": "disabled-local-contract",
        "expected_rollback_policy_id": "rollback-policy-v1",
...
def versioned_binding_request(**overrides):
    values = {
        ...
        "expected_contract_version": "phase-1f-a21",
    }
    values.update(overrides)
...
def versioned_host_request(**overrides):
    ...
    values = {
        ...
        "expected_host_contract_version": "phase-1f-a21",
    }
    values.update(overrides)
...
def versioned_execution_request(**overrides):
    base_binding = binding_request()
    base_host = host_request(binding_request=base_binding)
    base_execution = overrides.pop("base_execution_request", execution_request(host_request=base_host))
    values = {
        "base_execution_request": base_execution,
        ...
        "expected_execution_contract_version": "phase-1f-a21",
    }
    values.update(overrides)
    return Phase1FVersionedHostRuntimeExecutionRequest(**values)


def wiring_scope_attestation(**overrides):
    values = {
        "owner_approval_command_sha256": PHASE_1F_A20_APPROVAL_COMMAND_SHA256,
        "changed_files": (
    ...
    values.update(overrides)
    return Phase1FVersionedHostRuntimeWiringScopeAttestation(**values)


def phase1f_wiring_request(**overrides):
    base_binding = binding_request()
    base_host = host_request(binding_request=base_binding)
    base_execution = execution_request(host_request=base_host)
    values = {
        "base_wiring_request": base_wiring_request(execution_request=base_execution),
    ...
    values.update(overrides)
    return Phase1FVersionedHostRuntimeWiringRequest(**values)
...
def enabled_config(**overrides):
    ...
    values.update(overrides)
    return Phase1FVersionedHostRuntimeWiringConfig(**values)


class PanksterRuntimeSecurityHostRuntimeWiringPhase1FContractTests(unittest.TestCase):
    def test_phase_1f_host_wiring_scope_accepts_only_new_allowlisted_files(self):
        decision = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation())

        self.assertTrue(decision.allowed)
        self.assertEqual(decision.reason, "PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_PURE_CONTRACT_SCOPE_ACCEPTED_NO_GATEWAY_WIRED")
        self.assertEqual(decision.approved_file_scope, tuple(sorted(PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_FILE_ALLOWLIST)))
        self.assertFalse(decision.gateway_wired)
    ...
    def test_phase_1f_host_wiring_scope_rejects_bad_approval_duplicate_missing_pinned_and_unknown_files(self):
        bad_approval = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation(owner_approval_command_sha256="bad"))
        duplicate = validate_phase_1f_versioned_host_runtime_wiring_scope(
            wiring_scope_attestation(changed_files=("tools/pankster_runtime_security/host_runtime_wiring_phase1f_contracts.py", "tools/pankster_runtime_security/host_runtime_wiring_phase1f_contracts.py"))
        )
        missing = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation(changed_files=()))
        pinned = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation(changed_files=("tools/pankster_runtime_security/host_runtime_wiring_contracts.py",)))
        unknown = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation(changed_files=("gateway.py",)))
        ...
        self.assertEqual(pinned.reason, "PHASE_1E_HASH_PINNED_FILE_FORBIDDEN:tools/pankster_runtime_security/host_runtime_wiring_contracts.py")
        self.assertEqual(unknown.reason, "IMPLEMENTATION_FILE_OUT_OF_SCOPE:gateway.py")

    def test_phase_1f_host_wiring_scope_rejects_wiring_runtime_credentials_api_and_deploy_flags(self):
        for flag in (
            ...
            decision = validate_phase_1f_versioned_host_runtime_wiring_scope(wiring_scope_attestation(**{flag: True}))

            self.assertFalse(decision.allowed)
            self.assertEqual(decision.reason, f"IMPLEMENTATION_SCOPE_FLAG_FORBIDDEN:{flag}")

    def test_phase_1f_host_wiring_scope_manifest_contains_no_secret_or_runtime_state(self):
        manifest = phase_1f_versioned_host_runtime_wiring_scope_manifest(wiring_scope_attestation())

        self.assertEqual(manifest["host_wiring_scope_allowed"], "true")
        self.assertEqual(manifest["host_wiring_approval_command_sha256"], PHASE_1F_A20_APPROVAL_COMMAND_SHA256)
        self.assertEqual(set(manifest["host_wiring_approved_file_scope"].split(",")), PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_FILE_ALLOWLIST)

    def test_phase_1f_host_wiring_is_disabled_by_default_and_wires_nothing(self):
        decision = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )

        self.assertFalse(decision.allowed)
        self.assertEqual(decision.reason, "PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_DISABLED")
        self.assertFalse(decision.gateway_wired)
        self.assertFalse(decision.profile_worker_wired)
        self.assertFalse(decision.runtime_started)
        self.assertFalse(decision.runtime_bound)
    ...
    def test_phase_1f_host_wiring_prepares_secret_free_manifest_without_gateway_wiring(self):
        decision = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(),
        ...
        self.assertFalse(decision.allowed)
        self.assertEqual(decision.reason, "PHASE_1F_VERSIONED_HOST_RUNTIME_WIRING_CONTRACT_READY_NO_GATEWAY_WIRED")
        self.assertTrue(decision.implementation_scope_decision.allowed)
        self.assertEqual(decision.versioned_execution_decision.reason, "PHASE_1F_VERSIONED_HOST_RUNTIME_EXECUTION_CONTRACT_READY_NO_RUNTIME_STARTED")
        self.assertEqual(decision.base_wiring_decision.reason, "HOST_RUNTIME_WIRING_CONTRACT_READY_NO_GATEWAY_WIRED")
        self.assertEqual(decision.wiring_manifest["phase"], "1F-A21")
        ...
        self.assertEqual(decision.wiring_manifest["wiring_state"], "versioned_disabled_contract_ready_no_gateway_wired")
        self.assertNotIn("api_key", str(decision.wiring_manifest).lower())
        self.assertNotIn("token", str(decision.wiring_manifest).lower())
        self.assertFalse(decision.gateway_wired)
        self.assertFalse(decision.profile_worker_wired)
        self.assertFalse(decision.runtime_started)

    def test_phase_1f_host_wiring_rejects_missing_scope_bad_scope_mismatch_and_bad_version(self):
        missing_scope = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(implementation_scope_attestation=None),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        bad_scope = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(implementation_scope_attestation=wiring_scope_attestation(changed_files=("web_server.py",))),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        mismatch = prepare_phase_1f_versioned_host_runtime_wiring(
            request=Phase1FVersionedHostRuntimeWiringRequest(
                base_wiring_request=base_wiring_request(execution_request=execution_request(expected_profile_id="dev-director")),
        ...
        bad_version = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(expected_wiring_contract_version=""),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )

        self.assertEqual(missing_scope.reason, "HOST_WIRING_IMPLEMENTATION_SCOPE_ATTESTATION_MISSING")
        self.assertEqual(bad_scope.reason, "IMPLEMENTATION_FILE_OUT_OF_SCOPE:web_server.py")
        self.assertEqual(mismatch.reason, "VERSIONED_EXECUTION_REQUEST_MISMATCH")
        self.assertEqual(bad_version.reason, "EXPECTED_WIRING_CONTRACT_VERSION_MISSING")

    def test_phase_1f_host_wiring_rejects_gateway_profile_worker_runtime_flags_and_propagates_fail_closed(self):
        for kwargs, reason in (
            ({"gateway_wiring_enabled": True}, "GATEWAY_WIRING_OUT_OF_SCOPE"),
            ({"profile_worker_wiring_enabled": True}, "PROFILE_WORKER_WIRING_OUT_OF_SCOPE"),
            ({"hermes_core_wiring_enabled": True}, "HERMES_CORE_WIRING_OUT_OF_SCOPE"),
        ...
        audit_denied = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(False),
            broker_available=True,
        )
        broker_denied = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=False,
        )
        disabled_execution_denied = prepare_phase_1f_versioned_host_runtime_wiring(
            request=phase1f_wiring_request(),
        ...
        self.assertEqual(disabled_execution_denied.reason, "PHASE_1F_VERSIONED_HOST_RUNTIME_EXECUTION_DISABLED")
