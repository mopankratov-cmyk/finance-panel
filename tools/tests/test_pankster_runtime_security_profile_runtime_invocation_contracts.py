...
from tools.pankster_runtime_security.profile_runtime_invocation_contracts import (
    REQUIRED_PROFILE_RUNTIME_INVOCATION_CAPABILITIES,
    ProfileRuntimeInvocationConfig,
    ProfileRuntimeInvocationIdentity,
    ProfileRuntimeInvocationRequest,
    prepare_profile_runtime_invocation,
)
from tools.tests.test_pankster_runtime_security_profile_runtime_activation_execution_contracts import (
    profile_runtime_activation_execution_request,
)


def profile_runtime_invocation_identity(**overrides):
    values = {
        "invocation_name": "local-disabled-profile-runtime-invocation",
        "invocation_version": "1",
        "invocation_contract_version": "phase-1e-a39",
        "capabilities": tuple(sorted(REQUIRED_PROFILE_RUNTIME_INVOCATION_CAPABILITIES)),
    }
    values.update(overrides)
    return ProfileRuntimeInvocationIdentity(**values)


def profile_runtime_invocation_request(**overrides):
    values = {
        "invocation_identity": profile_runtime_invocation_identity(),
        "profile_runtime_activation_execution_request": profile_runtime_activation_execution_request(),
        ...
        "profile_runtime_invocation_policy_id": "profile-runtime-invocation-policy-v1",
    }
    values.update(overrides)
    return ProfileRuntimeInvocationRequest(**values)
...
def enabled_config(**overrides):
    ...
    values.update(overrides)
    return ProfileRuntimeInvocationConfig(**values)


class PanksterRuntimeSecurityProfileRuntimeInvocationContractTests(unittest.TestCase):
    def test_default_profile_runtime_invocation_is_disabled_and_starts_nothing(self):
        decision = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )

        self.assertFalse(decision.allowed)
        self.assertEqual(decision.reason, "PROFILE_RUNTIME_INVOCATION_DISABLED")
    ...
    def test_enabled_profile_runtime_invocation_prepares_secret_free_manifest_without_invocation(self):
        decision = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(),
        ...
        self.assertFalse(decision.allowed)
        self.assertEqual(decision.reason, "PROFILE_RUNTIME_INVOCATION_CONTRACT_READY_NO_RUNTIME_INVOKED")
        self.assertEqual(
            decision.profile_runtime_activation_execution_decision.reason,
            "PROFILE_RUNTIME_ACTIVATION_EXECUTION_CONTRACT_READY_NO_ACTIVATION_EXECUTED",
        )
        self.assertEqual(decision.profile_runtime_invocation_manifest["profile_id"], "dev-director")
        self.assertEqual(decision.profile_runtime_invocation_manifest["runtime_backend"], "disabled-local-contract")
        self.assertEqual(
            decision.profile_runtime_invocation_manifest["profile_runtime_invocation_policy_id"],
            "profile-runtime-invocation-policy-v1",
        )
        self.assertEqual(
            decision.profile_runtime_invocation_manifest["profile_runtime_invocation_state"],
            "disabled_contract_ready_no_runtime_invoked",
        )
        self.assertNotIn("credential", str(decision.profile_runtime_invocation_manifest).lower())
        self.assertFalse(decision.profile_started)
        self.assertFalse(decision.runtime_started)
        self.assertFalse(decision.activation_executed)
        self.assertFalse(decision.invocation_performed)

    def test_profile_runtime_invocation_rejects_out_of_scope_runtime_flags(self):
        cases = [
    ...
    def test_profile_runtime_invocation_requires_identity_capabilities(self):
        missing_name = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(
                invocation_identity=profile_runtime_invocation_identity(invocation_name="")
            ),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        missing_capability = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(
                invocation_identity=profile_runtime_invocation_identity(capabilities=("fail_closed",))
            ),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        ...
        self.assertEqual(missing_name.reason, "PROFILE_RUNTIME_INVOCATION_IDENTITY_FIELD_MISSING:invocation_name")
        self.assertTrue(missing_capability.reason.startswith("PROFILE_RUNTIME_INVOCATION_CAPABILITY_MISSING:"))

    def test_profile_runtime_invocation_revalidates_profile_policy_backend_and_execution_ids(self):
        cases = [
            ...
            ({"profile_runtime_invocation_policy_id": ""}, "PROFILE_RUNTIME_INVOCATION_POLICY_MISSING"),
        ]
        for overrides, reason in cases:
            with self.subTest(reason=reason):
                decision = prepare_profile_runtime_invocation(
                    request=profile_runtime_invocation_request(**overrides),
                    config=enabled_config(),
                    audit_sink=AuditSinkState(True),
                    broker_available=True,
                )
    ...
    def test_profile_runtime_invocation_propagates_activation_execution_fail_closed_reasons(self):
        audit_denied = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(False),
            broker_available=True,
        )
        broker_denied = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=False,
        )
        disabled_execution_denied = prepare_profile_runtime_invocation(
            request=profile_runtime_invocation_request(),
            config=enabled_config(profile_runtime_activation_execution_contract_enabled=False),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        ...
        self.assertEqual(disabled_execution_denied.reason, "PROFILE_RUNTIME_ACTIVATION_EXECUTION_DISABLED")
