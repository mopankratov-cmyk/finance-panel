...
from tools.pankster_runtime_security.profile_runtime_local_precheck_contracts import (
    REQUIRED_PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITIES,
    ProfileRuntimeLocalPrecheckConfig,
    ProfileRuntimeLocalPrecheckIdentity,
    ProfileRuntimeLocalPrecheckRequest,
    prepare_profile_runtime_local_precheck,
)
from tools.tests.test_pankster_runtime_security_profile_runtime_synthetic_dry_run_contracts import (
    profile_runtime_synthetic_dry_run_request,
)


def profile_runtime_local_precheck_identity(**overrides):
    values = {
        "local_precheck_name": "local-disabled-profile-runtime-local-precheck",
        "local_precheck_version": "1",
        "local_precheck_contract_version": "phase-1e-a48",
        "capabilities": tuple(sorted(REQUIRED_PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITIES)),
    }
    values.update(overrides)
    return ProfileRuntimeLocalPrecheckIdentity(**values)


def profile_runtime_local_precheck_request(**overrides):
    values = {
        "local_precheck_identity": profile_runtime_local_precheck_identity(),
        "profile_runtime_synthetic_dry_run_request": profile_runtime_synthetic_dry_run_request(),
        ...
        "profile_runtime_local_precheck_policy_id": "profile-runtime-local-precheck-policy-v1",
    }
    values.update(overrides)
    return ProfileRuntimeLocalPrecheckRequest(**values)
...
def enabled_config(**overrides):
    ...
    values.update(overrides)
    return ProfileRuntimeLocalPrecheckConfig(**values)


class PanksterRuntimeSecurityProfileRuntimeLocalPrecheckContractTests(unittest.TestCase):
    def test_default_profile_runtime_local_precheck_is_disabled_and_starts_nothing(self):
        decision = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )

        self.assertFalse(decision.allowed)
        self.assertEqual(decision.reason, "PROFILE_RUNTIME_LOCAL_PRECHECK_DISABLED")
    ...
    def test_enabled_profile_runtime_local_precheck_prepares_secret_free_manifest_without_precheck(self):
        decision = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(),
        ...
        self.assertEqual(
            decision.profile_runtime_synthetic_dry_run_decision.reason,
            "PROFILE_RUNTIME_SYNTHETIC_DRY_RUN_CONTRACT_READY_NO_SYNTHETIC_DRY_RUN_PERFORMED",
        )
        self.assertEqual(decision.profile_runtime_local_precheck_manifest["profile_id"], "dev-director")
        self.assertEqual(decision.profile_runtime_local_precheck_manifest["runtime_backend"], "disabled-local-contract")
        self.assertEqual(
            decision.profile_runtime_local_precheck_manifest["profile_runtime_local_precheck_policy_id"],
            "profile-runtime-local-precheck-policy-v1",
        )
        self.assertEqual(
            decision.profile_runtime_local_precheck_manifest["profile_runtime_local_precheck_state"],
            "disabled_contract_ready_no_local_precheck_performed",
        )
        self.assertNotIn("credential", str(decision.profile_runtime_local_precheck_manifest).lower())
        self.assertFalse(decision.profile_started)
        self.assertFalse(decision.runtime_started)
        self.assertFalse(decision.activation_executed)
        self.assertFalse(decision.invocation_performed)
    ...
    def test_profile_runtime_local_precheck_rejects_out_of_scope_runtime_flags(self):
        cases = [
    ...
    def test_profile_runtime_local_precheck_requires_identity_capabilities(self):
        missing_name = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(
                local_precheck_identity=profile_runtime_local_precheck_identity(local_precheck_name="")
            ),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        missing_capability = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(
                local_precheck_identity=profile_runtime_local_precheck_identity(capabilities=("fail_closed",))
            ),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        ...
        self.assertEqual(
            missing_name.reason,
            "PROFILE_RUNTIME_LOCAL_PRECHECK_IDENTITY_FIELD_MISSING:local_precheck_name",
        )
        self.assertTrue(missing_capability.reason.startswith("PROFILE_RUNTIME_LOCAL_PRECHECK_CAPABILITY_MISSING:"))

    def test_profile_runtime_local_precheck_revalidates_profile_policy_backend_and_dry_run_ids(self):
        cases = [
            ...
            ({"profile_runtime_local_precheck_policy_id": ""}, "PROFILE_RUNTIME_LOCAL_PRECHECK_POLICY_MISSING"),
        ]
        for overrides, reason in cases:
            with self.subTest(reason=reason):
                decision = prepare_profile_runtime_local_precheck(
                    request=profile_runtime_local_precheck_request(**overrides),
                    config=enabled_config(),
                    audit_sink=AuditSinkState(True),
                    broker_available=True,
                )
    ...
    def test_profile_runtime_local_precheck_propagates_synthetic_dry_run_fail_closed_reasons(self):
        audit_denied = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(False),
            broker_available=True,
        )
        broker_denied = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(),
            config=enabled_config(),
            audit_sink=AuditSinkState(True),
            broker_available=False,
        )
        disabled_dry_run_denied = prepare_profile_runtime_local_precheck(
            request=profile_runtime_local_precheck_request(),
            config=enabled_config(profile_runtime_synthetic_dry_run_contract_enabled=False),
            audit_sink=AuditSinkState(True),
            broker_available=True,
        )
        ...
        self.assertEqual(disabled_dry_run_denied.reason, "PROFILE_RUNTIME_SYNTHETIC_DRY_RUN_DISABLED")
