...
from tools.phase_1e_a11_runtime_integration_approval_request_validator import DEFAULT_EVIDENCE, EXPECTED_APPROVAL, EXPECTED_APPROVAL_SHA, validate_evidence


class Phase1EA11RuntimeIntegrationApprovalRequestValidatorTests(unittest.TestCase):
    def test_1e_a11_evidence_validates_approval_request(self):
    ...
    def test_1e_a11_records_exact_approval_command(self):
    ...
    def test_1e_a11_scope_allows_only_disabled_contracts_and_tests(self):
        evidence = json.loads(DEFAULT_EVIDENCE.read_text(encoding="utf-8"))
        ...
        for field, value in scope.items():
            if field not in {"disabled_by_default_integration_contracts_allowed", "tools_runtime_security_files_allowed", "unit_tests_allowed", "local_static_validation_allowed", "local_unittest_allowed"}:
                self.assertFalse(value)

    def test_1e_a11_future_file_scope_is_narrow(self):
        evidence = json.loads(DEFAULT_EVIDENCE.read_text(encoding="utf-8"))
    ...
    def test_1e_a11_records_targeted_test_pass_and_next_gate(self):
        evidence = json.loads(DEFAULT_EVIDENCE.read_text(encoding="utf-8"))
        ...
        self.assertEqual(content["next_gate"], "PHASE_1E_A12_DISABLED_RUNTIME_INTEGRATION_CONTRACT_AFTER_OWNER_APPROVAL")
